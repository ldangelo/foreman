...
export async function purgeLogsAction(opts, store, logsDir) {
    ...
    if (dryRun) {
        console.log(chalk.yellow(`Dry run complete — ${result.deleted} log group(s) would be deleted (${freedStr}), ${result.skipped} skipped, ${result.errors} error(s).`));
        console.log(chalk.dim("Run without --dry-run to apply changes."));
    }
    else {
        const color = result.errors > 0 ? chalk.yellow : chalk.green;
        console.log(color(`Done — ${result.deleted} log group(s) deleted (${freedStr}), ${result.skipped} skipped, ${result.errors} error(s).`));
    }
    return result;
}
// ── CLI Command ──────────────────────────────────────────────────────
export const purgeLogsCommand = new Command("purge-logs")
    .description("Remove old agent log files from ~/.foreman/logs/ based on a retention policy")
    .option("--days <n>", "Delete logs from runs older than N days (default: 7)", (v) => {
