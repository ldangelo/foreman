...
export class ConflictPatterns {
     ...
     */
    recordOutcome(filePath, extension, tier, success, failureReason, mergeQueueId, seedId) {
        this.db
            .prepare(`INSERT INTO conflict_patterns
           (file_path, file_extension, tier, success, failure_reason, merge_queue_id, seed_id, recorded_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(filePath, extension, tier, success ? 1 : 0, failureReason ?? null, mergeQueueId ?? null, seedId ?? null, new Date().toISOString());
    }
    /**
     ...
     */
    getSuccessContext(extension, tier) {
        const rows = this.db
            .prepare(`SELECT file_path FROM conflict_patterns
     ...
     */
    recordTestFailure(aiResolvedFiles, mergeQueueId) {
        const now = new Date().toISOString();
        const stmt = this.db.prepare(`INSERT INTO conflict_patterns
         (file_path, file_extension, tier, success, failure_reason, merge_queue_id, seed_id, recorded_at)
...
//# sourceMappingURL=conflict-patterns.js.map