...
- Phase sequence, reusable action (`prepare-worktree`, `setup-workspace`, `write-task-context`, `prompt-agent`, `bash`, `finalize`, PR gates, merge), models, retries, validation policies, mail hooks, artifacts all defined in YAML; project/global `.foreman/actions` / `~/.foreman/actions` JS/MJS overrides load at runtime without rebuild, stubs install by init/doctor/`foreman actions install`, and `foreman actions validate` checks exports plus unresolved workflow action refs
