...
function printSlingPlan(plan, parallel) {
    ...
    for (let si = 0; si < plan.sprints.length; si++) {
        ...
        const completed = sprint.stories.reduce((sum, st) => sum + st.tasks.filter((t) => t.status === "completed").length, 0);
...
const trdSubcommand = new Command("trd")
    ...
    // Read TRD file
    const resolved = resolve(trdFile);
    ...
    if (!opts.auto) {
        const targets = [];
        if (!opts.brOnly)
            targets.push("sd (beads)");
        if (!opts.sdOnly)
            targets.push("br (beads_rust)");
        const answer = await new Promise((resolve) => {
            const rl = createInterface({ input: process.stdin, output: process.stdout });
            rl.question(chalk.bold(`Create in ${targets.join(" + ")}? [y/N] `), (ans) => {
                rl.close();
                resolve(ans);
            });
        });
    ...
    const result = await execute(plan, parallel, slingOptions, seeds, beadsRust, spinner.update);
    spinner.finish();
    // Summary
    printSummary(result);
});
