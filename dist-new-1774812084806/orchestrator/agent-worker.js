 ...
 */
async function runPhase(role, prompt, config, progress, logFile, store, notifyClient, agentMailClient) {
 ...
 */
async function runPipeline(config, store, logFile, notifyClient, agentMailClient) {
    ...
    await executePipeline({
        ...
        async onPipelineComplete({ progress }) {
            ...
            if (agentMailClient) {
                ...
                const finalizeMsg = foremanMsgs.find((m) => (m.subject === "phase-complete" || m.subject === "agent-error") &&
                ...
                if (finalizeMsg?.subject === "phase-complete") {
                    ...
                    const body = (() => { try {
                        return JSON.parse(finalizeMsg.body ?? "{}");
                    }
                    catch {
                        return {};
                    } })();
...
async function markStuck(store, runId, projectId, seedId, seedTitle, progress, phase, reason, notifyClient, projectPath) {
    const isRateLimit = reason.includes("hit your limit") || reason.includes("rate limit");
    const now = new Date().toISOString();
