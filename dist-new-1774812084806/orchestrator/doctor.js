...
export class Doctor {
     ...
     */
    execFn;
    constructor(store, projectPath, mergeQueue, taskClient, execFn) {
        this.store = store;
    ...
    async checkFailedStuckRuns(opts = {}) {
        ...
        const { actionable: actionableFailed, historical: historicalFailed } = this.partitionByHistoricalRetry(unresolvedFailed);
     ...
     */
    partitionByHistoricalRetry(runs) {
        const actionable = [];
        const historical = [];
        for (const run of runs) {
    ...
    async checkCompletedRunsNotQueued(opts = {}) {
        ...
        if (fix && opts.projectPath) {
            try {
                const result = await this.mergeQueue.reconcile(this.store.getDb(), opts.projectPath);
                return {
                    name: "completed runs queued",
                    status: "fixed",
    ...
    async checkDataIntegrity(opts = {}) {
        const results = [];
        const [worktreeResults, zombieResults, staleResult, failedStuckResults, consistencyResults, blockedResult, recoveryResult, beadSyncResult] = await Promise.all([
