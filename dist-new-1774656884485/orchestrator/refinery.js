...
export class Refinery {
    ...
    projectPath;
    conflictResolver;
    constructor(store, seeds, projectPath) {
        this.store = store;
        this.seeds = seeds;
        ...
        try {
            ...
            for (const stackedRun of stackedRuns) {
                ...
                try {
                    ...
                    try {
                        await git(["rebase", "--abort"], this.projectPath);
                    }
                    catch { /* already clean */ }
                }
            }
                ...
                // Auto-resolves report-file conflicts during rebase; aborts on real code conflicts.
                {
                    ...
                    if (!rebaseOk) {
                        ...
                        if (stashedBeforeRebase) {
                            try {
                                await git(["stash", "pop"], this.projectPath);
                            }
                            catch { /* best effort */ }
                        }
    ...
    async createPRs(opts) {
        ...
        for (const run of completedRuns) {
            ...
            try {
                ...
                let commitLog = "";
                try {
                    commitLog = await git(["log", `${baseBranch}..${branchName}`, "--oneline"], this.projectPath);
                }
                catch {
                    // Non-fatal
                }
...
export async function preserveBeadChanges(projectPath, branchName, targetBranch) {
    ...
    try {
        ...
        await git(["commit", "-m", `chore: preserve seed changes from ${seedId}`], projectPath);
