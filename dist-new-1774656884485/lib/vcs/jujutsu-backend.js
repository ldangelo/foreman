...
export class JujutsuBackend {
    ...
    async branchExists(repoPath, branchName) {
        try {
            const out = await this.jj(["bookmark", "list", `--name=${branchName}`], repoPath);
    ...
    async branchExistsOnRemote(repoPath, branchName) {
        try {
            const out = await this.jj(["bookmark", "list", `--name=${branchName}`, "--remote=origin"], repoPath);
            return out.includes(branchName);
        }
        catch {
            return false;
    ...
    async createWorkspace(repoPath, seedId, baseBranch) {
        ...
        try {
            await this.jj(["bookmark", "create", branchName, "-r", `@${`foreman-${seedId}`}`], repoPath);
        }
        catch {
            // Bookmark may already exist — try to move it
            try {
                await this.jj(["bookmark", "move", branchName, "--to", `@${`foreman-${seedId}`}`], repoPath);
            }
            catch (moveErr) {
    ...
    async getHeadId(workspacePath) {
        return this.jj(["log", "--no-graph", "-r", "@", "-T", "change_id"], workspacePath);
    }
    /**
