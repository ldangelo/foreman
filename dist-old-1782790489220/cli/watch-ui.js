 ...
 */
export function readLastErrorLines(runId, n = 5) {
    try {
 ...
 */
export function renderAgentCardSummary(run, progress, index, attemptNumber, previousStatus) {
 ...
 */
export function renderAgentCard(run, progress, isExpanded = true, index, attemptNumber, previousStatus, showErrorLogs = false) {
    if (!isExpanded) {
    ...
    const attemptInfo = attemptNumber && attemptNumber > 1
        ? chalk.dim(` (attempt ${attemptNumber}${previousStatus ? ", prev: " + previousStatus : ""})`)
        : "";
    lines.push(`${indexPrefix}${collapseIndicator} ${statusColor(run.status, icon)} ${chalk.cyan.bold(run.seed_id)} ${statusColor(run.status, run.status.toUpperCase())} ${chalk.dim(time)}${attemptInfo}`);
...
export async function poll(store, runIds) {
    const entries = [];
    let totalCost = 0;
    let totalTools = 0;
    let totalFiles = 0;
    ...
    const failedCount = latestRuns.filter((run) => run.status === "failed" || run.status === "test-failed").length;
 ...
 */
export function renderWatchDisplay(state, showDetachHint = true, expandedRunIds, notification, showErrorLogs = false) {
    ...
    if (showDetachHint && !state.allDone) {
        const hintParts = [chalk.dim("Ctrl+C to detach")];
    ...
    const successRatePart = state.successRate !== undefined
        ...
        : "";
    lines.push(`${chalk.dim(String(state.runs.length) + " agents")}  ` +
    ...
    if (state.allDone) {
        ...
        if (state.failedCount > 0)
            parts.push(chalk.red(`${state.failedCount} failed`));
        if (state.stuckCount > 0)
            parts.push(chalk.yellow(`${state.stuckCount} rate-limited`));
...
export async function watchRunsInk(store, runIds, opts) {
    ...
    let autoDispatchNotification = null;
    ...
    try {
        while (!detached) {
            ...
            const currentActiveCount = state.runs.filter((e) => e.run.status === "pending" || e.run.status === "running").length;
...
//# sourceMappingURL=watch-ui.js.map