...
export async function createSentinelTaskClient(projectPath) {
    const { taskClient } = await createTaskClient(projectPath);
    return taskClient;
}
function wrapLocalSentinelStore(store) {
    return {
        close: () => store.close(),
        isOpen: () => store.isOpen(),
        logEvent: async (projectId, eventType, data) => store.logEvent(projectId, eventType, data),
        recordSentinelRun: async (run) => store.recordSentinelRun(run),
        updateSentinelRun: async (id, updates) => store.updateSentinelRun(id, updates),
        upsertSentinelConfig: async (projectId, config) => { store.upsertSentinelConfig(projectId, config); },
        getSentinelConfig: async (projectId) => store.getSentinelConfig(projectId),
        getSentinelRuns: async (projectId, limit) => store.getSentinelRuns(projectId, limit),
    };
}
export function wrapPostgresSentinelStore(store, projectId) {
    return {
        close: () => store.close(),
        isOpen: () => store.isOpen(),
        logEvent: async (pid, eventType, data) => {
        ...
        recordSentinelRun: async (run) => store.recordSentinelRun(projectId, run),
        updateSentinelRun: async (id, updates) => store.updateSentinelRun(id, updates),
        upsertSentinelConfig: async (_projectId, config) => { await store.upsertSentinelConfig(projectId, config); },
        getSentinelConfig: async (_projectId) => store.getSentinelConfig(projectId),
        getSentinelRuns: async (_projectId, limit) => store.getSentinelRuns(projectId, limit),
    };
}
// ─────────────────────────────────────────────────────────────────────────────
// Project Resolution
...
// Lockfile Management
// ─────────────────────────────────────────────────────────────────────────────
function getSentinelLockPath(projectId) {
    return join(homedir(), ".foreman", "sentinels", `${projectId}.lock`);
}
function readSentinelLock(projectId) {
...
function writeSentinelLock(projectId, lock) {
    const lockPath = getSentinelLockPath(projectId);
...
function removeSentinelLock(projectId) {
    const lockPath = getSentinelLockPath(projectId);
    if (existsSync(lockPath)) {
        try {
            unlinkSync(lockPath);
...
async function stopProjectSentinel(projectId, force) {
    // Try lockfile first
...
sentinelCommand
    ...
    try {
        ...
        if (options.dryRun)
            console.log(chalk.dim("  (dry-run mode)"));
        console.log();
...
sentinelCommand
    ...
    try {
        ...
        const failureThreshold = parseInt(opts.failureThreshold, 10);
        const options = {
            branch: opts.branch,
            testCommand: opts.testCommand,
            intervalMinutes,
