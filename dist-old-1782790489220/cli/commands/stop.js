 ...
 */
export async function stopAction(id, opts, store, projectPath) {
 ...
 */
async function stopRun(run, store, opts) {
    const { dryRun, force } = opts;
    const errors = [];
    let processKilled = false;
    console.log(`  ${chalk.cyan(run.seed_id)} ${chalk.dim(`[${run.id}]`)} status=${run.status}`);
    ...
    if (pid && isAlive(pid)) {
        ...
        console.log(`    ${chalk.yellow("warn")} no pid found — marking stuck anyway`);
    }
 ...
 */
export async function listActiveRuns(store, projectPath) {
    const project = await store.getProjectByPath(projectPath);
    if (!project) {
...
export async function stopCommandAction(id, opts) {
    let projectPath;
    let isGitRepo = true;
    try {
...
function printStopResult(run, result) {
...
async function findRun(store, id, projectId) {
...
function getDescendantProcessGroups(pid) {
