    ...
    try {
        ...
        if (opts.dryRun) {
            ...
            for (const record of result.preview.slice(0, 5)) {
                console.log(`  ${chalk.dim(record.bead.id)} → ${record.nativeId.slice(0, 8)} ` +
...
async function runElixirMigrationImport(opts) {
    ...
    const payload = opts.fromNode
        ...
        : JSON.parse(readFileSync(resolve(opts.file), "utf8"));
...
async function buildNodeMigrationPayload(opts) {
    const projectPath = await resolveProjectPathFromOptions(opts);
