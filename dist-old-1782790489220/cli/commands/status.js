...
function resolveRegisteredProject(projects, projectPath) {
    const resolvedProjectPath = resolve(projectPath);
    return projects.find((record) => resolve(record.path) === resolvedProjectPath) ?? null;
}
function renderableRunFromElixir(run) {
    return {
...
function computeStatusCountsFromElixirTasks(tasks) {
...
export async function fetchDaemonStatusSnapshot(projectPath) {
    try {
...
export function renderDaemonRunCard(run) {
...
export async function fetchStatusCounts(projectPath) {
    try {
        ...
        if (project) {
            ...
            const client = createTrpcClient();
            const stats = await client.projects.stats({ projectId: project.id });
            return {
                total: stats.tasks.total,
                ready: stats.tasks.ready,
                inProgress: stats.tasks.inProgress,
 ...
 */
function renderStatusCounts(store, projectId) {
    const outcomeCounts = store.getRecentOutcomeCounts(projectId);
    if (outcomeCounts.failed > 0)
        console.log(`  Failed:      ${chalk.red(outcomeCounts.failed)} ${chalk.dim("(last 24h)")}`);
    if (outcomeCounts.stuck > 0)
        console.log(`  Stuck:       ${chalk.red(outcomeCounts.stuck)} ${chalk.dim("(last 24h)")}`);
 ...
 * Pure read operation - accepts narrow StatusReadStore interface.
 */
async function renderActiveAgents(store, projectId) {
...
async function renderStatus(projectPath) {
    ...
    if (daemonSnapshot) {
        if (daemonSnapshot.failed > 0)
            console.log(`  Failed:      ${chalk.red(daemonSnapshot.failed)} ${chalk.dim("(last 24h)")}`);
        if (daemonSnapshot.stuck > 0)
            console.log(`  Stuck:       ${chalk.red(daemonSnapshot.stuck)} ${chalk.dim("(last 24h)")}`);
...
function createStatusDetachController(message) {
    let detached = false;
    const listeners = [];
...
function sleepOrDetach(ms, detach) {
    return Promise.race([
...
export const statusCommand = new Command("status")
    ...
    if (opts.all) {
        ...
        const aggregated = { total: 0, ready: 0, inProgress: 0, completed: 0, blocked: 0 };
        let totalFailed = 0;
        let totalStuck = 0;
        ...
        for (const proj of projects) {
            try {
                const client = createTrpcClient();
                const stats = await client.projects.stats({ projectId: proj.id });
                const needsHuman = await client.projects.listNeedsHuman({ projectId: proj.id });
                const activeRuns = await client.runs.listActive({ projectId: proj.id });
        ...
        if (totalFailed > 0)
            console.log(`  Failed (24h): ${chalk.red(totalFailed)}`);
        if (totalStuck > 0)
            console.log(`  Stuck (24h):  ${chalk.red(totalStuck)}`);
    ...
    if (opts.json) {
        ...
        try {
            ...
            let stuck = 0;
            let activeRuns = [];
            let successRateData = { rate: null, merged: 0, failed: 0 };
