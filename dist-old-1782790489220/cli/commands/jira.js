...
function parseIssueTypeWorkflow(options) {
    const result = {};
...
const configureCommand = new Command("configure")
    ...
    const projects = opts.project.map((key) => ({
        key: key.toUpperCase(),
...
const statusCommand = new Command("status")
    ...
    const client = createTrpcClient();
    try {
        const status = await client.jira.getStatus({});
        if (opts.json) {
...
const enableWebhookCommand = new Command("enable-webhook")
    ...
    try {
        ...
        console.log(chalk.dim("  3. Enable the webhook in Jira admin → System → Webhooks"));
    }
    catch (err) {
        const error = err;
