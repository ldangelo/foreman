...
export function detectLayoutMode(width) {
    if (width < MIN_WIDTH)
        return "too-narrow";
...
export function getPanelWidths(mode, totalWidth) {
...
// ── Panel header ─────────────────────────────────────────────────────────
function panelHeader(panel, state, width) {
    ...
    const fg = isFocused ? (t) => chalk.cyan.bold(t) : (t) => t;
...
function getOfflineIndicator(panel, state) {
    switch (panel) {
...
function renderAgentsPanel(state, width) {
    const lines = [];
...
function renderBoardPanel(state, width) {
    const lines = [];
    ...
    for (const status of statusOrder) {
        const count = state.board.counts[status];
        if (count === 0)
            continue;
        const colorFn = BOARD_STATUS_COLOR[status] ?? ((t) => chalk.white(t));
...
function renderInboxPanel(state, width) {
    const lines = [];
...
function renderEventsPanel(state, width) {
    const lines = [];
...
function formatEventSummary(entry) {
    ...
    switch (et) {
        ...
        case "worktree-rebased":
            return d?.worktreePath ? `Rebased: ${truncateMiddle(d.worktreePath, 30)}` : "Worktree rebased";
        case "worktree-rebase-failed":
            return d?.worktreePath ? `Rebase fail: ${truncateMiddle(d.worktreePath, 30)}` : "Rebase failed";
        default:
...
export function computeLayoutSections(state, totalWidth) {
    ...
    const visiblePanels = [
        { id: "agents", width: widths.agents },
        { id: "board", width: widths.board },
        { id: "inbox", width: widths.inbox },
        { id: "events", width: widths.inbox }, // events shares inbox width
    ].filter(({ id }) => {
        if (id === "agents")
            return true;
        if (id === "board")
            return !state.agentsOffline || state.board !== null;
        if (id === "inbox")
            return !state.inboxOffline && state.inbox !== null;
        if (id === "events")
            return !state.eventsOffline && state.events !== null;
        return true;
    });
    if (mode === "narrow") {
...
function renderPanelBody(panel, state, width) {
    switch (panel) {
 ...
 */
export function renderWatchLayout(state, totalWidth) {
    const mode = detectLayoutMode(totalWidth);
    if (mode === "too-narrow") {
        return [
    ...
    if (mode === "narrow") {
        ...
        const maxRows = Math.max(sections.find(s => s.panel === "agents")?.lines.length ?? 0, sections.find(s => s.panel === "board")?.lines.length ?? 0, sections.find(s => s.panel === "inbox")?.lines.length ?? 0);
        for (let r = 0; r < maxRows; r++) {
            ...
            const trim = (s, w) => s.slice(0, w).padEnd(w);
            lines.push([
                chalk.dim("│"),
...
function renderPanelHeaderText(panel, state, width) {
    const isFocused = state.focusedPanel === panel;
    const label = PANEL_LABELS[panel];
...
function renderBorderedSection(panel, contentLines, width, state) {
    const lines = [];
    const isFocused = state.focusedPanel === panel;
    const label = PANEL_LABELS[panel];
    const fg = isFocused ? chalk.cyan.bold : chalk.white;
...
//# sourceMappingURL=WatchLayout.js.map