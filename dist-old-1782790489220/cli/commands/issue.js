...
import { GhCli, GhRateLimitError, GhNotFoundError, } from "../../lib/gh-cli.js";
...
function printIssue(issue) {
    ...
    if (issue.assignees.length > 0) {
        console.log(`  ${chalk.dim("Assignees:")} ${chalk.cyan(issue.assignees.map((a) => a.login).join(", "))}`);
    }
    if (issue.labels.length > 0) {
        console.log(`  ${chalk.dim("Labels:")} ${chalk.yellow(issue.labels.map((l) => l.name).join(", "))}`);
    }
    if (issue.body) {
...
function parseRepoKey(repoKey) {
    ...
    if (parts.length < 2) {
        throw new Error(`Invalid repo key '${repoKey}'. Expected format: owner/repo (e.g. myorg/myrepo)`);
    }
    return { owner: parts[0], repo: parts[1] };
}
export const REQUIRED_FOREMAN_GITHUB_LABELS = [
    {
        name: "foreman",
        color: "0052cc",
...
export async function ensureRequiredGithubLabels(gh, owner, repo) {
    return gh.ensureLabels(owner, repo, REQUIRED_FOREMAN_GITHUB_LABELS);
}
export function buildGithubRepoConfigInput(projectId, owner, repo, opts, existing) {
    const autoImport = opts.disableAutoImport
        ? false
        : opts.autoImport === true
            ? true
            : existing?.auto_import ?? false;
    const syncStrategy = opts.syncStrategy
...
export function mergeGithubRepoConfigInput(projectId, owner, repo, existing, overrides) {
    return {
        id: existing?.id,
        projectId,
        owner,
        ...
        webhookSecret: overrides.webhookSecret !== undefined
            ? overrides.webhookSecret
            : existing?.webhook_secret ?? null,
        webhookEnabled: overrides.webhookEnabled !== undefined
        ...
        lastSyncAt: overrides.lastSyncAt !== undefined
            ? overrides.lastSyncAt
            : existing?.last_sync_at ?? null,
    };
}
function handleGhError(err, action) {
    ...
    if (err instanceof GhRateLimitError) {
        console.error(chalk.red(`Error: GitHub API rate limit exceeded. Retry after ${err.retryAfter} seconds.`));
        process.exit(1);
    }
...
// foreman issue list
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("list")
    ...
    try {
        ...
        console.log(`  ${chalk.dim("Number".padEnd(COL_NUMBER))} ${chalk.dim("State".padEnd(COL_STATE))} Title`);
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("configure")
    ...
    try {
        ...
        console.log(`  ${chalk.dim("Default labels:")} ${chalk.yellow(row.default_labels.join(", ") || "(none)")}`);
        if (labelResult) {
            console.log(`  ${chalk.dim("Labels created:")} ${chalk.cyan(String(labelResult.created.length))}`);
            console.log(`  ${chalk.dim("Labels updated:")} ${chalk.cyan(String(labelResult.updated.length))}`);
            console.log(`  ${chalk.dim("Labels unchanged:")} ${chalk.cyan(String(labelResult.unchanged.length))}`);
        }
        console.log();
    }
    catch (err) {
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("import")
    ...
    try {
        ...
        if (opts.issue) {
            ...
            const filterCount = [opts.label, opts.milestone, opts.assignee].filter(Boolean)
                .length;
            if (filterCount === 0) {
                console.error(chalk.red("Error: Specify --issue for single import, or --label/--milestone/--assignee for bulk import."));
                process.exit(1);
            }
            const issues = await gh.listIssues(owner, repo, {
                labels: opts.label,
...
async function importIssueAsTask(adapter, gh, projectId, issue, owner, repo, opts) {
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("labels")
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("milestones")
    ...
    try {
        ...
        for (const ms of filtered.sort((a, b) => a.number - b.number)) {
            ...
            console.log(`  ${badge} ${chalk.cyan(String(ms.number).padStart(3))} ${chalk.bold(ms.title)} ${chalk.dim(`(${ms.open_issues} open, ${ms.closed_issues} closed)`)}`);
        }
        console.log();
    }
    catch (err) {
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("webhook")
    ...
    if (!opts.enable && !opts.disable) {
        console.error(chalk.red("Error: specify --enable or --disable"));
        process.exit(1);
    }
    const { projectId } = await resolveProject(opts);
    const { owner, repo } = parseRepoKey(opts.repo);
    const gh = new GhCli();
    const adapter = new PostgresAdapter();
    const webhookUrl = opts.url ?? `http://localhost:3847/webhook`;
    try {
        if (opts.disable) {
            ...
            if (repoConfig) {
                await adapter.upsertGithubRepo(mergeGithubRepoConfigInput(projectId, owner, repo, repoConfig, {
                    webhookSecret: null,
        ...
        if (opts.enable) {
            ...
            if (!secret) {
                const { generateWebhookSecret } = await import("../../daemon/webhook-handler.js");
                const newSecret = generateWebhookSecret();
                const webhook = await gh.createWebhook(owner, repo, webhookUrl, newSecret);
                ...
                console.log(`  ${chalk.dim("Webhook URL:")} ${chalk.cyan(webhookUrl)}`);
                console.log(`  ${chalk.dim("Webhook ID:")} ${chalk.cyan(String(webhook.id))}`);
                console.log(chalk.yellow(`\n  ⚠️  Save this secret — it will not be shown again:\n\n    ${newSecret}\n`));
                console.log(chalk.dim("  Configure your GitHub repository webhook URL to:\n" +
                    `    ${webhookUrl}\n\n` +
                    "  And paste the secret above when prompted by GitHub.\n"));
                return;
            }
            // Re-use existing secret
            const webhook = await gh.createWebhook(owner, repo, webhookUrl, secret);
            await adapter.upsertGithubRepo(mergeGithubRepoConfigInput(projectId, owner, repo, repoConfig, {
        ...
        if (err instanceof GhNotFoundError) {
            console.error(chalk.red(`Error: Repository ${owner}/${repo} not found or no admin access`));
        }
        else {
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("status")
    ...
    try {
        const repoConfig = await adapter.getGithubRepo(projectId, owner, repo);
        if (!repoConfig) {
            console.log(chalk.yellow(`  ${chalk.bold(owner)}/${chalk.bold(repo)} is not configured for Foreman.\n  Run: foreman issue configure --repo ${owner}/${repo}`));
            return;
        }
        ...
        console.log(`  ${chalk.dim("Last sync:")} ${repoConfig.last_sync_at ? chalk.cyan(repoConfig.last_sync_at) : chalk.dim("never")}`);
        if (syncEvents.length > 0) {
            console.log(chalk.bold(`\n  Recent sync events:\n`));
            for (const event of syncEvents) {
                const dir = event.direction === "from_github" ? chalk.blue("←") : chalk.green("→");
                console.log(`  ${dir} ${chalk.cyan(event.event_type.padEnd(20))} ${chalk.dim(event.processed_at)}`);
            }
        }
        console.log();
    }
...
// ---------------------------------------------------------------------------
issueCommand.addCommand(new Command("link")
