...
async function resolveDaemonDebugContext(projectPath) {
    try {
...
async function resolveElixirDebugContext(projectPath) {
    try {
        const projects = await listRegisteredProjects();
        const project = projects.find((record) => record.path === projectPath);
        if (!project)
...
async function resolveDaemonRuns(context, beadId) {
    const rows = await context.client.runs.list({ projectId: context.projectId, beadId, limit: 50 });
...
async function resolveElixirRuns(context, beadId) {
...
// ── Diagnostic prompt ───────────────────────────────────────────────────────
function buildDiagnosticPrompt(seedId, runSummary, messages, reports, logContent) {
