...
function wrapLocalMergeQueue(queue) {
    return {
 ...
 */
export async function cleanOrphanWorktrees(store, vcs, projectPath, worktreesDir, projectId, opts) {
    const log = opts?.logger ?? ((msg) => console.log(msg));
    ...
    const activeRunGroups = await Promise.all(activeStatuses.map((s) => store.getRunsByStatus(s, projectId)));
    const activeRuns = activeRunGroups.flat();
    const activeWorktreePaths = new Set(activeRuns.map((r) => (r.worktree_path ? resolve(r.worktree_path) : null)).filter(Boolean));
...
export async function closeForemanPullRequest(projectPath, branchName, opts) {
    const execFn = opts?.execFileAsync ?? (await getDefaultExecFileAsync());
    let prStateRaw = "";
    try {
        const { stdout } = await execFn("gh", ["pr", "view", branchName, "--json", "state,headRefName,url", "--jq", "."], { cwd: projectPath });
 ...
 */
export async function countCommitsAhead(projectPath, targetBranch, branchName, execFn) {
    const fn = execFn ?? (await getDefaultExecFileAsync());
    try {
        const { stdout } = await fn("git", ["rev-list", "--count", `${targetBranch}..${branchName}`], { cwd: projectPath });
 ...
 */
export async function isBranchMergedIntoTarget(projectPath, targetBranch, branchName, execFn) {
    const fn = execFn ?? (await getDefaultExecFileAsync());
    try {
        await fn("git", ["merge-base", "--is-ancestor", branchName, targetBranch], { cwd: projectPath });
        return true;
    }
    catch {
        return false;
 ...
 */
export async function detectAndHandleStaleBranches(store, seeds, mergeQueue, projectPath, projectId, skipSeedIds, opts) {
    ...
    for (const run of completedRuns) {
        if (skipSeedIds.has(run.seed_id))
            continue;
    ...
    try {
        ...
        targetBranch = await resolveDefaultBranch(projectPath, (path) => vcs.detectDefaultBranch(path), loadProjectConfig(projectPath));
    }
    catch {
        targetBranch = "dev";
    }
    ...
    const activeMqSeedIds = new Set(mqEntries
            ...
            if (run.started_at) {
                ...
                if (elapsedMinutes > stuckTimeout) {
                    if (!dryRun) {
                        ...
                        await store.logEvent(run.project_id, "stuck", { seedId: run.seed_id, elapsedMinutes: Math.round(elapsedMinutes), detectedBy: "timeout" }, run.id);
                    }
 ...
 */
async function syncElixirResetToReady(seedId) {
...
export const resetCommand = new Command("reset")
    ...
    const beadFilter = (opts.task ?? opts.bead);
    ...
    try {
        ...
        const seeds = taskClient;
        const store = ForemanStore.forProject(projectPath);
        const helperStore = registered ? PostgresStore.forProject(registered.id) : wrapLocalRunStore(store);
        ...
        if (beadFilter) {
            ...
            const statuses = all
                ? ["pending", "running", "failed", "stuck", "conflict", "test-failed"]
        ...
        for (const seedId of seedIds) {
            ...
            switch (result.action) {
                case "skipped-closed":
                    ...
                    break;
                case "already-open":
                    console.log(`  ${chalk.dim("skip")} bead ${chalk.cyan(seedId)} is already ${result.targetStatus ?? "retryable"}`);
                    break;
                case "reset":
                    console.log(`  ${chalk.yellow("reset")} bead ${chalk.cyan(seedId)} → ${result.targetStatus ?? "retryable"}`);
                    seedsReset++;
                    // TRD-043: unlink from GitHub PR if this task has an associated issue
                    await unlinkGitHubIssueIfNeeded(seedId, store);
                    break;
                case "not-found":
        ...
        if (!dryRun && !preserveWorktree) {
            ...
            if (existsSync(worktreesDir)) {
                ...
                const sweep = await cleanOrphanWorktrees(helperStore, vcs, projectPath, worktreesDir, runtimeProjectId);
        ...
        if (!dryRun) {
            const staleEntries = (await mergeQueue.list()).filter((e) => e.status === "conflict" || e.status === "failed");
        ...
        let mismatchResult = { mismatches: [], fixed: 0, errors: [] };
        let staleResult = { results: [], closed: 0, reset: 0, errors: [] };
        if (backendType !== "native") {
            ...
            const detectedStale = await detectAndHandleStaleBranches(helperStore, seeds, mergeQueue, projectPath, runtimeProjectId, seedIds, // skip seeds already handled by the main reset loop
            { dryRun });
            ...
            for (const r of staleResult.results) {
                ...
                if (r.action === "error") {
                    ...
                    console.log(`  ${dryRun ? chalk.yellow("(would close)") : chalk.green("close")} ` +
                        `bead ${chalk.cyan(r.seedId)} — ${r.reason}`);
                }
                else {
                    console.log(`  ${dryRun ? chalk.yellow("(would reset)") : chalk.yellow("reset")} ` +
...
export function getWorkerPid(run) {
