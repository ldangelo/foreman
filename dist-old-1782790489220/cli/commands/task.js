 ...
 */
async function fetchRunActivity(projectPath, runId, projectId) {
    if (!runId)
        return null;
 ...
 */
function renderRunStatusLine(activity) {
    const parts = [];
...
function normalizeTaskIdPrefix(raw) {
...
function allocateTaskId(projectKey, existingIds) {
...
async function resolveTaskProjectRegistration(opts) {
    ...
    if (!record) {
        throw new Error(`Project at '${projectPath}' is not registered with the daemon. Run 'foreman project list' to see registered projects.`);
    }
    return {
        projectId: record.id,
...
async function resolveTaskProjectContext(opts) {
...
async function listAllTasks(client, projectId) {
    return await client.tasks.list({ projectId, limit: 1000 });
}
async function listAllElixirTasks(client, projectId) {
    const tasks = await client.listTasks();
    return tasks
        .filter((task) => (task.project_id ?? projectId) === projectId)
        .map(elixirTaskToTaskRow);
}
async function getElixirTaskRow(client, taskId) {
    const task = await client.getTask(taskId);
    return task ? elixirTaskToTaskRow(task) : null;
}
function elixirRunToActivity(run) {
...
async function sendElixirTaskCommand(client, commandType, payload) {
...
function resolveTaskId(rows, taskIdOrPrefix) {
...
function isBackwardStatusTransition(fromStatus, toStatus) {
...
function renderColumn(value, width, style = (text) => text) {
    return style(pad(value, width));
}
function colorPriority(text, priority) {
    switch (priority) {
        case 0:
 ...
 */
function renderPrBadge(prState) {
    if (!prState)
        return chalk.dim("—");
...
function printTaskTable(rows, prStates) {
    if (rows.length === 0) {
    ...
    console.log(chalk.bold(pad("ID", idWidth)) +
        COL_GAP +
    ...
    for (const t of rows) {
        ...
        console.log(chalk.dim(t.id.padEnd(idWidth)) +
            COL_GAP +
 ...
 */
function renderRunStatusBadge(activity) {
    if (!activity)
        return chalk.dim("—");
    if (activity.isStuck) {
    ...
    if (activity.status === "running" && activity.currentPhase) {
        const phaseColors = {
            explorer: (s) => chalk.cyan(s),
            developer: (s) => chalk.green(s),
            qa: (s) => chalk.yellow(s),
            reviewer: (s) => chalk.magenta(s),
            finalize: (s) => chalk.blue(s),
        };
        const colorFn = phaseColors[activity.currentPhase] ?? ((s) => s);
 ...
 */
function printTaskTableWithRunStatus(rows, runActivityMap, prStates) {
    if (rows.length === 0) {
        console.log(chalk.dim("No tasks found."));
        return;
    }
...
const IMPORTABLE_BEAD_STATUS_TO_TASK_STATUS = {
    open: "backlog",
    in_progress: "ready",
    closed: "merged",
};
function resolveBeadsImportPath(projectPath) {
    const candidates = [
    ...
    throw new Error(`No beads export found. Expected one of: ${candidates.join(", ")}`);
}
function parseBeadsJsonl(jsonlPath) {
    ...
    return lines.map((line, index) => {
        try {
            ...
            throw new Error(`Invalid JSON in ${jsonlPath} at line ${index + 1}: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
function normalizeImportedBeadPriority(bead) {
    if (typeof bead.priority === "number") {
        if (Number.isInteger(bead.priority) && bead.priority >= 0 && bead.priority <= 4) {
            return bead.priority;
        }
        throw new Error(`Bead '${bead.id}' has unsupported numeric priority '${bead.priority}'. Expected 0-4.`);
    }
...
function normalizeImportedBeadType(bead) {
...
function mapImportedBeadStatus(status) {
    if (!status)
        return null;
    return IMPORTABLE_BEAD_STATUS_TO_TASK_STATUS[status] ?? null;
}
function summarizeImportPreview(records) {
    if (records.length === 0) {
    ...
    for (const record of records.slice(0, 5)) {
        console.log(`  ${chalk.dim(record.bead.id)} → ${formatTaskIdDisplay(record.nativeId)} ` +
...
export async function performBeadsImport(projectPath, opts = {}) {
    ...
    if (!opts.dryRun) {
        ...
        for (const bead of beads) {
            ...
            if (!fromTaskId || !Array.isArray(bead.dependencies))
                continue;
...
const createCommand = new Command("create")
    ...
    .option("--type <type>", "Task type: task, bug, feature, epic, chore, docs, question (default: task)")
    .option("--priority <level>", "Priority: 0-4 or critical/high/medium/low/backlog (default: medium)")
    .option("--from-text <description>", "Create task(s) from a natural-language description (or file path) using an LLM — replaces 'foreman bead'")
    ...
    if (opts.fromText !== undefined) {
        ...
        if (opts.title !== undefined)
            incompatible.push("--title");
        if (opts.description !== undefined)
            incompatible.push("--description");
        if (opts.project !== undefined)
            incompatible.push("--project");
        if (incompatible.length > 0) {
            console.error(chalk.red(`Error: --from-text cannot be combined with ${incompatible.join(", ")} — the description text drives title/description, and the LLM path runs against a project directory (use --project-path).`));
            process.exit(1);
        }
        await createTasksFromText(opts.fromText, {
            type: opts.type,
    ...
    if (opts.parent !== undefined)
        fromTextOnly.push("--parent");
    if (opts.dryRun)
        fromTextOnly.push("--dry-run");
    if (opts.model !== undefined)
        fromTextOnly.push("--model");
    if (opts.llm === false)
        fromTextOnly.push("--no-llm");
    if (fromTextOnly.length > 0) {
        console.error(chalk.red(`Error: ${fromTextOnly.join(", ")} require(s) --from-text.`));
        process.exit(1);
    }
    if (opts.title === undefined) {
        console.error(chalk.red(`Error: --title is required (or use --from-text "<description>" for natural-language creation).`));
        process.exit(1);
    }
    ...
    try {
        ...
        console.error(chalk.red(`Error: Invalid priority '${priorityInput}'. Use 0-4 or: critical, high, medium, low, backlog`));
        process.exit(1);
    }
    if (!VALID_TASK_TYPES.includes(typeInput)) {
        console.error(chalk.red(`Error: Invalid type '${typeInput}'. Valid types: ${VALID_TASK_TYPES.join(", ")}`));
        process.exit(1);
    }
    try {
        if (foremanBackendMode() === "elixir") {
            ...
            console.log(chalk.green(`✓ Task created`) + chalk.dim(` [${createdTask.id}]`));
...
const listCommand = new Command("list")
    ...
    try {
        if (foremanBackendMode() === "elixir") {
            ...
            rows = [...rows].sort((a, b) => a.priority - b.priority || a.created_at.localeCompare(b.created_at));
        ...
        if (opts.showPr) {
            ...
            await Promise.all(rows.map(async (task) => {
                try {
                    const prState = await client.tasks.getPrState({ projectId, taskId: task.id });
...
const showCommand = new Command("show")
    ...
    try {
        if (foremanBackendMode() === "elixir") {
            ...
            const incomingDeps = rows.filter((row) => Array.isArray(row.dependencies) && (row.dependencies?.includes(taskRow.id)));
        ...
        const resolvedId = resolveTaskId(rows, id);
        const task = await client.tasks.get({ projectId, taskId: resolvedId });
        if (!task) {
            console.error(chalk.red(`Error: Task '${id}' not found.`));
            process.exit(1);
        }
    ...
    try {
        if (foremanBackendMode() === "elixir") {
            ...
            if (task?.status === "ready") {
                console.log(chalk.green(`✓ Task '${formatTaskIdDisplay(resolvedId)}' approved and ready for dispatch.`));
            }
            else {
        ...
        const rows = await listAllTasks(client, projectId);
        const resolvedId = resolveTaskId(rows, id);
        const task = await client.tasks.approve({ projectId, taskId: resolvedId });
        if (task?.status === "ready") {
            console.log(chalk.green(`✓ Task '${formatTaskIdDisplay(resolvedId)}' approved and ready for dispatch.`));
        }
        else {
...
const updateCommand = new Command("update")
    ...
    if (opts.priority !== undefined) {
        try {
            ...
            console.error(chalk.red(`Error: Invalid priority '${opts.priority}'. Use 0-4 or: critical, high, medium, low, backlog`));
            process.exit(1);
        }
    }
        ...
        if (foremanBackendMode() === "elixir") {
            ...
            if (updateOpts.status !== undefined && !updateOpts.force) {
                ...
                if (current && isBackwardStatusTransition(current.status, updateOpts.status)) {
                    console.error(chalk.red(`Error: Task '${formatTaskIdDisplay(resolvedId)}' cannot transition from '${current.status}' to '${updateOpts.status}'.`));
...
const importCommand = new Command("import")
    ...
    try {
        ...
        if (opts.dryRun) {
            ...
            console.log(chalk.green(`Would import ${result.imported} tasks (${result.duplicateSkips} skipped: already exist by external_id${result.unsupportedStatusSkips > 0 ? `, ${result.unsupportedStatusSkips} skipped: unsupported status` : ""}).`));
            return;
        }
        console.log(chalk.green(`Imported ${result.imported} tasks (${result.duplicateSkips} skipped: already exist by external_id${result.unsupportedStatusSkips > 0 ? `, ${result.unsupportedStatusSkips} skipped: unsupported status` : ""}).`));
...
const depAddCommand = new Command("add")
    ...
    .option("--type <type>", "Dependency type: blocks (default) or parent-child", "blocks")
    .option("--project <name>", "Registered project name (default: current directory)")
    .option("--project-path <absolute-path>", "Absolute project path (advanced/script usage)")
    .action(async (fromId, toId, opts) => {
    if (opts.type !== "blocks" && opts.type !== "parent-child") {
        console.error(chalk.red(`Error: Invalid type '${opts.type}'. Use 'blocks' or 'parent-child'.`));
        process.exit(1);
    }
    try {
        const { client, projectId } = await resolveTaskProjectContext(opts);
        ...
        console.log(chalk.green(`✓ Dependency added: '${formatTaskIdDisplay(resolvedFromId)}' ${verb} '${formatTaskIdDisplay(resolvedToId)}'.`));
    }
    catch (err) {
        if (err instanceof Error && err.message.includes("circular dependency")) {
            console.error(chalk.red(`Error: Adding this dependency would create a circular dependency.`));
            process.exit(1);
        }
        console.error(chalk.red(`Error: ${err instanceof Error ? err.message : String(err)}`));
        process.exit(1);
...
const depRemoveCommand = new Command("remove")
    ...
    try {
        ...
        console.log(chalk.green(`✓ Dependency removed: '${formatTaskIdDisplay(resolvedFromId)}' → '${formatTaskIdDisplay(resolvedToId)}'.`));
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err instanceof Error ? err.message : String(err)}`));
        process.exit(1);
