...
function summaryFromElixirProject(project) {
...
function defaultElixirProjectId(projectPath, name) {
...
export function ensureCliPostgresPool(projectPath) {
...
export async function resolveProjectPathFromOptions(opts) {
...
export async function resolveProjectPathFromOption(project) {
    return resolveProjectPathFromOptions({ project });
}
export async function resolveRepoRootProjectPath(opts) {
    if (opts.project || opts.projectPath) {
    ...
    try {
        const projects = await listRegisteredProjects();
        const projectsByRemoteKey = new Map();
 ...
 */
export async function isMultiProjectMode() {
    try {
 ...
 */
export async function requireProjectInMultiMode(projectFlag, allFlag) {
    if (projectFlag || allFlag)
        return;
    const multiMode = await isMultiProjectMode();
    if (!multiMode)
        return;
    console.error(chalk.red("Error: Multiple projects registered. Please specify --project <name> or use --all.\n" +
 ...
 */
export async function requireProjectOrAllInMultiMode(projectFlag, allFlag) {
    if (projectFlag || allFlag)
        return;
    await requireProjectInMultiMode(projectFlag, allFlag);
}
//# sourceMappingURL=project-task-support.js.map