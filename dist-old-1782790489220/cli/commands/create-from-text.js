/**
 * Natural-language task creation (shared implementation).
 *
 * Extracted from the legacy `foreman bead` command so that both spellings
 * delegate to the same logic:
 *   - foreman task create --from-text "<description>"   (canonical)
 *   - foreman bead "<description>"                      (hidden, deprecated)
 *
 * Parses a free-form description (or a file path) into one or more structured
 * issues using the Claude CLI (unless `--no-llm`), then creates them through
 * the beads_rust (`br`) client.
 */
 ...
 */
export async function createTasksFromText(description, opts, projectPathArg) {
    const projectPath = projectPathArg ?? resolve(".");
    ...
    if (!(await beads.isInitialized())) {
        console.error(chalk.red(`Tasks not initialized in this directory. Run 'foreman init' first.`));
        process.exitCode = 1;
        return;
    }
 ...
 */
export function splitTitleFromText(text, maxLength = 200) {
...
//# sourceMappingURL=create-from-text.js.map