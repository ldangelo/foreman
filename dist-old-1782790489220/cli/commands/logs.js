...
export function tailFileLines(path, count, maxBytes = 4 * 1024 * 1024) {
    ...
    try {
        ...
        if (start > 0)
            lines.shift(); // Drop likely partial first line from bounded tail read.
        return lines.slice(Math.max(0, lines.length - count));
    }
    finally {
        closeSync(fd);
...
export function extractPhaseEvents(errContent) {
    const events = [];
...
export function extractRecentToolEvents(logContent, limit) {
    const events = [];
...
function renderRunHeader(resolved) {
...
function renderFileStats(runId) {
    for (const suffix of ["log", "err", "out"]) {
...
function renderSummary(runId, tailCount) {
...
function normalizeDaemonRun(row) {
    return {
        ...row,
...
function normalizeElixirRun(row) {
...
async function resolveElixirRun(id, opts) {
    ...
    if (runId && /^[0-9a-f-]{8,}$/i.test(runId)) {
        ...
        if (direct)
            return { run: direct, progress: null, taskId: direct.seed_id };
    }
    if (!id)
        return null;
    ...
    if (matches.length === 1) {
        ...
        if (taskRunId) {
            ...
            if (run)
                return { run, progress: null, taskId };
        }
    }
...
async function resolveDaemonRun(id, opts) {
    const projectPath = await resolveProjectPathFromOptions(opts);
    const projects = await listRegisteredProjects();
    const project = projects.find((entry) => entry.path === projectPath || entry.name === opts.project || entry.id === opts.project);
    if (!project)
    ...
    if (runId && /^[0-9a-f-]{8,}$/i.test(runId)) {
        try {
            const directRow = await client.runs.get({ runId });
            if (directRow) {
                const run = normalizeDaemonRun(directRow);
                const progress = await client.runs.getProgress({ runId: run.id });
    ...
    if (matches.length === 1 && matches[0]?.run_id) {
        const runRow = await client.runs.get({ runId: matches[0].run_id });
        if (!runRow)
            return null;
    ...
    const runRows = await client.runs.list({ projectId: project.id, limit: 100 });
    const runs = runRows.map(normalizeDaemonRun);
    const match = runs.find((run) => run.id === id || run.id.startsWith(id) || run.seed_id === id || run.seed_id.startsWith(id));
    if (!match)
        return null;
    const progress = await client.runs.getProgress({ runId: match.id });
    return { run: match, progress: progress ?? progressFromRun(runRows.find((row) => row.id === match.id) ?? {}), taskId: match.seed_id };
}
async function resolveLocalRun(id, opts) {
    const projectPath = await resolveProjectPathFromOptions(opts);
    ...
    try {
        ...
        if (runId) {
            ...
            if (direct)
                return { run: direct, progress: store.getRunProgress(direct.id), taskId: direct.seed_id };
        }
        if (!id || !project)
            return null;
...
async function resolveRun(id, opts) {
    try {
