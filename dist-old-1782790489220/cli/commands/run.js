...
import { ensureBundledWorkflowsInstalled, findStaleWorkflows, listAvailableWorkflows, loadWorkflowConfig, } from "../../lib/workflow-loader.js";
...
function createRegisteredDispatcherOverrides(projectId, daemonStore) {
    ...
    return {
        ...
        getRecentFailureCount: async (_projectId, since) => {
            ...
            const runs = (await Promise.all(statuses.map((status) => pg.listPipelineRuns(projectId, { status, limit: 1000 })))).flat();
        ...
        nativeTaskOps: {
            ...
            getReadyTasks: async () => await pg.listDispatchableReadyTasks(projectId, 1000),
            getTaskByExternalId: async (externalId) => await pg.getTaskByExternalId(projectId, externalId),
            getTaskById: async (taskId) => await pg.getTask(projectId, taskId),
            claimTask: async (taskId, runId) => await pg.claimTask(projectId, taskId, runId),
        },
        runOps: {
 ...
 */
export async function createTaskClients(projectPath, _runtimeMode = resolveRuntimeMode(), registeredProjectId) {
 ...
 */
async function runRefineryMerge(store, projectPath, taskClient, runLookup, registeredProject) {
...
async function isAnonymousJujutsuRevision(vcs, projectPath, branch) {
    if (!branch || vcs.name !== "jujutsu" || typeof vcs.branchExists !== "function") {
        return false;
    }
    return !(await vcs.branchExists(projectPath, branch).catch(() => false));
}
export async function resolveOwnedControllerBranch(vcs, projectPath, preferredDefaultBranch) {
    const maybeVcs = vcs;
    if (typeof maybeVcs.getCurrentBranch !== "function" ||
    ...
    const defaultBranch = normalizeBranchLabel(preferredDefaultBranch ?? await vcs.detectDefaultBranch(projectPath)) ?? "";
    const currentIsAnonymousRevision = await isAnonymousJujutsuRevision(vcs, projectPath, currentBranch);
    const shouldUseOwnedBranch = vcs.name === "jujutsu" &&
    ...
    if (dirtyPaths.length > 0) {
        throw new Error(`Foreman-owned branch requires a clean controller checkout. Dirty paths: ${dirtyPaths.slice(0, 8).join(", ")}`);
    }
 ...
 */
async function createRunVcsBackend(projectPath) {
...
async function resolveRunRegisteredProject(projectPath) {
    const projects = await listRegisteredProjects();
    return projects.find((project) => project.path === projectPath) ?? null;
}
export function collectRuntimeAssetIssues(projectPath, projectCfg) {
    const issues = [];
    try {
...
export async function checkBranchMismatch(taskClient, projectPath) {
    let vcs;
    try {
    ...
    const mismatchByBranch = new Map();
    for (const bead of inProgressBeads) {
        try {
            const detail = await taskClient.show(bead.id);
    ...
    for (const [targetBranch, beadIds] of mismatchByBranch) {
        ...
        if (shouldSwitch) {
            ...
            console.log(chalk.yellow(`Skipping beads ${beadList} — they target ${targetBranch}.`) +
 ...
 */
export function validateWorkflowOverride(workflowName, projectPath) {
    try {
        ...
        return {
            ok: false,
            message: `Cannot load workflow '${workflowName}': ${reason}\n` +
...
export const runCommand = new Command("run")
    ...
    const resumeFailed = opts.resumeFailed;
    const watch = opts.watch;
    const telemetry = opts.telemetry;
    const pipeline = opts.pipeline; // --no-pipeline sets to false
    const workflowOverride = opts.workflow?.trim() || undefined;
    const beadFilter = (opts.bead ?? opts.task);
    ...
    try {
        ...
        const dispatcher = new Dispatcher(taskClient, store, projectPath, null, // BvClient removed — native task store only
            ...
            try {
                if (project) {
                    ...
                    if (sentinelConfig && sentinelConfig.enabled === 1) {
                        const sentinelTaskClient = taskClient;
                        ...
                        console.log(chalk.dim(`[sentinel] Auto-started on branch ${sentinelConfig.branch} (every ${sentinelConfig.interval_minutes}m)`));
                    }
                }
            }
            catch (sentinelErr) {
        ...
        if (!dryRun && project) {
            try {
                ...
                if (taskSyncResult.synced > 0 || taskSyncResult.mismatches.length > 0) {
                    console.log(chalk.dim(`[startup] Reconciled ${taskSyncResult.synced} task(s), ` +
        ...
        if (!dryRun) {
            try {
                const configuredDefaultBranch = await resolveDefaultBranch(projectPath, (path) => startupVcs.detectDefaultBranch(path), projectCfg);
                const controller = await resolveOwnedControllerBranch(startupVcs, projectPath, configuredDefaultBranch);
                if (controller.usedOwnedBranch) {
                    targetBranch = controller.targetBranch;
                    console.log(chalk.dim(`[startup] Using Foreman-owned branch ${FOREMAN_OWNED_BRANCH} (target: ${controller.defaultBranch})`));
                }
                const cb = targetBranch ? undefined : normalizeBranchLabel(await startupVcs.getCurrentBranch(projectPath));
                const db = targetBranch
                    ? undefined
                    : normalizeBranchLabel(await resolveDefaultBranch(projectPath, (path) => startupVcs.detectDefaultBranch(path), projectCfg));
                if (cb && db && cb !== db) {
                    ...
                    if (!confirmed) {
                        console.log(chalk.dim(`Aborted. Switch to ${db} or the desired target branch and re-run.`));
                        await stopSentinel();
                        store.close();
                        await notifyServer.stop().catch(() => { });
                        process.exit(1);
        ...
        if (!dryRun && project) {
            try {
                const runLookup = registered && daemonStore ? daemonStore : store;
        ...
        while (true) {
            ...
            if (result.dispatched.length === 0) {
                ...
                if (watch && result.activeAgents > 0) {
                    ...
                    console.log(chalk.dim(`No new tasks dispatched — waiting for ${result.activeAgents} active agent(s) to finish…`));
                ...
                if (watch) {
                    ...
                    if (PIPELINE_LIMITS.emptyPollCycles > 0 &&
                        emptyPollCount >= PIPELINE_LIMITS.emptyPollCycles) {
                        const elapsedSec = Math.round((emptyPollCount * PIPELINE_TIMEOUTS.monitorPollMs) / 1000);
                        console.log(chalk.yellow(`\nNo ready beads after ${emptyPollCount} poll cycle(s) (~${elapsedSec}s). Exiting dispatch loop.`));
                        console.log(chalk.dim("  • Re-run 'foreman run' once tasks become unblocked\n" +
                    ...
                    if (!waitingForTasksLogged) {
                        console.log(chalk.dim(`No ready beads — waiting for tasks to become available…`));
                        waitingForTasksLogged = true;
                    }
                    await new Promise((resolve) => setTimeout(resolve, PIPELINE_TIMEOUTS.monitorPollMs));
                    continue;
                }
