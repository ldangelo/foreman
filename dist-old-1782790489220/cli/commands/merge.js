 ...
 */
export async function createMergeTaskClient(projectPath, registeredProjectId) {
    const { taskClient } = await createTaskClient(projectPath, { registeredProjectId });
    return taskClient;
}
async function createMergeVcsBackend(projectPath) {
...
function wrapLocalMergeQueue(queue, store, projectPath) {
    return {
...
export const mergeCommand = new Command("merge")
    ...
    try {
        ...
        const targetBranch = opts.targetBranch
            ?? await resolveDefaultBranch(projectPath, (path) => vcs.detectDefaultBranch(path), projectCfg);
        ...
        if (opts.resolve) {
            ...
            if (run.status !== "conflict") {
                console.error(chalk.red(`Error: Run '${runId}' is not in conflict state (current status: '${run.status}'). Only runs with status 'conflict' can be resolved.`));
                store.close();
                process.exit(1);
            }
        ...
        if (opts.stats !== undefined) {
            ...
            const period = (typeof opts.stats === "string" ? opts.stats : "all");
            const stats = await costTracker.getStats(period);
            if (opts.json) {
        ...
        if (opts.dryRun) {
            ...
            const dryRunResults = await dryRunMerge(projectPath, targetBranch, branches, (opts.task ?? opts.bead));
            for (const entry of dryRunResults) {
                const conflictIcon = entry.hasConflicts
                    ? chalk.red("CONFLICT")
                    : chalk.green("OK");
                const tierStr = entry.estimatedTier !== undefined
        ...
        if (opts.list) {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                const elapsed = Math.round((Date.now() - new Date(entry.enqueued_at).getTime()) / 60000);
                const filesCount = entry.files_modified.length;
                const num = `${i + 1}`.padStart(2);
                console.log(`  ${chalk.dim(num + ".")} ${statusLabel(entry.status)} ${chalk.cyan(entry.seed_id)} ${chalk.dim(entry.branch_name)} ${chalk.dim(`(${elapsed}m ago, ${filesCount} files)`)}`);
                if (entry.error) {
        ...
        const taskFilter = (opts.task ?? opts.bead);
        if (taskFilter) {
        ...
        const testFailures = [];
        const prsCreated = [];
        const skippedIds = []; // entries skipped due to --seed filter
        ...
        if (conflicts.length > 0) {
            ...
            console.log();
            console.log(chalk.dim("  Resolve with: foreman merge --resolve <runId> --strategy theirs|abort"));
            console.log();
        }
        ...
        if (merged.length > 0 || conflicts.length > 0) {
            try {
                ...
                const rate = await costTracker.getResolutionRate(30);
                if (rate.total > 0) {
                    console.log(chalk.dim(`AI resolution rate: ${rate.successes}/${rate.total} conflicts (${rate.rate.toFixed(1)}%) over last 30 days\n`));
                }
            }
            catch {
