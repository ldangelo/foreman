...
import { priorityLabel, formatTaskIdDisplay, } from "../../lib/task-store.js";
...
export function normalizeStatusForBoard(status) {
    const normalized = status.replace(/-/g, "_");
    return BOARD_STATUSES.includes(normalized) ? normalized : null;
}
export function boardColumnForTaskStatus(status) {
    const normalized = status.replace(/-/g, "_");
...
function boardStatusToStoreStatus(status) {
 ...
 */
function storeStatusToBoardStatus(status) {
    const normalized = status.replace(/-/g, "_");
    return BOARD_STATUSES.includes(normalized) ? normalized : null;
}
/**
 ...
 */
function normalizeStatusForStore(status) {
 ...
 */
function parseTaskUpdatedAt(task) {
    const timestamp = Date.parse(task.updated_at);
    return Number.isNaN(timestamp) ? 0 : timestamp;
}
export function sortBoardTasks(tasks, sortMode) {
    const sorted = [...tasks];
 ...
 */
export function sortBoardColumns(taskMap, sortMode) {
...
async function resolveBoardContext(projectPath) {
    const projects = await listRegisteredProjects();
    const resolvedProjectPath = resolve(projectPath);
    const project = projects.find((record) => resolve(record.path) === resolvedProjectPath);
    if (!project) {
        throw new Error(`Project at '${projectPath}' is not registered in Elixir projections. Run 'foreman project register ${resolvedProjectPath}'.`);
    }
...
function boardTaskFromElixir(row) {
...
export async function loadBoardTasks(projectPath) {
    ...
    const rows = context.backend === "elixir"
        ...
        : (await context.client.tasks.list({ projectId: context.projectId, limit: 1000 }))
...
export async function loadBoardTask(projectPath, taskId) {
    const context = await resolveBoardContext(projectPath);
    if (context.backend === "elixir") {
        const row = await context.client.getTask(taskId);
        if (!row || (row.project_id && row.project_id !== context.projectId))
            return null;
        return boardTaskFromElixir(row);
    }
    const row = await context.client.tasks.get({ projectId: context.projectId, taskId });
    return row ? boardTaskFromRow(row) : null;
}
export async function pollBoardInboxTaskUpdates(projectPath, lastSeenId, limit = 100, cursorSeeded = lastSeenId !== null) {
    const context = await resolveBoardContext(projectPath);
    if (context.backend === "elixir") {
        return { taskIds: [], newestId: lastSeenId };
    }
    const { client, projectId } = context;
    const rows = await client.mail.listGlobal({ projectId, limit });
...
export function applyBoardTaskUpdate(taskMap, task, taskId, sortMode) {
    const next = new Map();
    for (const [status, tasks] of taskMap) {
...
export async function refreshBoardTasksById(projectPath, taskMap, taskIds, sortMode) {
    let next = taskMap;
...
export async function loadBoardTaskNotes(projectPath, taskId) {
    const context = await resolveBoardContext(projectPath);
    if (context.backend === "elixir") {
...
function getVisibleStatuses(terminalWidth, selectedColIndex) {
    ...
    const maxCols = Math.max(1, Math.floor((Math.max(terminalWidth, minColWidth) + COLUMN_GAP) / (minColWidth + COLUMN_GAP)));
...
function getColumnWidth(terminalWidth, columnCount) {
    if (columnCount <= 0) {
        return terminalWidth;
    }
    const availableWidth = Math.max(MIN_COL_WIDTH * columnCount + COLUMN_GAP * (columnCount - 1), terminalWidth);
    return Math.max(MIN_COL_WIDTH, Math.floor((availableWidth - COLUMN_GAP * (columnCount - 1)) / columnCount));
}
export function getVisibleTaskCapacity(columnHeight, taskCount, userLimit) {
...
export function getVisibleTaskWindow(tasks, selectedIndex, maxVisiblePerCol) {
...
function renderEmptyTaskSlot() {
    return h(Box, { flexDirection: "column", width: "100%" }, h(Text, { dimColor: true }, " "), h(Text, { dimColor: true }, " "));
}
function renderTaskCardView(task, isSelected, isFlash) {
    ...
    return h(Box, { flexDirection: "column", width: "100%" }, h(Box, { width: "100%", backgroundColor: rowBackgroundColor }, h(Text, { color: rowTextColor }, `${isSelected || isFlash ? "▶" : " "} `), h(Box, { flexGrow: 1, minWidth: 0 }, h(Text, { color: rowTextColor, wrap: "truncate-end" }, task.title)), h(Text, {
        color: badgeStyle.textColor,
        backgroundColor: badgeStyle.backgroundColor,
    }, ` ${badge} `)), h(Box, { width: "100%", backgroundColor: rowBackgroundColor }, h(Box, { width: "50%", minWidth: 0 }, h(Text, { color: metaTextColor, dimColor: !rowBackgroundColor, wrap: "truncate-end" }, formatTaskIdDisplay(task.id))), h(Box, { width: "50%", minWidth: 0 }, h(Text, { color: metaTextColor, dimColor: !rowBackgroundColor, wrap: "truncate-end" }, task.status))));
}
function renderBoardColumn(status, absoluteIndex, state, columnWidth, columnHeight, userVisibleLimit) {
    ...
    if (taskWindow.hiddenBefore > 0) {
        slots.push(h(Text, { dimColor: true, wrap: "truncate-end" }, `↑ ${taskWindow.hiddenBefore} earlier`));
    }
    for (let index = 0; index < visibleLimit; index += 1) {
        ...
        slots.push(renderTaskCardView(task, isSelectedColumn && taskWindow.startIndex + index === state.nav.rowIndex, task.id === state.flashTaskId));
    }
    ...
    return h(Box, {
        ...
        wrap: "truncate-end",
    }, `${STATUS_LABELS[status]} (${tasks.length})`), ...slots);
}
function renderHelpOverlayView(width) {
    ...
    return h(Box, { borderStyle: "round", borderColor: "yellow", flexDirection: "column", width: panelWidth }, h(Text, { color: "yellow", bold: true }, "HELP — Key Bindings"), ...rows.map(([keyLabel, description]) => h(Box, { key: `${keyLabel}:${description}`, width: "100%" }, h(Box, { width: keyWidth, minWidth: 0 }, h(Text, { color: "cyan", wrap: "truncate-end" }, keyLabel)), h(Box, { flexGrow: 1, minWidth: 0 }, h(Text, { wrap: "truncate-end" }, description)))), h(Text, { dimColor: true }, "Press ? or Esc to close"));
}
function renderTaskDetailView(task, width, notesStatus, notesError, terminalHeight) {
    ...
    if (task.description) {
        ...
        children.push(h(Box, { key: "desc:first", width: "100%" }, h(Box, { width: fieldWidth, minWidth: 0 }, h(Text, { bold: true, wrap: "wrap" }, "Description:")), h(Box, { flexGrow: 1, minWidth: 0 }, h(Text, { wrap: "wrap" }, firstLine ?? ""))));
        // Show all description lines with wrapping (no line limit)
        for (const [index, line] of rest.entries()) {
            children.push(h(Box, { key: `desc:${index}`, width: "100%" }, h(Box, { width: fieldWidth }, h(Text, null, " ")), h(Box, { flexGrow: 1, minWidth: 0 }, h(Text, { dimColor: true, wrap: "wrap" }, line))));
        }
    }
    for (const [label, value] of rows) {
        ...
        children.push(h(Box, { key: label, width: "100%" }, h(Box, { width: fieldWidth, minWidth: 0 }, h(Text, { bold: true, wrap: "wrap" }, label)), h(Box, { flexGrow: 1, minWidth: 0 }, h(Text, { wrap: "wrap" }, value))));
    }
    if (notesStatus === "loading") {
        children.push(h(Text, { key: "notes-loading", dimColor: true }, "Notes: loading…"));
    }
    else if (notesStatus === "error") {
        children.push(h(Text, { key: "notes-error", dimColor: true }, `Notes: unavailable${notesError ? ` (${notesError})` : ""}`));
    }
    else if (task.notes) {
        if (task.notes.length === 0) {
            ...
            for (const [noteIndex, note] of task.notes.entries()) {
                ...
                children.push(h(Text, { key: `note:${note.id}:meta`, dimColor: true, wrap: "wrap" }, `[${when} ${phase}${note.kind}] ${note.author}`));
                // Show all lines of note body with wrapping
                for (const [lineIndex, line] of note.body.split("\n").entries()) {
                    children.push(h(Text, { key: `note:${note.id}:body:${lineIndex}`, wrap: "wrap" }, line));
                }
            }
        }
    }
    ...
    return h(Box, { borderStyle: "round", borderColor: "blue", flexDirection: "column", width: panelWidth, height: panelHeight }, ...visibleChildren);
}
const REFRESH_SPINNER_FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
function renderBoardStatusText(state, totalTasks) {
...
function renderBoardFrame(state, projectName, terminalWidth, terminalHeight, userVisibleLimit) {
    ...
    const boardContent = [
        h(Box, { width: "100%", marginBottom: 1 }, h(Text, { color: "blue", bold: true, wrap: "truncate-end" }, `Foreman Kanban Board — ${projectName}`), h(Spacer, null), h(Text, { dimColor: state.refreshStatus !== "refreshing", color: state.refreshStatus === "refreshing" ? "cyan" : undefined }, renderBoardStatusText(state, totalTasks))),
        h(Box, { flexDirection: "row", gap: COLUMN_GAP, marginBottom: 1, width: "100%" }, ...visibleStatuses.map((status) => {
            const absoluteIndex = BOARD_STATUSES.indexOf(status);
            return h(Box, { key: `jump:${status}`, width: columnWidth, minWidth: columnWidth }, h(Text, {
        ...
        h(Box, { flexDirection: "row", gap: COLUMN_GAP, alignItems: "flex-start", width: "100%" }, ...visibleStatuses.map((status) => {
            const absoluteIndex = BOARD_STATUSES.indexOf(status);
            return h(Box, { key: `column:${status}`, width: columnWidth, minWidth: columnWidth, height: columnHeight }, renderBoardColumn(status, absoluteIndex, state, columnWidth, columnHeight, userVisibleLimit));
        })),
    ...
    if (state.errorMessage) {
        boardContent.push(h(Box, { marginTop: 1 }, h(Text, { color: "red", bold: true }, "ERROR "), h(Text, { color: "red", wrap: "truncate-end" }, state.errorMessage)));
    }
    // When help is shown, render help overlay at bottom (existing behavior)
    if (state.showHelp) {
        const tree = h(Box, { flexDirection: "column", width: terminalWidth }, ...boardContent, h(Box, { marginTop: 1 }, renderHelpOverlayView(Math.max(24, terminalWidth - 2))));
        return renderToString(tree, { columns: terminalWidth });
    }
    const tree = h(Box, { flexDirection: "column", width: terminalWidth }, ...boardContent);
    return renderToString(tree, { columns: terminalWidth });
}
/**
 ...
 */
export function renderBoard(state, projectName, terminalWidth, userVisibleLimit, terminalHeight = getTerminalHeight()) {
    ...
    if (state.showDetail && state.detailTask) {
        return `${CLEAR_SCREEN}${renderTaskDetail(state.detailTask, terminalWidth, state.detailNotesStatus, state.detailNotesError, terminalHeight)}`;
    }
 ...
 */
export function renderTaskDetail(task, width, notesStatus = "idle", notesError = null, terminalHeight) {
 ...
 */
export function editTaskInEditor(task, fullSchema, onError) {
 ...
 */
export function applyStatusChange(projectPath, taskId, newStatus) {
    throw new Error("applyStatusChange is now async; use applyStatusChangeAsync().");
}
async function sendElixirBoardCommand(client, commandType, payload) {
...
export async function applyStatusChangeAsync(projectPath, taskId, newStatus) {
    try {
        const context = await resolveBoardContext(projectPath);
        if (context.backend === "elixir") {
 ...
 */
export function closeTask(projectPath, taskId, reason) {
    throw new Error("closeTask is now async; use closeTaskAsync().");
}
export async function closeTaskAsync(projectPath, taskId, _reason) {
    try {
        const context = await resolveBoardContext(projectPath);
        if (context.backend === "elixir") {
 ...
 */
export function saveEditedTask(projectPath, originalId, updated) {
    throw new Error("saveEditedTask is now async; use saveEditedTaskAsync().");
}
export async function saveEditedTaskAsync(projectPath, originalId, updated) {
    try {
        const context = await resolveBoardContext(projectPath);
        const updates = {
            title: updated.title,
 ...
 */
export function normalizeNavRowIndex(nav, tasks) {
 ...
 */
export function getHighlightedTask(nav, tasks) {
    const currentTasks = tasks.get(BOARD_STATUSES[nav.colIndex]) ?? [];
 ...
 * On error or non-zero exit, returns null and sets errorMessage.
 */
export function createTaskInEditor(onError) {
    const editor = resolveEditor();
 ...
 */
export async function createTaskAsync(projectPath, taskData) {
    try {
        const context = await resolveBoardContext(projectPath);
        ...
        const created = await context.client.tasks.create(createInput);
 ...
 */
export function createKeyHandler(projectPath, callbacks = {}) {
    return async function handleKey(key, state) {
        const result = {
            nav: { ...state.nav },
            errorMessage: null,
            flashTaskId: null,
 ...
 */
export async function runBoard(opts) {
    ...
    let sortMode = "updated"; // Default: sort by updated_at (most recent first)
    let showHelp = false;
    let showDetail = false;
    let detailTask = null;
    let detailNotesStatus = "idle";
    let detailNotesError = null;
    let detailNotesTaskId = null;
    ...
    let inboxMonitorTimer = null;
    let boardInboxLastSeenId = null;
    ...
    const processInboxTaskUpdates = async () => {
        ...
        try {
            ...
            const refreshedTasks = await Promise.all(update.taskIds.map(async (taskId) => ({ taskId, task: await loadBoardTask(projectPath, taskId) })));
            let nextTasks = tasks;
    ...
    const onData = async (chunk) => {
        ...
        if (key === "\x1B[A")
            normalizedKey = KEY_k; // arrow up
        else if (key === "\x1B[B")
            normalizedKey = KEY_j; // arrow down
        else if (key === "\x1B[C")
            normalizedKey = KEY_l; // arrow right
        else if (key === "\x1B[D")
            normalizedKey = KEY_h; // arrow left
        const currentState = {
            tasks,
            nav,
