 ...
 */
export function resolveDefaultBrOnly(opts) {
    if (!opts.sdOnly && !opts.brOnly) {
        opts.brOnly = true;
    }
}
export function applySdOnlyDeprecation(opts) {
    if (!opts.sdOnly)
        return false;
    process.stderr.write(chalk.yellow("SLING-DEPRECATED: --sd-only is deprecated and will be removed in a future release. " +
        "Foreman now writes sling output to the native task store only. Legacy backend flags are ignored.\n"));
    opts.sdOnly = false;
    opts.brOnly = false;
    return true;
}
export function getSlingLegacyBackendFlagNotice() {
    return "SLING-DEPRECATED: legacy backend-targeting flags are ignored; sling writes native tasks only.";
}
async function resolveSlingProjectPath(opts) {
...
function createDaemonSlingWriter(client, projectId) {
    return {
        ...
        async update(id, opts) {
            ...
            return await client.tasks.get({ projectId, taskId: id });
        },
...
async function createElixirSlingWriter(projectId) {
    const manager = new ElixirServerManager();
    const status = await manager.ensureRunning();
    const client = new ElixirServerClient(status.url, process.env.FOREMAN_SERVER_AUTH_TOKEN);
    const listProjectTasks = async () => (await client.listTasks()).filter((task) => task.project_id === projectId);
    return {
        async getByExternalId(externalId) {
        ...
        async addDependency(fromId, toId, _type = "blocks") {
            const response = await client.sendCommand({
...
export function parsePrdReadinessScore(content) {
    const normalized = content.replaceAll("*", "");
    const match = normalized.match(/readiness(?:\s+score|_score)?\s*[:|]\s*([0-5](?:\.\d+)?)/i);
    if (!match?.[1]) {
        return null;
    }
...
function findLatestGeneratedTrd(trdDir) {
...
async function handleTrdImport(projectPath, resolvedTrdPath, opts, jsonExtras = {}) {
    ...
    if (!opts.auto) {
        const answer = await new Promise((resolveAnswer) => {
            ...
            rl.question(chalk.bold("Create native tasks in the project task store? [y/N] "), (ans) => {
                rl.close();
                resolveAnswer(ans);
            });
        });
    ...
    const result = await execute(plan, parallel, slingOptions, taskWriter, spinner.update);
    spinner.finish();
    printSummary(result);
}
// ── Preview display ──────────────────────────────────────────────────────
function printSlingPlan(plan, parallel) {
    const totalTasks = plan.sprints.reduce((sum, s) => sum + s.stories.reduce((ss, st) => ss + st.tasks.length, 0), 0);
    const totalHours = plan.sprints.reduce((sum, s) => sum +
        s.stories.reduce((ss, st) => ss + st.tasks.reduce((ts, t) => ts + t.estimateHours, 0), 0), 0);
    console.log(chalk.bold(`\nEpic: ${plan.epic.title} (${totalTasks} tasks, ${plan.sprints.length} sprints, ~${totalHours}h)\n`));
    const sprintToGroup = new Map();
    ...
    for (let si = 0; si < plan.sprints.length; si++) {
        ...
        const sprintHours = sprint.stories.reduce((sum, st) => sum + st.tasks.reduce((ts, t) => ts + t.estimateHours, 0), 0);
        ...
        console.log(`${prefix}${chalk.bold(sprint.title)} (${sprintHours}h, ${sprintTasks} tasks)` +
            ` ${priorityTag}${groupTag}`);
        for (const story of sprint.stories) {
            const storyCompleted = story.tasks.filter((t) => t.status === "completed").length;
            const storyTag = storyCompleted === story.tasks.length
            ...
            for (const task of story.tasks) {
                ...
                console.log(`${prefix}    ${statusIcon} ${chalk.dim(task.trdId)}  ${task.title}${est}${deps}${risk}`);
            }
        }
        console.log();
    }
...
function createProgressSpinner() {
    ...
    return {
        update(processed, total, _tracker) {
...
const prdSubcommand = new Command("prd")
    ...
    if (readinessScore !== null && readinessScore < MIN_PRD_READINESS_SCORE) {
        console.error(chalk.red(`SLING-009: PRD readiness score ${readinessScore.toFixed(1)} is below the minimum required score of ${MIN_PRD_READINESS_SCORE.toFixed(1)}.`));
        process.exitCode = 1;
        return;
    }
