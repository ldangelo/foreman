...
function printSection(title, results, jsonOutput) {
    if (!jsonOutput) {
...
// ── Command ──────────────────────────────────────────────────────────────
async function resolveDoctorProjectPath() {
...
export const doctorCommand = new Command("doctor")
    ...
    .option("--log-days <n>", "Retention window for --clean-logs in days (default: 7)", parseNonNegativeIntOption("--log-days"))
    .action(async (opts) => {
    const fix = opts.fix ?? false;
    const dryRun = opts.dryRun ?? false;
    const jsonOutput = opts.json ?? false;
    const cleanLogs = opts.cleanLogs ?? false;
    const logDays = opts.logDays ?? 7;
    if (!jsonOutput) {
    ...
    try {
        ...
        if (jsonOutput) {
            ...
            const parts = [];
            if (pass > 0)
                parts.push(chalk.green(`${pass} passed`));
            if (fixed > 0)
                parts.push(chalk.cyan(`${fixed} fixed`));
            if (warn > 0)
                parts.push(chalk.yellow(`${warn} warning(s)`));
            if (fail > 0)
                parts.push(chalk.red(`${fail} failed`));
