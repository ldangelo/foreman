...
export function buildInitWizardConfig(answers) {
...
async function promptJiraAuth(rl) {
...
async function promptGitHubAuth(rl) {
...
async function runInitWizard(projectDir) {
    const rl = createInterface({ input, output });
    try {
        const vcsBackend = await promptChoice(rl, "VCS backend", ["git", "jujutsu", "auto"], "auto");
        // Get all available workflow templates from BUNDLED_WORKFLOW_NAMES
        const workflowChoices = [...BUNDLED_WORKFLOW_NAMES];
        const workflowTemplate = await promptChoice(rl, "Workflow template", workflowChoices, "default");
        const issueTracker = await promptChoice(rl, "Issue tracker", ["beads", "jira", "github"], "beads");
        let jira;
        if (issueTracker === "jira") {
            ...
            try {
                ...
                const msg = err instanceof Error && err.message.includes("FOREMAN_MASTER_KEY")
                    ? err.message
...
export async function maybeRegisterInitializedProjectInElixir(projectDir, projectName) {
    if (foremanBackendMode() !== "elixir")
        return;
    await registerProjectInElixir(projectDir, { name: projectName, status: "active" });
}
export function formatInitDatabaseError(err, projectDir) {
...
export const initCommand = new Command("init")
    ...
    try {
        ...
        await initProjectStore(projectDir, projectName, {
            getProjectByPath: async (path) => (path === projectDir ? { id: project.id } : null),
