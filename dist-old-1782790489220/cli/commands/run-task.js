 ...
 */
export function skipFlagsDeprecationWarning(opts, context = "run") {
    const flags = [];
    if (opts.skipExplore)
        flags.push("--skip-explore");
    if (opts.skipReview)
        flags.push("--skip-review");
    if (flags.length === 0)
        return null;
...
async function syncElixirTaskToPostgres(taskId, registeredProjectId, projectPath) {
    try {
        const manager = new ElixirServerManager();
        if (!(await manager.health()).ok)
            return null;
...
function elixirTaskToPostgresTask(task) {
...
function normalizeElixirStatusForPostgres(status) {
    if (status === "open")
        return "backlog";
...
function postgresRowToIssue(projectPath, row) {
    return {
        id: row.id,
        title: row.title,
        status: row.status,
 ...
 */
async function resolveRegisteredProject(projectPath) {
 ...
 */
async function checkWorktreeLock(store, taskId, projectId) {
 ...
 */
export async function runTaskAction(taskId, workflowPath, opts) {
    const { model, dryRun = false, watch = true, targetBranch, runId: requestedRunId, project, projectPath: optsProjectPath, } = opts;
    ...
    let task;
    try {
        task = await taskClient.show(taskId);
    }
    catch {
    ...
    let taskLabels;
    try {
        const detail = await taskClient.show(taskId);
    ...
    const seedInfo = issueToSeedInfo(task);
