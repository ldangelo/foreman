...
function adaptDaemonRun(row, projectPath) {
    const statusMap = {
        pending: "pending",
        running: "running",
        success: "completed",
...
async function resolveDaemonAttachContext(projectPath) {
    try {
...
async function resolveDaemonRun(context, id) {
    const runs = await context.client.runs.list({ projectId: context.projectId, limit: 100 });
 ...
 */
export async function attachAction(id, opts, store, projectPath, daemon) {
...
export async function listSessionsEnhancedDaemon(context) {
    const rows = await context.client.runs.list({ projectId: context.projectId, limit: 100 });
...
async function handleStreamDaemon(run, context, signal, pollIntervalMs = 1000) {
    ...
    console.log();
    const seenIds = new Set();
    const existing = await context.client.mail.list({ projectId: context.projectId, runId: run.id });
    for (const msg of existing.map(adaptDaemonMessage)) {
        seenIds.add(msg.id);
        printMessage(msg);
    }
    const currentRun = await context.client.runs.get({ runId: run.id });
    if (currentRun) {
    ...
    return new Promise((resolve) => {
        ...
        const poll = () => {
            void (async () => {
                const messages = await context.client.mail.list({ projectId: context.projectId, runId: run.id });
                ...
                const latestRun = await context.client.runs.get({ runId: run.id });
                if (latestRun) {
...
async function handleKillDaemon(run, context, store, fallbackRun) {
    const pid = getWorkerPid(run);
    if (!pid) {
        console.log("No pid found for this run.");
        return 0;
