...
async function resolveDashboardProjectRecord(projects, projectPath, projectSelector) {
    if (projectSelector) {
...
export async function fetchDaemonDashboardState(projectPath, projectId) {
    try {
        ...
        if (foremanBackendMode() === "elixir") {
            ...
            for (const project of visible) {
                ...
                const human = projectTasks.filter((task) => NEEDS_HUMAN_STATUSES.includes(task.status));
        ...
        for (const project of visible) {
            const [stats, human, runs] = await Promise.all([
                client.projects.stats({ projectId: project.id }),
                client.projects.listNeedsHuman({ projectId: project.id }),
                client.runs.listActive({ projectId: project.id }),
            ]);
            projectRows.push({
                id: project.id,
                name: project.name,
...
export const NEEDS_HUMAN_STATUSES = ["conflict", "failed", "stuck", "backlog"];
 ...
 * Satisfies REQ-011.1.
 */
export function sortNeedsHumanTasks(tasks) {
 ...
 */
export function readProjectRegistry(store) {
 ...
 */
function readProjectDbSnapshot(project, eventsLimit) {
    ...
    try {
        ...
        // ── Active runs ─────────────────────────────────────────────────
        const activeRuns = db.prepare(`SELECT * FROM runs WHERE project_id = ? AND status IN ('pending', 'running') ORDER BY created_at DESC`).all(project.id);
        // ── Completed runs (last 5) ──────────────────────────────────────
        const completedRuns = db.prepare(`SELECT * FROM runs WHERE project_id = ? AND status = 'completed' ORDER BY completed_at DESC LIMIT 5`).all(project.id);
        // ── Progresses ──────────────────────────────────────────────────
        const progresses = new Map();
        ...
        // ── Metrics ────────────────────────────────────────────────────
        const totalCostRow = db.prepare(`SELECT COALESCE(SUM(c.estimated_cost), 0) AS total_cost,
              COALESCE(SUM(c.tokens_in + c.tokens_out), 0) AS total_tokens
       FROM costs c
       JOIN runs r ON r.id = c.run_id
       WHERE r.project_id = ?`).get(project.id);
        const taskStatusRows = db.prepare(`SELECT r.status, COUNT(*) as count FROM runs r WHERE r.project_id = ? GROUP BY r.status`).all(project.id);
        const tasksByStatus = {};
        ...
        // ── Events ─────────────────────────────────────────────────────
        const events = db.prepare(`SELECT * FROM events WHERE project_id = ? ORDER BY created_at DESC LIMIT ?`).all(project.id, eventsLimit);
        // ── Success rate (last 24h) ─────────────────────────────────────
        const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
        const srRow = db.prepare(`SELECT
         SUM(CASE WHEN status = 'merged' THEN 1 ELSE 0 END) AS merged,
         SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) AS failed
       FROM runs
       WHERE project_id = ? AND completed_at >= ?`).get(project.id, since24h);
        const merged = srRow?.merged ?? 0;
        const failed = srRow?.failed ?? 0;
        const total = merged + failed;
        const rate = total >= 3 ? merged / total : null;
        // ── Needs Human tasks ───────────────────────────────────────────
        let needsHumanTasks = [];
        try {
            const placeholders = NEEDS_HUMAN_STATUSES.map(() => "?").join(", ");
            needsHumanTasks = db.prepare(`SELECT * FROM tasks WHERE status IN (${placeholders})
         ORDER BY priority ASC, updated_at ASC
         LIMIT 200`).all(...NEEDS_HUMAN_STATUSES).map((t) => ({
                ...t,
                projectName: project.name,
                projectId: project.id,
                projectPath: project.path,
...
function makeOfflineSnapshot(project) {
    return {
        project: {
            id: project.id,
            name: project.name,
 ...
 */
export async function readProjectSnapshot(projects, eventsLimit = 8) {
    ...
    if (event.details) {
        try {
            ...
            if (parsed.commitHash)
                parts.push(String(parsed.commitHash).slice(0, 8));
            if (parts.length > 0)
                detail = ` — ${parts.join(" ")}`;
        }
        catch {
 ...
 */
export function renderNeedsHumanPanel(tasks, maxRows = 10) {
    if (tasks.length === 0) {
        return (chalk.bold.red("⚠  NEEDS HUMAN ATTENTION:") + "\n" +
            THICK_RULE + "\n" +
    ...
    for (const task of visible) {
        ...
        lines.push(`  ${statusLabel} ${priorityLabel}  ${chalk.white(titleStr)}${projectLabel}  ${ageStr}`);
    }
 ...
 * Satisfies REQ-012.
 */
export function renderProjectAgentPanel(project, activeRuns, completedRuns, progresses, metrics, events, offline) {
    const lines = [];
...
export function renderDashboard(state) {
    ...
    for (const project of state.projects) {
        ...
        lines.push(renderProjectAgentPanel(project, activeRuns, completedRuns, state.progresses, projectMetrics, events, offline));
    }
    ...
    const globalRate = globalTotal >= 3 ? globalMerged / globalTotal : null;
 ...
 */
export async function approveTask(taskId, projectPath) {
    const projects = await listRegisteredProjects();
    const resolvedProjectPath = resolve(projectPath);
 ...
 * @param projectPath - Path to the project that owns this task.
 */
export async function retryTask(taskId, projectPath) {
    const projects = await listRegisteredProjects();
    const resolvedProjectPath = resolve(projectPath);
    const project = projects.find((record) => resolve(record.path) === resolvedProjectPath);
    if (!project)
