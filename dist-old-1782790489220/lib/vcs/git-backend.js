...
export class GitBackend {
    ...
    async createWorkspace(repoPath, seedId, baseBranch) {
        ...
        if (existsSync(workspacePath)) {
            try {
                ...
                const hasUnstagedChanges = rebaseMsg.includes("unstaged changes") ||
                ...
                if (hasUnstagedChanges) {
                    ...
                    throw new Error(`Rebase failed in ${workspacePath}: ${rebaseMsg.slice(0, 300)}`);
                }
            }
        ...
        try {
            await this.git(["worktree", "add", "-b", branchName, workspacePath, base], repoPath);
        }
        catch (err) {
    ...
    async removeWorkspace(repoPath, workspacePath) {
        try {
            ...
            if (!removeMsg.includes("is not a working tree")) {
                console.error(`[git] Warning: git worktree remove --force failed for ${workspacePath}: ${removeMsg}`);
            }
            try {
                ...
                console.error(`[git] Warning: fs.rm fallback also failed for ${workspacePath}: ${rmMsg}`);
            }
        }
        try {
            ...
            console.error(`[git] Warning: worktree prune failed after removing ${workspacePath}: ${msg}`);
        }
    }
    /**
    ...
    async saveWorktreeState(workspacePath) {
        try {
            const stashOut = await this.git(["stash", "push", "--include-untracked", "-m", "foreman-worktree-state"], workspacePath);
    ...
    async restoreWorktreeState(workspacePath) {
    ...
    async rebase(workspacePath, onto) {
        try {
            ...
            try {
                const statusOut = await this.git(["diff", "--name-only", "--diff-filter=U"], workspacePath);
    ...
    async restackBranch(repoPath, branchName, oldBase, newBase) {
        try {
    ...
    async getConflictingFiles(workspacePath) {
        const out = await this.git(["diff", "--name-only", "--diff-filter=U"], workspacePath);
        return out
            .split("\n")
            .map((f) => f.trim())
            .filter(Boolean);
     ...
     */
    async stageFile(workspacePath, filePath) {
        await this.git(["add", filePath], workspacePath);
    }
    async stageFiles(workspacePath, filePaths) {
     ...
     */
    async removeFromIndex(workspacePath, filePath) {
    ...
    async getUntrackedFiles(workspacePath) {
        try {
            const out = await this.git(["ls-files", "--others", "--exclude-standard"], workspacePath);
            return out.split("\n").map((f) => f.trim()).filter(Boolean);
        }
        catch {
            return [];
