...
import { buildTrackedStateRestoreCommand, getWorkspacePath, getWorkspaceRoot, } from "../workspace-paths.js";
...
export class JujutsuBackend {
    ...
    async getBookmarksAtRevision(repoPath, rev) {
        try {
            const bookmarks = await this.jj(["log", "--no-graph", "-r", rev, "-T", "separate(' ', bookmarks)"], repoPath);
            return bookmarks
                .split(" ")
    ...
    async branchExists(repoPath, branchName) {
        try {
            const out = await this.jj(["bookmark", "list", branchName], repoPath);
     ...
     */
    async branchExistsOnRemote(repoPath, branchName) {
        try {
            const out = await this.jj(["bookmark", "list", "--remote", "origin", branchName], repoPath);
            return out.includes(branchName);
        }
        catch {
            return false;
    ...
    async deleteBranch(repoPath, branchName, options) {
        ...
        const targetBranch = options?.targetBranch ?? (await this.detectDefaultBranch(repoPath));
        let isFullyMerged = false;
        try {
            await this.git(["merge-base", "--is-ancestor", branchName, targetBranch], repoPath);
            isFullyMerged = true;
        }
        catch {
            isFullyMerged = false;
   ...
   * that branch/revision instead of inheriting the controller workspace parent.
     ...
     */
    async createWorkspace(repoPath, seedId, baseBranch) {
        ...
        try {
            await this.jj(["bookmark", "create", branchName, "-r", workspaceRef], repoPath);
        }
        catch {
            // Bookmark may already exist — try to move it
            try {
                await this.jj(["bookmark", "move", branchName, "--allow-backwards", "--to", workspaceRef], repoPath);
            }
            catch (moveErr) {
     ...
     */
    async removeWorkspace(repoPath, workspacePath) {
     ...
     */
    async listWorkspaces(repoPath) {
        try {
            const raw = await this.jj(["workspace", "list"], repoPath);
            if (!raw)
                return [];
            const workspaces = [];
     ...
     */
    async commitNoEdit(workspacePath) {
     ...
     */
    async push(workspacePath, branchName, options) {
     ...
     */
    async pull(workspacePath, branchName) {
        await this.jj(["git", "fetch", "--remote", "origin"], workspacePath);
        try {
            await this.jj(["bookmark", "track", `${branchName}@origin`], workspacePath);
        }
        catch {
            // bookmark may already be tracked
        }
    }
    async saveWorktreeState(_workspacePath) {
        return false;
    }
    async restoreWorktreeState(_workspacePath) {
        return;
    }
    ...
    async rebaseBranch(repoPath, branchName, onto) {
        try {
    ...
    async restackBranch(repoPath, branchName, _oldBase, newBase) {
     ...
     */
    async abortRebase(workspacePath) {
        try {
    ...
    async merge(repoPath, sourceBranch, targetBranch) {
        ...
        try {
            ...
            await this.jj(["new", target, sourceBranch, "-m", `Merge ${sourceBranch} into ${target}`], repoPath);
            return { success: true };
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    async mergeWithStrategy(repoPath, sourceBranch, targetBranch, strategy) {
        try {
    ...
    async rollbackFailedMerge(workspacePath, beforeRef) {
     ...
     */
    async getHeadId(workspacePath) {
        const currentBookmarks = await this.getBookmarksAtRevision(workspacePath, "@");
        if (currentBookmarks.length > 0) {
            return this.jj(["log", "--no-graph", "-r", "@", "-T", "change_id.short()"], workspacePath);
        }
     ...
     */
    async resolveRef(repoPath, ref) {
        return this.jj(["log", "--no-graph", "-r", ref, "-T", "commit_id"], repoPath);
    }
    async isAncestor(repoPath, ancestorRef, descendantRef) {
        try {
            const ancestorCommit = await this.resolveRef(repoPath, ancestorRef);
            const reachable = await this.jj(["log", "--no-graph", "-r", `${ancestorRef}::${descendantRef}`, "-T", "commit_id"], repoPath);
     ...
     */
    async getChangedFiles(repoPath, from, to) {
        try {
            const out = await this.jj(["diff", "--summary", "--from", from, "--to", to], repoPath);
            if (!out)
                return [];
            return out
                .split("\n")
     ...
     */
    async getRefCommitTimestamp(repoPath, ref) {
        try {
            const out = await this.jj(["log", "-r", ref, "--no-graph", "-T", "author.timestamp().utc().unix()"], repoPath);
     ...
     */
    async getModifiedFiles(workspacePath) {
        const out = await this.jj(["diff", "--summary", "-r", "@"], workspacePath);
        return out
            .split("\n")
            .map((l) => l.replace(/^[MA?D]\s+/, "").trim())
            .filter(Boolean);
     ...
     */
    async getConflictingFiles(workspacePath) {
        try {
     ...
     */
    async cleanWorkingTree(workspacePath) {
     ...
     */
    async mergeWithoutCommit(repoPath, sourceBranch, targetBranch) {
     ...
     */
    async abortMerge(workspacePath) {
     ...
     */
    async stageFile(_workspacePath, _filePath) {
        // jj auto-stages — nothing to do
    }
    async stageFiles(_workspacePath, _filePaths) {
        // jj auto-stages — nothing to do
    }
    /**
     ...
     */
    async checkoutFile(workspacePath, ref, filePath) {
        ...
        if (ref === "--theirs") {
            ...
            try {
                const content = await this.jj(["file show", "@-", "--", filePath], workspacePath);
        ...
        // Normal ref checkout
        const content = await this.jj(["file", "show", ref, "--", filePath], workspacePath);
        await fs.writeFile(join(workspacePath, filePath), content, "utf-8");
    }
    /**
     * Get the content of a file at a specific ref.
     */
    async showFile(repoPath, ref, filePath) {
     ...
     */
    async removeFromIndex(_workspacePath, _filePath) {
        // jj has no separate index — nothing to do
    }
    /**
     * Apply a patch file via colocated git metadata.
     */
    async applyPatchToIndex(workspacePath, patchFilePath) {
     ...
     */
    async getMergeBase(repoPath, ref1, ref2) {
        try {
            // Get parents of ref1
            const parents1 = await this.jj(["log", "-r", ref1, "--no-graph", "-T", "parents"], repoPath);
            ...
            for (const parent of parents1.split(" ").filter(Boolean)) {
                try {
                    ...
                    const base = await this.jj(["log", "-r", `(${parent}) && (${ref2})`, "--no-graph", "-T", "change_id"], repoPath);
                    if (base.trim()) {
                        return parent;
                    }
                }
     ...
     */
    async getUntrackedFiles(workspacePath) {
        try {
            ...
            const trackedOut = await this.jj(["files", "--rev", "@"], workspacePath);
            const trackedFiles = new Set(trackedOut.split("\n").map((f) => f.trim()).filter(Boolean));
...
//# sourceMappingURL=jujutsu-backend.js.map