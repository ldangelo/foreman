...
const LEGACY_TASK_UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
export function normalizeTaskIdPrefix(raw) {
...
export function isLegacyUuidTaskId(taskId) {
    return LEGACY_TASK_UUID_PATTERN.test(taskId);
}
export function isCompactTaskId(taskId) {
    return new RegExp(`^[a-z0-9]+(?:-[a-z0-9]+)*-[0-9a-f]{${COMPACT_TASK_ID_SUFFIX_HEX_LENGTH}}$`, "i").test(taskId);
}
export function formatTaskIdDisplay(taskId) {
...
export function parsePriority(value) {
    ...
    if (aliasedPriority !== undefined)
        return aliasedPriority;
    const n = parseInt(lower, 10);
    if (!isNaN(n) && n >= 0 && n <= 4)
        return n;
    throw new RangeError(`Invalid priority '${value}'. Use 0–4 or: critical, high, medium, low, backlog`);
}
/**
...
export class InvalidStatusTransitionError extends Error {
    ...
    fromStatus;
    toStatus;
    constructor(taskId, fromStatus, toStatus) {
...
export class CircularDependencyError extends Error {
    ...
    constructor(fromId, toId) {
        super(`Adding dependency from '${fromId}' to '${toId}' would create a circular dependency`);
        this.fromId = fromId;
        this.toId = toId;
...
export class NativeTaskStore {
    ...
    canRunMigrations() {
        const dbLike = this.db;
        return (typeof dbLike.prepare === "function" &&
    ...
    migrateLegacyTaskIds() {
        ...
        const candidates = this.db
            .prepare(`SELECT t.id, t.status, r.status AS run_status
           FROM tasks t
           LEFT JOIN runs r ON r.id = t.run_id`)
            .all();
        const existingIds = new Set(this.db.prepare("SELECT id FROM tasks").all()
            .map((row) => row.id));
        const remaps = [];
                ...
                for (const { oldId, newId } of remaps) {
                  ...
                  WHERE seed_id = ? OR branch_name LIKE ?`)
                    ...
                    if (hasConflictPatterns) {
                        this.db
                            .prepare(`UPDATE conflict_patterns
                    SET seed_id = ?
                  WHERE seed_id = ?`)
                            .run(newId, oldId);
                    }
                    this.db
                        .prepare(`UPDATE task_dependencies
                  SET from_task_id = ?
                WHERE from_task_id = ?`)
                        .run(newId, oldId);
                    this.db
    ...
    create(opts) {
        ...
        this.db
         ...
         VALUES (?, ?, ?, ?, ?, 'backlog', ?, ?, ?)`)
            .run(id, opts.title, opts.description ?? null, opts.type ?? "task", opts.priority ?? 2, opts.externalId ?? null, now, now);
    ...
    approve(id) {
        ...
        const tx = this.db.transaction(() => {
            ...
            this.db
                .prepare("UPDATE tasks SET status = 'ready', approved_at = ?, updated_at = ? WHERE id = ?")
                .run(now, now, id);
        });
        tx();
    }
    ...
    close(id, _reason) {
        ...
        const tx = this.db.transaction(() => {
            ...
            this.db
                .prepare("UPDATE tasks SET status = 'closed', closed_at = ?, updated_at = ? WHERE id = ?")
                .run(now, now, id);
        });
        tx();
    }
    ...
    claim(id, runId) {
        ...
        const tx = this.db.transaction(() => {
            ...
            if (row.run_id && row.run_id !== runId) {
                throw new Error(`NativeTaskStore.claim: task '${id}' already claimed by run '${row.run_id}'`);
            }
            this.db
                .prepare("UPDATE tasks SET status = 'in-progress', run_id = ?, updated_at = ? WHERE id = ?")
                .run(runId, now, id);
        });
        tx();
    }
    ...
    addDependency(fromId, toId, type = "blocks") {
        const tx = this.db.transaction(() => {
            ...
            const from = this.db.prepare("SELECT id FROM tasks WHERE id = ?").get(fromId);
            if (!from)
                throw new TaskNotFoundError(fromId);
            const to = this.db.prepare("SELECT id FROM tasks WHERE id = ?").get(toId);
            if (!to)
            ...
            this.db
                .prepare(`INSERT OR IGNORE INTO task_dependencies (from_task_id, to_task_id, type)
           VALUES (?, ?, ?)`)
     ...
     */
    removeDependency(fromId, toId, type = "blocks") {
        this.db
            .prepare("DELETE FROM task_dependencies WHERE from_task_id = ? AND to_task_id = ? AND type = ?")
            .run(fromId, toId, type);
    }
    /**
     ...
     */
    getDependencies(id, direction = "outgoing") {
     ...
     */
    hasCyclicDependency(fromId, toId) {
