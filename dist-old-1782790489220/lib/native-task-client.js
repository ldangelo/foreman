...
import { InvalidStatusTransitionError, NativeTaskStore, TaskNotFoundError, normalizeTaskIdPrefix, parsePriority, } from "./task-store.js";
const COMPACT_TASK_ID_SUFFIX_HEX_LENGTH = 5;
function allocateTaskId(projectPath) {
...
function toIssue(projectPath, issue) {
    return {
        ...issue,
...
export class NativeTaskClient {
    ...
    validateStatusTransition(id, fromStatus, toStatus) {
        const statusOrder = {
            backlog: 0,
            ready: 1,
            "in-progress": 2,
    ...
    async list(opts) {
        ...
        return this.withStore((taskStore) => taskStore
    ...
    async ready() {
        if (this.registeredProjectId) {
            return (await this.postgres.listReadyTasks(this.registeredProjectId)).map((issue) => this.toPostgresIssue(issue));
        }
...
//# sourceMappingURL=native-task-client.js.map