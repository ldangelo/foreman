...
function createDisabledLocalStoreDb() {
...
function normalizeNativeTask(row) {
    if (!row)
        return null;
    return {
        ...row,
...
function normalizeNativeTasks(rows) {
    return rows.map((row) => normalizeNativeTask(row)).filter((row) => row !== null);
}
// ── Error classes ───────────────────────────────────────────────────────
/**
...
export class InvalidTaskStatusError extends Error {
    attemptedStatus;
    validStatuses;
    constructor(attemptedStatus, validStatuses) {
        super(`Invalid task status '${attemptedStatus}'. Valid statuses: ${validStatuses.join(", ")}`);
...
function tasksTableNeedsClosedStatusMigration(db) {
    const row = db.prepare("SELECT sql FROM information_schema.tables WHERE table_name = 'tasks'").get();
    const schemaSql = row?.sql;
    return typeof schemaSql === "string" && !schemaSql.includes("'closed'");
}
function migrateLegacyTasksTable(db) {
    const taskRows = db
        .prepare(`SELECT id, title, description, type, priority, status, run_id, branch,
              external_id, created_at, updated_at, approved_at, closed_at
         FROM tasks`)
        .all();
    let dependencyRows = [];
    try {
        dependencyRows = db.prepare("SELECT from_task_id, to_task_id, type FROM task_dependencies").all();
    }
    catch {
        dependencyRows = [];
    }
        ...
        const tx = db.transaction(() => {
          ...
          external_id, created_at, updated_at, approved_at, closed_at
            ...
            for (const row of taskRows) {
                insertTask.run(row.id, row.title, row.description, row.type, row.priority, normalizeLegacyTaskStatus(row.status), row.run_id, row.branch, row.external_id, row.created_at, row.updated_at, row.approved_at, row.closed_at);
            }
            db.exec("DROP TABLE tasks_legacy");
            db.exec(TASK_DEPENDENCIES_SCHEMA);
            if (dependencyRows.length > 0) {
                const insertDependency = db.prepare("INSERT INTO task_dependencies (from_task_id, to_task_id, type) VALUES (?, ?, ?)");
...
export class ForemanStore {
     ...
     */
    static forDashboard(projectPath) {
        return new ForemanStore(join(projectPath, ".foreman", "foreman.db"));
    }
    /**
     ...
     * @returns A disabled local-store compatibility handle.
     */
    static openReadonly(_projectPath) {
     ...
     */
    listTasksByStatus(statuses, limit = 200) {
    ...
    registerProject(name, path) {
        ...
        this.db
            .prepare(`INSERT INTO projects (id, name, path, status, created_at, updated_at)
    ...
    // ── Runs ────────────────────────────────────────────────────────────
    createRun(projectId, seedId, agentType, worktreePath, opts) {
        const now = new Date().toISOString();
        const run = {
            id: randomUUID(),
        ...
        this.db
            .prepare(`INSERT INTO runs (id, project_id, seed_id, agent_type, session_key, worktree_path, status, started_at, completed_at, created_at, base_branch, merge_strategy)
    ...
    getActiveRuns(projectId) {
        if (projectId) {
            return this.db
                .prepare("SELECT * FROM runs WHERE project_id = ? AND status IN ('pending', 'running') ORDER BY created_at DESC")
                .all(projectId);
        }
        return this.db
            .prepare("SELECT * FROM runs WHERE status IN ('pending', 'running') ORDER BY created_at DESC")
            .all();
    }
    getRunsByStatus(status, projectId) {
        if (projectId) {
            return this.db
                .prepare("SELECT * FROM runs WHERE project_id = ? AND status = ? ORDER BY created_at DESC")
     ...
     */
    getRunsByStatuses(statuses, projectId) {
        if (statuses.length === 0)
            return [];
        const placeholders = statuses.map(() => "?").join(", ");
        if (projectId) {
            return this.db
                .prepare(`SELECT * FROM runs WHERE project_id = ? AND status IN (${placeholders}) ORDER BY created_at DESC`)
    ...
    getRunsByStatusSince(status, since, projectId) {
        if (projectId) {
            return this.db
                .prepare("SELECT * FROM runs WHERE project_id = ? AND status = ? AND created_at >= ? ORDER BY created_at DESC")
                .all(projectId, status, since);
        }
        return this.db
     ...
     */
    getRunsByStatusesSince(statuses, since, projectId) {
        if (statuses.length === 0)
            return [];
        const placeholders = statuses.map(() => "?").join(", ");
        if (projectId) {
            return this.db
                .prepare(`SELECT * FROM runs WHERE project_id = ? AND status IN (${placeholders}) AND created_at >= ? ORDER BY created_at DESC`)
                .all(projectId, ...statuses, since);
        }
        return this.db
            .prepare(`SELECT * FROM runs WHERE status IN (${placeholders}) AND created_at >= ? ORDER BY created_at DESC`)
            .all(...statuses, since);
    }
    /**
     ...
     */
    purgeOldRuns(olderThan, projectId) {
        const terminalStatuses = ["failed", "merged", "test-failed", "conflict"];
        const placeholders = terminalStatuses.map(() => "?").join(", ");
        if (projectId) {
            const result = this.db
                .prepare(`DELETE FROM runs WHERE project_id = ? AND status IN (${placeholders}) AND created_at < ?`)
                .run(projectId, ...terminalStatuses, olderThan);
            return result.changes;
        }
        const result = this.db
            .prepare(`DELETE FROM runs WHERE status IN (${placeholders}) AND created_at < ?`)
    ...
    getRunsForSeed(seedId, projectId) {
        if (projectId) {
            return this.db
                .prepare("SELECT * FROM runs WHERE project_id = ? AND seed_id = ? ORDER BY created_at DESC, rowid DESC")
                .all(projectId, seedId);
        }
        return this.db
     ...
     */
    hasActiveOrPendingRun(seedId, projectId) {
        ...
        if (projectId) {
            row = this.db
                .prepare(`SELECT 1 FROM runs WHERE project_id = ? AND seed_id = ? AND status IN (${placeholders}) LIMIT 1`)
                .get(projectId, seedId, ...blockingStatuses);
        }
        else {
            row = this.db
                .prepare(`SELECT 1 FROM runs WHERE seed_id = ? AND status IN (${placeholders}) LIMIT 1`)
     ...
     */
    getRunsByBaseBranch(baseBranch, projectId) {
        if (projectId) {
            return this.db
                .prepare("SELECT * FROM runs WHERE project_id = ? AND base_branch = ? ORDER BY created_at DESC")
                .all(projectId, baseBranch);
        }
        return this.db
    ...
    // ── Costs ───────────────────────────────────────────────────────────
    recordCost(runId, tokensIn, tokensOut, cacheRead, estimatedCost) {
        this.db
            .prepare(`INSERT INTO costs (id, run_id, tokens_in, tokens_out, cache_read, estimated_cost, recorded_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(randomUUID(), runId, tokensIn, tokensOut, cacheRead, estimatedCost, new Date().toISOString());
    }
    getCosts(projectId, since) {
        if (projectId && since) {
            return this.db
                .prepare(`SELECT c.* FROM costs c
        ...
        return this.db.prepare("SELECT * FROM costs ORDER BY recorded_at DESC").all();
    }
    /**
    ...
    getCostBreakdown(runId) {
        ...
        const byPhase = { ...(progress.costByPhase ?? {}) };
     ...
     */
    getPhaseMetrics(projectId, since) {
        const conditions = [];
        const params = [];
        if (projectId) {
    ...
    // ── Success Rate ─────────────────────────────────────────────────────
    getRecentOutcomeCounts(projectId, since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString()) {
        const rows = projectId
            ? this.db
                .prepare(`SELECT rowid, seed_id, status, completed_at, created_at FROM runs
           WHERE project_id = ?
        ...
        let stuck = 0;
        for (const row of rows) {
            if (seenSeeds.has(row.seed_id))
                continue;
            seenSeeds.add(row.seed_id);
            if (!row.completed_at || row.completed_at <= since)
                continue;
    ...
    // ── Events ──────────────────────────────────────────────────────────
    logEvent(projectId, eventType, details, runId) {
        const detailsStr = details
            ...
            : null;
        this.db
            .prepare(`INSERT INTO events (id, project_id, run_id, event_type, details, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
            .run(randomUUID(), projectId, runId ?? null, eventType, detailsStr, new Date().toISOString());
    }
    getEvents(projectId, limit, eventType) {
        const conditions = [];
        const params = [];
        if (projectId) {
            conditions.push("project_id = ?");
     ...
     */
    logRateLimitEvent(projectId, model, phase, error, retryAfterSeconds, runId) {
        this.db
            .prepare(`INSERT INTO rate_limit_events (id, project_id, run_id, model, phase, error, retry_after_seconds, recorded_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(randomUUID(), projectId, runId ?? null, model, phase ?? null, error, retryAfterSeconds ?? null, new Date().toISOString());
    }
    /**
     ...
     */
    getRateLimitCountsByModel(projectId, hoursBack = 24) {
        const since = new Date(Date.now() - hoursBack * 60 * 60 * 1000).toISOString();
        const rows = this.db
            .prepare(`SELECT model, COUNT(*) as count FROM rate_limit_events
     ...
     */
    getRecentRateLimitEvents(projectId, limit = 10) {
        return this.db
            .prepare(`SELECT id, model, phase, error, retry_after_seconds, recorded_at
     ...
     */
    getMessages(runId, agentType, unreadOnly = false) {
        if (unreadOnly) {
            return this.db
     ...
     */
    getAllMessagesGlobal(limit = 200) {
     ...
     */
    markAllMessagesRead(runId, agentType) {
        this.db
            .prepare("UPDATE messages SET read = 1 WHERE run_id = ? AND recipient_agent_type = ? AND deleted_at IS NULL")
            .run(runId, agentType);
    }
    /**
    ...
    // ── Sentinel ─────────────────────────────────────────────────────────
    upsertSentinelConfig(projectId, config) {
        const now = new Date().toISOString();
        const existing = this.getSentinelConfig(projectId);
        if (existing) {
            const fields = ["updated_at = @updated_at"];
            const values = { project_id: projectId, updated_at: now };
            ...
            this.db.prepare(`INSERT INTO sentinel_configs (project_id, branch, test_command, interval_minutes, failure_threshold, enabled, pid, created_at, updated_at)
         VALUES (@project_id, @branch, @test_command, @interval_minutes, @failure_threshold, @enabled, @pid, @created_at, @updated_at)`).run(row);
            return this.getSentinelConfig(projectId);
        }
    }
    getSentinelConfig(projectId) {
        return (this.db.prepare("SELECT * FROM sentinel_configs WHERE project_id = ?").get(projectId) ?? null);
    }
    recordSentinelRun(run) {
        this.db.prepare(`INSERT INTO sentinel_runs (id, project_id, branch, commit_hash, status, test_command, output, failure_count, started_at, completed_at)
       VALUES (@id, @project_id, @branch, @commit_hash, @status, @test_command, @output, @failure_count, @started_at, @completed_at)`).run({
            id: run.id,
    ...
    getSentinelRuns(projectId, limit) {
        const conditions = [];
        const params = [];
        if (projectId) {
            conditions.push("project_id = ?");
    ...
    setMergeAgentConfig(config) {
        ...
        if (existing) {
            ...
            this.db
                .prepare(`INSERT INTO merge_agent_config (id, enabled, poll_interval_ms, created_at, updated_at)
    ...
    getMetrics(projectId, since) {
        ...
        const totals = this.db
            .prepare(`SELECT COALESCE(SUM(c.estimated_cost), 0) as totalCost,
    ...
    getTaskById(id) {
        try {
            return normalizeNativeTask(this.db
