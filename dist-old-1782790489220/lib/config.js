...
function envNonNegativeNumber(name, defaultValue) {
    const raw = process.env[name];
    if (raw === undefined || raw === "")
        return defaultValue;
 ...
 */
function envModel(name, defaultValue) {
    const raw = process.env[name];
    if (raw === undefined || raw === "")
        return defaultValue;
    return raw;
...
export function getHighspeedModel() {
 ...
 */
export function calculateRateLimitBackoffMs(retryCount) {
    const delays = [
