...
export class ProjectRegistry {
        ...
        try {
            ...
            if (!Array.isArray(parsed)) {
                ...
                if (typeof legacy?.version === "number" && Array.isArray(legacy?.projects)) {
                    ...
                    return this.migrateLegacyRecords(legacy.projects);
                }
                throw new ProjectRegistryJsonError(`Invalid projects.json: expected array, got ${typeof parsed}`);
            }
            return parsed;
        }
        catch (err) {
    ...
    async writeJson(records) {
        ...
        try {
            ...
            throw new ProjectRegistryJsonError(`Failed to write projects.json: ${err.message}`, err);
        }
    }
    ...
    async writeJsonMirror(records) {
        try {
            await this.writeJson(records);
            this.invalidateCache();
        }
        catch (err) {
            console.warn(`[ProjectRegistry] JSON mirror write failed: ${err.message}. ` +
     ...
     */
    async update(projectIdOrName, patch) {
        if (this.pg) {
     ...
     * @throws ProjectNotFoundError if project doesn't exist
     */
    async updateJiraConfig(projectIdOrName, jiraUpdate) {
        ...
        if (!config.issueTracker) {
            config.issueTracker = { backend: "jira", jira: {} };
        }
        // Get current jira config
        const issueTracker = config.issueTracker;
        const jiraConfig = issueTracker.jira ?? {};
        // Merge jira updates
        if (jiraUpdate.apiUrl !== undefined)
            jiraConfig.apiUrl = jiraUpdate.apiUrl;
        if (jiraUpdate.email !== undefined)
            jiraConfig.email = jiraUpdate.email;
        if (jiraUpdate.apiToken !== undefined)
            jiraConfig.apiToken = jiraUpdate.apiToken;
        if (jiraUpdate.pollIntervalSeconds !== undefined)
            jiraConfig.pollIntervalSeconds = jiraUpdate.pollIntervalSeconds;
        if (jiraUpdate.webhookEnabled !== undefined)
            jiraConfig.webhookEnabled = jiraUpdate.webhookEnabled;
        if (jiraUpdate.webhookSecretEnvVar !== undefined)
            jiraConfig.webhookSecretEnvVar = jiraUpdate.webhookSecretEnvVar;
    ...
    resolve(nameOrIdOrPath) {
        ...
        try {
            ...
            if (!Array.isArray(parsed)) {
                const legacy = parsed;
                if (typeof legacy?.version === "number" && Array.isArray(legacy?.projects)) {
                    arr = this.migrateLegacyRecordsSync(legacy.projects);
                }
                else {
                    arr = [];
                }
    ...
    migrateLegacyRecordsSync(entries) {
        const now = new Date().toISOString();
        return entries.map((entry) => ({
            id: this.generateProjectId(entry.name),
            name: entry.name,
    ...
    async sync(projectId) {
        ...
        try {
            ...
            console.warn(`[ProjectRegistry] git fetch failed for '${projectId}': ${err.message}`);
        }
        const now = new Date().toISOString();
