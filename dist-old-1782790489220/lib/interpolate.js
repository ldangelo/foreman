const SUPPORTED_KEYS = new Set(['id', 'title', 'description', 'type', 'priority', 'projectReportsDir']);
/**
 ...
 */
export function interpolateTaskPlaceholders(template, task) {
    if (!template)
        return template;
    const result = [];
    let i = 0;
        ...
        if (template[i] === '{') {
            ...
            if (match) {
                ...
                if (SUPPORTED_KEYS.has(key)) {
                    ...
                    console.warn(`[interpolate] Unknown placeholder \`{task.${key}}\` — leaving as-is.`);
