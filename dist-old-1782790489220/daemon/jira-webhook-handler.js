 ...
 */
export function verifyJiraSignature(rawBody, signature, secret) {
    if (!signature || !secret)
        return false;
    ...
    const expected = createHmac("sha256", secret)
        ...
        .digest("hex");
    try {
        return timingSafeEqual(Buffer.from(expected, "utf8"), Buffer.from(sigValue, "utf8"));
    }
    catch {
        return false;
    }
 ...
 */
export function generateJiraWebhookSecret() {
 ...
 */
export function createJiraWebhookHandler(ctx, config) {
    return async (request, reply) => {
        const rawBody = request.body;
        const bodyBuffer = Buffer.from(JSON.stringify(rawBody));
        // Verify signature
        const signature = request.headers["x-atlassian-webhook-signature-v2"];
        ...
        request.log.info({ event: payload.webhookEvent, issue: payload.issue?.key }, "[jira-webhook] Received Jira event");
        ...
        if (!projectConfig.startStatus.includes(statusChange.to)) {
            request.log.info({ issue: payload.issue.key, newStatus: statusChange.to }, "[jira-webhook] Transition not to startStatus — skipping");
            return reply.code(200).send({ received: true, ignored: true });
        }
        // Check if transitioning FROM a startStatus (should not trigger)
        if (projectConfig.startStatus.includes(statusChange.from)) {
            request.log.info({ issue: payload.issue.key, from: statusChange.from, to: statusChange.to }, "[jira-webhook] Transition from startStatus to startStatus — skipping (already triggered)");
            return reply.code(200).send({ received: true, ignored: true });
        }
        ...
        try {
            ...
            if (result.triggered) {
                request.log.info({ issue: payload.issue.key, taskId: result.taskId }, "[jira-webhook] Triggered workflow successfully");
                return reply.code(200).send({ received: true, triggered: true, taskId: result.taskId });
            }
            else {
                request.log.info({ issue: payload.issue.key, reason: result.reason }, `[jira-webhook] Skipped: ${result.reason}`);
                return reply.code(200).send({ received: true, triggered: false, reason: result.reason });
            }
        }
        catch (err) {
            request.log.error({ error: err instanceof Error ? err.message : String(err) }, "[jira-webhook] Error processing webhook");
            return reply.code(500).send({ error: "Internal error processing webhook" });
        }
    };
}
//# sourceMappingURL=jira-webhook-handler.js.map