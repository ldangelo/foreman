...
import { GhCli, GhNotInstalledError, GhNotAuthenticatedError, GhError, } from "../lib/gh-cli.js";
...
const tasksRouter = t.router({
    ...
    claim: t.procedure
        ...
        .mutation(async ({ input, ctx }) => {
        const claimed = await ctx.adapter.claimTask(input.projectId, input.taskId, input.runId);
        if (!claimed)
            return { claimed: false, taskId: input.taskId };
...
const githubRouter = t.router({
    ...
    upsertRepo: t.procedure
        ...
        .mutation(async ({ input, ctx }) => {
        const existing = await ctx.adapter.getGithubRepo(input.projectId, input.owner, input.repo);
    ...
    listSyncEvents: t.procedure
        ...
        .query(async ({ input, ctx }) => {
        return ctx.adapter.listGithubSyncEvents(input.projectId, input.externalId, input.limit ?? 100);
    }),
    // --- Sync operations -----------------------------------------------------
    /**
    ...
    syncIssues: t.procedure
        ...
        .mutation(async ({ input, ctx }) => {
        // Fetch repo config
        const repoConfig = await ctx.adapter.getGithubRepo(input.projectId, input.owner, input.repo);
        if (!repoConfig) {
            throw new TRPCError({
                code: "NOT_FOUND",
        ...
        if (input.mode === "pull" || input.mode === "bidirectional") {
            ...
            for (const ghIssue of githubIssues) {
                ...
                await ctx.adapter.recordGithubSyncEvent({
                    ...
                    resolvedWith: strategy === "github-wins" || strategy === "last-write-wins"
        ...
        if (input.mode === "push" || input.mode === "bidirectional") {
            ...
            const foremanTasks = await ctx.adapter.listTasksWithExternalId(input.projectId);
            for (const task of foremanTasks) {
                ...
                if (!issueNumber)
                    continue;
                try {
                    const ghIssue = await ctx.gh.updateIssue(input.owner, input.repo, issueNumber, {
                        title: task.title,
        ...
        if (input.mode === "create") {
            ...
            const foremanOnlyTasks = allTasks.filter((t) => t.external_id === null || !t.external_id.startsWith("github:"));
...
const mailRouter = t.router({
    send: t.procedure
        ...
        .mutation(async ({ input, ctx }) => {
        return ctx.adapter.sendMessage(input.projectId, input.runId, input.senderAgentType, input.recipientAgentType, input.subject, input.body);
    }),
    list: t.procedure
        .input(z.object({
        projectId: PROJECT_ID_SCHEMA,
...
const projectsRouter = t.router({
    ...
    add: t.procedure
        ...
        try {
            ...
            if (err instanceof GhNotInstalledError) {
                throw new TrpcProjectError("GitHub CLI (gh) is required but not installed. Install it from https://cli.github.com");
            }
            if (err instanceof GhNotAuthenticatedError) {
                throw new TrpcProjectError(`GitHub authentication required: ${err.message}. Run 'gh auth login' first.`);
            }
            throw err;
        }
        ...
        try {
            ...
            if (err instanceof GhError) {
                throw new TrpcProjectError(`Failed to fetch repository metadata for '${input.githubUrl}': ${err.message}`);
            }
            throw err;
        }
        ...
        try {
            ...
            if (err instanceof GhNotAuthenticatedError) {
                throw new TrpcProjectError(`GitHub authentication required to clone '${input.githubUrl}'. Run 'gh auth login' first.`);
            }
            if (err instanceof GhError) {
                throw new TrpcProjectError(`Failed to clone repository '${input.githubUrl}': ${err.message}`);
            }
            throw err;
        }
    ...
    update: t.procedure
        ...
        const record = await registry.update(input.id, basicUpdates);
    ...
    remove: t.procedure
        ...
        if (!input.force) {
            ...
            if (hasActive) {
                throw new TrpcProjectError(`Project '${input.id}' has active runs. ` +
...
function parseGitHubUrl(url) {
    ...
    throw new TrpcProjectError(`Invalid GitHub URL '${url}'. Expected formats: ` +
        `"owner/repo", "https://github.com/owner/repo", or "git@github.com:owner/repo"`);
}
function toRepoKey(owner, repo) {
 ...
 */
async function hasActiveRuns(projectId, ctx) {
