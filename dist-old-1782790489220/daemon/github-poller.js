...
export class GitHubIssuesPoller {
    ...
    _running = false;
    _stopped = false;
    constructor(adapter, registry, config = {}, gh) {
    ...
    start() {
        ...
        const intervalMs = parseInt(process.env.FOREMAN_GITHUB_POLL_INTERVAL_MS ?? "", 10) || this.config.pollIntervalMs;
     ...
     */
    async pollAll() {
        const summary = { repos: 0, issues: 0, imported: 0, updated: 0, errors: 0 };
        ...
        for (const project of projects) {
            ...
            try {
                ...
                console.error(`[GitHubIssuesPoller] Error polling project "${project.name}":`, err instanceof Error ? err.message : String(err));
                summary.errors++;
            }
        }
        return summary;
    ...
    async pollProject(project) {
        ...
        try {
            ...
            console.error(`[GitHubIssuesPoller] Failed to list repos for project ${project.id}:`, err instanceof Error ? err.message : String(err));
            return summary;
        }
        if (repos.length === 0)
            return summary;
        for (const repo of repos) {
            try {
                ...
                console.error(`[GitHubIssuesPoller] Error polling ${repo.owner}/${repo.repo}:`, err instanceof Error ? err.message : String(err));
                summary.errors++;
            }
        }
        return summary;
     ...
     */
    async pollRepo(projectId, owner, repo) {
        let issues;
        try {
            ...
            if (err instanceof GhRateLimitError) {
                console.warn(`[GitHubIssuesPoller] Rate limited polling ${owner}/${repo}; backing off. Retry after ${err.retryAfter}s`);
        ...
        for (const issue of issues) {
            ...
            if (existing.length > 0) {
                ...
                const changed = task.title !== issue.title ||
                ...
                imported++;
                console.log(`[GitHubIssuesPoller] Imported issue #${issue.number} as task ${task.id} (${status})`);
            }
        }
    ...
    shouldBeReady(issue) {
        ...
        return (labelNames.includes(this.config.foremanLabel) ||
     ...
     */
    async pollRepoManual(projectId, owner, repo) {
 ...
 */
export async function closeLinkedGithubIssue(adapter, gh, projectId, taskId) {
    try {
        const task = await adapter.getTask(projectId, taskId);
        if (!task) {
            console.warn(`[closeLinkedGithubIssue] Task ${taskId} not found in project ${projectId}`);
            return;
        }
        ...
        console.error(`[closeLinkedGithubIssue] Failed to close linked GitHub issue for task ${taskId}:`, err instanceof Error ? err.message : String(err));
    }
}
/**
 ...
 */
export async function linkPrToGithubIssue(gh, projectId, taskId, prUrl) {
    try {
        ...
        console.log(`[linkPrToGithubIssue] Linked PR ${prUrl} to GitHub issue #${issueNumber}`);
    }
    catch (err) {
        console.error(`[linkPrToGithubIssue] Failed to link PR to GitHub issue:`, err instanceof Error ? err.message : String(err));
    }
}
