...
import { NullAgentMailClient } from "../lib/agent-mail-client.js";
import { createProjectMailClient } from "../lib/project-mail-client.js";
import { createBashTool, createReadTool, createEditTool, createWriteTool, createGrepTool, createFindTool, createLsTool, } from "@mariozechner/pi-coding-agent";
const execFileAsync = promisify(execFileSync);
export function wrapLocalRefineryQueue(queue) {
    return {
...
export class RefineryAgent {
    ...
    constructor(mergeQueue, vcsBackend, projectPath, config = {}, runLookup) {
     ...
     */
    async runAgent(entry, prState, worktreePath) {
        const logPath = this.ensureLogDir(entry.id);
     ...
     */
    buildAgentTaskPrompt(entry, prState, _worktreePath, targetBranch) {
     ...
     */
    async checkBuildOk(worktreePath) {
        try {
     ...
     */
    async checkTestsOk(worktreePath) {
        try {
     ...
     */
    async commitAndPush(worktreePath) {
        try {
            ...
            await this.vcsBackend.commit(worktreePath, "fix: refinery agent — auto-fix from merge queue");
            // Push to origin
     ...
     */
    async mergeBranch(entry, _targetBranch) {
        try {
            ...
            await execFileAsync("gh", ["pr", "merge", entry.branch_name, "--squash", "--admin"], { cwd: this.projectPath, maxBuffer: PIPELINE_BUFFERS.maxBufferBytes });
            return true;
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
     ...
     */
    async escalate(entry, reason) {
        try {
            this.logAction(entry.id, `Escalating: ${reason}`);
            // Get PR title and body
            const { stdout: prView } = await execFileAsync("gh", ["pr", "view", entry.branch_name, "--json", "title,body,url"], { cwd: this.projectPath, maxBuffer: PIPELINE_BUFFERS.maxBufferBytes });
...
- ESCALATE: <reason why it couldn't be fixed>
`;
//# sourceMappingURL=refinery-agent.js.map