...
export class PostgresMergeCostTracker {
    ...
    async getStats(period = "all") {
        ...
        const [totals] = await query(`SELECT
         ...
         COUNT(*) AS entryCount
       FROM merge_costs ${whereClause}`, params);
        const tierRows = await query(`SELECT
         tier,
         COUNT(*) AS count,
         COALESCE(SUM(actual_cost_usd), 0) AS totalCostUsd,
         COALESCE(SUM(input_tokens), 0) AS totalInputTokens,
       ...
       GROUP BY tier`, params);
        const modelRows = await query(`SELECT
         model,
         COUNT(*) AS count,
         COALESCE(SUM(actual_cost_usd), 0) AS totalCostUsd,
         COALESCE(SUM(input_tokens), 0) AS totalInputTokens,
    ...
    async getSessionSummary(sessionId) {
       ...
       FROM merge_costs
       WHERE project_id = $1 AND session_id = $2`, [this.projectId, sessionId]);
        return {
            sessionId,
    ...
    async getResolutionRate(days = 30) {
        ...
        const [row] = await query(`SELECT COUNT(*) AS total FROM merge_costs WHERE project_id = $1 AND recorded_at >= $2`, [this.projectId, cutoff.toISOString()]);
        const total = Number(row?.total ?? 0);
        return { successes: total, total, rate: total === 0 ? 0 : 100 };
    }
}
//# sourceMappingURL=postgres-merge-cost-tracker.js.map