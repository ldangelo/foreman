...
export class MergeValidator {
     ...
     */
    proseDetection(content, fileExtension) {
        if (content.length === 0) {
            return false;
        }
    ...
    async syntaxCheck(filePath, content) {
        ...
        const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "foreman-syntax-check-"));
        ...
        try {
            ...
            return await new Promise((resolve) => {
                const child = execFile(cmd, args, { timeout: 15_000 }, (error, _stdout, stderr) => {
                    if (error) {
                        resolve({
                            pass: false,
     ...
     */
    async validate(filePath, content, fileExtension) {
        // 1. Conflict markers
