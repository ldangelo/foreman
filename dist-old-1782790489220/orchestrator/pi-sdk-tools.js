...
export function createCloseBeadTool(projectPath) {
    return {
        ...
        async execute(_toolCallId, params) {
            ...
            if (!beadId) {
                return {
                    content: [{ type: "text", text: "Failed to close bead: missing beadId" }],
                    details: undefined,
                };
            }
            try {
                const brBin = process.env["BR_BIN"] ?? "br";
                const { stdout } = await execFileAsync(brBin, ["close", beadId, "--reason", params.reason], { cwd: projectPath });
                return {
                    content: [{ type: "text", text: `Bead ${beadId} closed: ${stdout.trim()}` }],
                    details: undefined,
                };
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                return {
                    content: [{ type: "text", text: `Failed to close bead ${beadId}: ${msg}` }],
                    details: undefined,
                };
            }
        },
