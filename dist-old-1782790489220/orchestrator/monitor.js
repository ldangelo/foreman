...
export class Monitor {
    ...
    projectPath;
    vcsBackend;
    constructor(store, taskClient, projectPath, vcsBackend) {
        this.store = store;
        ...
        for (const run of activeRuns) {
            ...
            try {
                ...
                if (issueStatus === "closed" || issueStatus === "completed") {
                    ...
                    await this.store.logEvent(run.project_id, "complete", { seedId: run.seed_id, detectedBy: "monitor" }, run.id);
                ...
                if (run.started_at) {
                    ...
                    if (elapsedMinutes > stuckTimeout) {
                        ...
                        await this.store.logEvent(run.project_id, "stuck", { seedId: run.seed_id, elapsedMinutes: Math.round(elapsedMinutes) }, run.id);
                ...
                await this.store.logEvent(run.project_id, "fail", { seedId: run.seed_id, error: message }, run.id);
    ...
    async detectHungSessions(opts) {
        ...
        for (const run of activeRuns) {
            ...
            if (staleMs > threshold) {
                ...
                await this.store.updateRun(run.id, { status: "stuck" });
                await this.store.logEvent(run.project_id, "stuck", {
                    seedId: run.seed_id,
    ...
    async recoverStuck(run, maxRetries = PIPELINE_LIMITS.maxRecoveryRetries) {
        ...
        if (retryCount >= maxRetries) {
            ...
            await this.store.logEvent(run.project_id, "fail", { seedId: run.seed_id, reason: `Max retries (${maxRetries}) exceeded` }, run.id);
            return false;
        }
        ...
        if (hasProgress && run.worktree_path) {
            ...
            await this.store.logEvent(run.project_id, "recover", {
                seedId: run.seed_id,
        ...
        try {
            ...
            await this.store.logEvent(run.project_id, "recover", { seedId: run.seed_id, attempt: retryCount + 1, maxRetries, worktreePreserved: false }, run.id);
            return true;
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            ...
            await this.store.logEvent(run.project_id, "fail", { seedId: run.seed_id, reason: `Recovery failed: ${message}` }, run.id);
            return false;
        }
    }
}
