...
export interface DispatcherStoreDeps {
    ...
    createRun(projectId: string, seedId: string, agentType: string, worktreePath?: string | null | undefined, opts?: {
    ...
    /** Update a run record. */
    updateRun(runId: string, updates: Partial<Pick<Run, "status" | "session_key" | "worktree_path" | "started_at" | "completed_at">>): Awaitable<void>;
    ...
    getRunsByStatusesSince(statuses: Run["status"][], since: string, projectId?: string): Awaitable<Run[]>;
    /** Log an event. */
    logEvent(projectId: string, eventType: EventType, details: Record<string, unknown> | string, runId?: string): Awaitable<void>;
    /** Send a message. */
    sendMessage(runId: string, senderAgentType: string, recipientAgentType: string, subject: string, body: string): Awaitable<unknown>;
    /** Update run progress. */
...
export interface DispatcherOverrides {
    ...
    /** Custom run queries. */
    getRunsByStatus?: (status: string, projectId: string) => Promise<{
        status: string;
    }[]>;
    getRunsForSeed?: (seedId: string, projectId: string) => Promise<{
        id: string;
    }[]>;
    ...
    getActiveRuns?: (projectId: string) => Promise<{
        id: string;
    }[]>;
...
//# sourceMappingURL=dispatcher-dependencies.d.ts.map