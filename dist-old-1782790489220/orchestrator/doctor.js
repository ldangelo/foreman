 ...
 */
function isSDKBasedRun(sessionKey) {
...
export class Doctor {
     ...
     */
    execFn;
    constructor(store, projectPath, mergeQueue, taskClient, execFn, mergeQueueLike, runLookup, backendType) {
        this.store = store;
    ...
    async reconcileMissingCompletedRuns(projectPath) {
        ...
        if (this.runLookup) {
            return await Promise.resolve(mergeQueue.reconcile(projectPath));
        }
    ...
    async checkGitTownMainBranch() {
        ...
        let defaultBranch;
        try {
            defaultBranch = await resolveDefaultBranch(this.projectPath, async (projectPath) => (await this.getVcsBackend()).detectDefaultBranch(projectPath), loadProjectConfig(this.projectPath));
        }
        catch {
            return {
                name: "git town main branch configured",
     ...
     */
    async checkOldLogs(thresholdDays = 7, warnThreshold = 10) {
        ...
        const totalRunIds = new Set(entries
    ...
    async checkOrphanedWorktrees(opts = {}) {
        const results = [];
        const { fix = false, dryRun = false } = opts;
        ...
        const foremanWorktrees = worktrees.filter((wt) => wt.branch && wt.branch.startsWith("foreman/"));
        ...
        for (const wt of foremanWorktrees) {
            ...
            const activeRun = runs.find((r) => ["pending", "running"].includes(r.status) && r.worktree_path === wt.path);
            const completedRun = runs.find((r) => r.status === "completed");
            const mergedRun = runs.find((r) => r.status === "merged");
            const prCreatedRun = runs.find((r) => r.status === "pr-created");
            const failableRun = runs.find((r) => ["failed", "stuck", "conflict", "test-failed"].includes(r.status));
            if (activeRun) {
                ...
                if (dryRun) {
                    ...
                    try {
                        ...
                        try {
                            await execFileAsync("git", ["worktree", "prune"], { cwd: this.projectPath });
                        }
                        catch { /* */ }
                        results.push({
                            name: `worktree: ${seedId}`,
    ...
    async checkStalePendingRuns(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const runStore = this.getRunStore();
        const project = await Promise.resolve(runStore.getProjectByPath(this.projectPath));
        if (!project) {
     ...
     */
    async isBranchMerged(seedId, defaultBranch) {
        const branchName = `foreman/${seedId}`;
        try {
            await this.execFn("git", ["merge-base", "--is-ancestor", branchName, defaultBranch], { cwd: this.projectPath });
    ...
    async checkFailedStuckRuns(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const runStore = this.getRunStore();
        const project = await Promise.resolve(runStore.getProjectByPath(this.projectPath));
        if (!project)
         ...
         * attention.
         */
        const filterAutoResolved = async (runs) => {
        ...
        const { unresolved: unresolvedFailed, autoResolvedCount: failedResolved } = await filterAutoResolved(failedRuns);
        const { unresolved: unresolvedStuck, autoResolvedCount: stuckResolved } = await filterAutoResolved(stuckRuns);
        ...
        const { actionable: actionableFailed, historical: historicalFailed } = await this.partitionByHistoricalRetry(unresolvedFailed, runStore, project.id);
        ...
        const hasAnyIssue = totalResolved > 0 ||
            agedTotal > 0 ||
     ...
     */
    async partitionByHistoricalRetry(runs, runStore = this.getRunStore(), projectId) {
        const actionable = [];
        const historical = [];
        for (const run of runs) {
            const allSeedRuns = await Promise.resolve(runStore.getRunsForSeed(run.seed_id, projectId));
            const hasLaterSuccess = allSeedRuns.some((r) => ["completed", "merged"].includes(r.status) &&
    ...
    async checkRunStateConsistency(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const runStore = this.getRunStore();
        const project = await Promise.resolve(runStore.getProjectByPath(this.projectPath));
        if (!project)
        ...
        const inconsistentRuns = activeRuns.filter((r) => r.completed_at !== null && r.status !== "merged" && r.status !== "pr-created");
     ...
     */
    async checkBeadStatusSync(opts = {}) {
        const { fix = false, dryRun = false } = opts;
    ...
    async checkBrRecoveryArtifacts(opts = {}) {
        const { fix = false, dryRun = false } = opts;
     ...
     */
    async checkStaleMergeQueueEntries(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const mergeQueue = this.getMergeQueueLike();
        if (!mergeQueue) {
        ...
        const staleEntries = allEntries.filter((e) => {
            if (e.status !== "pending" && e.status !== "merging")
                return false;
     ...
     */
    async checkDuplicateMergeQueueEntries(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const mergeQueue = this.getMergeQueueLike();
        if (!mergeQueue) {
        ...
        const duplicates = Array.from(branchCounts.entries()).filter(([, entries]) => entries.length > 1);
     ...
     */
    async checkOrphanedMergeQueueEntries(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const mergeQueue = this.getMergeQueueLike();
        if (!mergeQueue) {
     ...
     */
    async checkResolvedMergeQueueEntries(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const mergeQueue = this.getMergeQueueLike();
        if (!mergeQueue) {
     ...
     */
    async checkStuckConflictFailedEntries(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        const mergeQueue = this.getMergeQueueLike();
        if (!mergeQueue) {
     ...
     */
    async checkMergeQueueHealth(opts = {}) {
     ...
     */
    async checkOrphanedGlobalStoreRuns(opts = {}) {
        const { fix = false, dryRun = false } = opts;
        ...
        try {
            ...
            for (const project of projects) {
                ...
                const globalRuns = globalDb
                    .prepare("SELECT * FROM runs WHERE project_id = ? AND status IN ('completed', 'pr-created') ORDER BY created_at ASC")
                    .all(project.id);
                ...
                try {
                    ...
                    const existingIds = new Set(localDb.prepare("SELECT id FROM runs").all().map((r) => r.id));
            ...
            for (const { run, projectPath, projectName, projectId } of orphaned) {
                ...
                try {
                    ...
                    if (!localProject) {
                        ...
                        localDb
                            .prepare(`INSERT OR IGNORE INTO projects (id, name, path, status, created_at, updated_at)
                 VALUES (?, ?, ?, 'active', ?, ?)`)
                            .run(projectId, projectName, projectPath, new Date().toISOString(), new Date().toISOString());
                    }
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                        .run(run.id, effectiveProjectId, run.seed_id, run.agent_type, run.session_key, run.worktree_path, run.status, run.started_at, run.completed_at, run.created_at, run.base_branch ?? null, run.tmux_session ?? null, run.progress);
                    ...
                    for (const cost of globalCosts) {
                        localDb
                            .prepare(`INSERT OR IGNORE INTO costs
                   (id, run_id, tokens_in, tokens_out, cache_read, estimated_cost, recorded_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`)
                            .run(cost.id, cost.run_id, cost.tokens_in, cost.tokens_out, cost.cache_read, cost.estimated_cost, cost.recorded_at);
                    }
                 ...
                 VALUES (?, ?, ?, ?, ?, ?)`)
                            .run(event.id, effectiveProjectId, event.run_id, event.event_type, event.details, event.created_at);
                    }
                    migratedCount++;
                }
                catch (err) {
    ...
    async checkDataIntegrity(opts = {}) {
        const results = [];
        const [worktreeResults, zombieResults, staleResult, failedStuckResults, consistencyResults, blockedResult, recoveryResult, orphanedGlobalStoreResult, beadSyncResult] = await Promise.all([
