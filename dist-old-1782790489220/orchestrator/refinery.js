...
const TERMINAL_TASK_STATUS_BY_REFINERY_RUN_STATUS = {
    failed: "failed",
    "test-failed": "failed",
    stuck: "stuck",
    conflict: "conflict",
...
export class Refinery {
    ...
    registeredProjectId;
    postgresAdapter;
    constructor(store, seeds, projectPath, vcsBackend, opts = {}) {
        this.store = store;
        this.seeds = seeds;
    ...
    async lookupRunsByStatus(status, projectId) {
    ...
    async lookupRunsByStatuses(statuses, projectId) {
        return projectId === undefined
    ...
    async lookupRunsByBaseBranch(baseBranch, projectId) {
        return projectId === undefined
    ...
    async persistRunUpdate(run, updates) {
        const nextStatus = updates.status;
        const shouldPreserveTerminalSuccess = (currentStatus) => currentStatus !== undefined
    ...
    async persistRunEvent(run, eventType, data) {
        if (this.registeredProjectId && this.postgresAdapter) {
            try {
     ...
     */
    async autoResolveRebaseConflicts(targetBranch, workspacePath = this.projectPath) {
     ...
     */
    async autoCommitStateFiles(workspacePath = this.projectPath) {
        try {
     ...
     */
    async removeReportFiles(workspacePath = this.projectPath) {
        const resolver = workspacePath === this.projectPath
            ? this.conflictResolver
            : new ConflictResolver(workspacePath, DEFAULT_MERGE_CONFIG, this.vcsBackend);
     ...
     */
    async archiveReportsPostMerge(seedId, workspacePath = this.projectPath) {
        const resolver = workspacePath === this.projectPath
            ? this.conflictResolver
            : new ConflictResolver(workspacePath, DEFAULT_MERGE_CONFIG, this.vcsBackend);
     ...
     */
    async runBeforeRemoveHook(workspacePath, seedId) {
        try {
    ...
    async getExistingPrUrl(branchName) {
        try {
    ...
    async getExistingPrState(branchName) {
        try {
    ...
    async finalizeSuccessfulMerge(run, branchName, targetBranch) {
        ...
        await this.persistRunEvent(run, "merge", { seedId: run.seed_id, branchName, targetBranch });
    ...
    async ensurePullRequestForRun(opts) {
        ...
        if (opts.existingOk !== false && !this.isTestRuntime()) {
            ...
            if (existingPr?.headRefName === branchName && existingPr.url) {
                ...
                if (existingPr.state === "OPEN") {
                    ...
                    await this.persistRunEvent(run, "pr-created", {
                        seedId: run.seed_id,
                        branchName,
                        baseBranch,
                    ...
                    const prState = opts.draft ?? false ? "draft" : "open";
                    await this.persistRunUpdate(run, {
        ...
        await this.persistRunEvent(run, "pr-created", { seedId: run.seed_id, branchName, baseBranch, prUrl, draft: opts.draft ?? false, existing: false });
     ...
     */
    async #linkPrToGithubIssue(runId, prUrl) {
        if (!this.registeredProjectId || !this.postgresAdapter)
            return;
        try {
    ...
    async mergePullRequest(opts) {
        ...
        try {
            ...
            await this.persistRunEvent(run, "fail", { seedId: run.seed_id, branchName, error: message });
     ...
     */
    async closeNativeTaskPostMerge(runId, _seedId) {
        try {
            if (this.registeredProjectId && this.postgresAdapter) {
                const [task] = await this.postgresAdapter.listTasks(this.registeredProjectId, {
                    runId,
        ...
        try {
            ...
            for (const stackedRun of stackedRuns) {
                ...
                if (!branchExists)
                    continue;
                try {
                    const restackResult = await this.vcsBackend.restackBranch(this.projectPath, stackedBranch, mergedBranch, targetBranch);
                    if (!restackResult.success) {
                        throw new Error(restackResult.conflictingFiles?.length
    ...
    async createPrForConflict(run, branchName, baseBranch, conflictNote) {
        try {
            ...
            await this.persistRunUpdate(run, { status: "pr-created" });
            await this.persistRunEvent(run, "pr-created", { seedId: run.seed_id, branchName, baseBranch, prUrl, conflictNote });
            return { runId: run.id, seedId: run.seed_id, branchName, prUrl };
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            await this.persistRunUpdate(run, { status: "conflict" });
            await this.persistRunEvent(run, "fail", { seedId: run.seed_id, branchName, error: `PR creation failed: ${message}` });
            return null;
        }
    }
    /**
     ...
     */
    async getCompletedRuns(projectId, seedId) {
        if (seedId) {
    ...
    async mergeCompleted(opts) {
        const defaultTargetBranch = normalizeBranchLabel(opts?.targetBranch ?? await this.vcsBackend.detectDefaultBranch(this.projectPath)) ?? "dev";
                ...
                try {
                    ...
                    // we don't fall back to raw git rebase semantics in colocated jj repos.
                    {
                        ...
                        try {
                            if (this.vcsBackend.name === "jujutsu") {
                                ...
                                try {
                                    ...
                                    if (!rebaseResult.success) {
                                        throw new Error(rebaseResult.conflictingFiles?.length
                            ...
                            if (!useIntegrationWorktree) {
                                ...
                                try {
                                    await gitSpecial(["checkout", "--detach", mergeTargetRef], mergeWorkspacePath);
                                }
                                catch { /* best effort */ }
                                try {
                                    await this.vcsBackend.resetHard(mergeWorkspacePath, mergeTargetRef);
                                }
                                catch { /* best effort */ }
                            }
                            if (stashedBeforeRebase) {
                                try {
                                    await this.vcsBackend.restoreWorktreeState(mergeWorkspacePath);
                                }
                                catch { /* best effort — may be empty */ }
                            }
                        }
                        if (!rebaseOk) {
                            await this.addFailureNote(run.seed_id, `Merge failed: conflict on ${new Date().toISOString().slice(0, 10)} — manual retry required. Rebase conflicts detected.`);
                            this.sendMail(run.id, "merge-failed", {
                                seedId: run.seed_id,
                                branchName,
                    ...
                    try {
                        ...
                        const mergeResult = useIntegrationWorktree
                            ? await gitSpecial(["merge", branchName, "--no-commit", "--no-ff"], mergeWorkspacePath).then(() => ({ success: true })).catch(async (err) => {
                                const message = err instanceof Error ? err.message : String(err);
                                if (message.includes("CONFLICT") || message.includes("Merge conflict") || message.includes("Automatic merge failed")) {
                                    ...
                                    return { success: false, conflicts: conflicts.split("\n").map((f) => f.trim()).filter(Boolean) };
                                }
                                throw err;
                            })
                        ...
                        if (mergeMsg.includes("CONFLICT") || mergeMsg.includes("Merge conflict") || conflictingFiles.length > 0) {
                            ...
                            if (codeConflicts.length > 0) {
                                ...
                                try {
                                    ...
                                    try {
                                        await this.vcsBackend.resetHard(mergeWorkspacePath, "HEAD");
                                    }
                                    catch { /* best effort */ }
                                }
                                await this.addFailureNote(run.seed_id, `Merge failed: conflict on ${new Date().toISOString().slice(0, 10)} — manual retry required. Conflicting files: ${codeConflicts.join(", ")}`);
                                this.sendMail(run.id, "merge-failed", {
                                    seedId: run.seed_id,
                                    branchName,
                    ...
                    if (runTests) {
                        ...
                        if (!testResult.ok) {
                            ...
                            await this.addFailureNote(run.seed_id, `Merge failed: post-merge tests failed on ${new Date().toISOString().slice(0, 10)} — manual retry required. ${testResult.output.slice(0, 300)}`);
     ...
     */
    async #restoreOriginalProjectBranch(originalBranch) {
        if (!originalBranch)
            return;
        try {
    ...
    async resolveConflict(runId, strategy, opts) {
        ...
        try {
            const mergeResult = await this.vcsBackend.mergeWithStrategy(this.projectPath, branchName, targetBranch, "theirs");
            if (!mergeResult.success) {
                throw new Error(mergeResult.conflicts?.length
            ...
            await this.persistRunEvent(run, "fail", { seedId: run.seed_id, error: message });
        ...
        await this.persistRunEvent(run, "merge", { seedId: run.seed_id, branchName, strategy: "theirs", targetBranch });
    ...
    async createPRs(opts) {
        ...
        for (const run of completedRuns) {
            ...
            try {
                ...
                await this.persistRunUpdate(run, { status: "pr-created" });
                await this.persistRunEvent(run, "pr-created", { seedId: run.seed_id, branchName, baseBranch, prUrl, draft });
                created.push({
                    runId: run.id,
                    seedId: run.seed_id,
                    branchName,
 ...
 */
export async function preserveBeadChanges(projectPath, branchName, targetBranch, vcsBackend) {
