...
import { buildPhasePrompt, parseVerdict, extractIssues, parseFinalizeFailureScope, parseFinalizeIntegrationStatus, parseFinalizeValidationStatus, qaReportHasTestEvidence, } from "./roles.js";
...
import { createPhaseRecord, finalizePhaseRecord, generateActivityLog, writeIncrementalPipelineReport } from "./activity-logger.js";
...
async function phaseAgentError(agentMailClient, phaseName, seedId, sinceIso) {
    if (!agentMailClient)
        return null;
    ...
    const msg = [...messages].reverse().find((candidate) => candidate.subject === "agent-error" &&
...
function readRelativeFile(worktreePath, relativePath) {
    if (!relativePath)
        return null;
...
function sendTraceMail(ctx, client, phaseName, seedId, worktreePath, result) {
    const traceMarkdown = readRelativeFile(worktreePath, result.traceMarkdownFile);
    if (!traceMarkdown)
        return;
    ctx.sendMailText(client, "foreman", `${phaseName.charAt(0).toUpperCase() + phaseName.slice(1)} Trace`, traceMarkdown);
}
/**
...
export function isMaxTurnsExceededError(error) {
...
export function shouldUseCooldownRetry(error, phase) {
 ...
 */
function getHaikuFallbackModel(model) {
...
function findDebugArtifacts(root, max = 20) {
    try {
...
async function writeNormalPhaseProgress(store, runId, progress, observabilityWriter) {
...
async function writeNormalPhaseEvent(store, projectId, runId, eventType, data, observabilityWriter) {
...
function isGeneratedWorkflowArtifact(filePath) {
    const name = basename(filePath);
    return (name.endsWith("_REPORT.md") ||
...
function toSandboxProviderConfig(config) {
    return {
...
export function applyEffectiveSandboxConfig(ctx) {
 ...
 */
export async function runBashPhase(bashCommand, taskMeta, cwd, artifactFile, timeoutMs = BASH_PHASE_TIMEOUT_MS, sandboxConfig) {
...
function execFilePromise(command, args, options) {
    ...
    const execPromise = new Promise((resolve, reject) => {
        const proc = execFile(command, args, restOptions, (error, stdout, stderr) => {
            ...
            if (error && !('code' in error)) {
                ...
                const status = typeof rawCode === 'number' ? rawCode : null;
    ...
    const timeoutPromise = new Promise((_, reject) => setTimeout(() => {
        reject(Object.assign(new Error(`ETIMEDOUT: ${command} timed out after ${timeoutMs}ms`), { code: 'ETIMEDOUT' }));
    }, timeoutMs));
 ...
 */
export function parseCompletedTaskIds(gitLogOutput) {
 ...
 */
function detectCompletedTasks(worktreePath) {
    try {
 ...
 */
async function executeEpicPipeline(ctx) {
    ...
    const allPhaseRecords = [];
    const allRetryCounts = {};
    let totalProgress = {
        toolCalls: 0,
        toolBreakdown: {},
        filesChanged: [],
    ...
    // ── Outer task loop ──────────────────────────────────────────────────
    const activeBugBeadIds = new Map();
    for (let taskIdx = 0; taskIdx < epicTasks.length; taskIdx++) {
        ...
        if (result.success) {
            ...
            if (workflowConfig.onError === "stop") {
                ...
                await ctx.markStuck(store, runId, config.projectId, seedId, config.seedTitle, totalProgress, "epic-task-failed", `Task ${task.seedId} failed — epic halted (onError=stop)`, config.projectPath, ctx.notifyClient);
                return;
            }
 ...
 */
async function executeSingleTaskPipeline(ctx) {
    const { config, workflowConfig, store, logFile } = ctx;
    const { seedId } = config;
    const progress = {
        toolCalls: 0,
 ...
 */
async function runPhaseSequence(ctx, phases, initialProgress, 
/** When true (epic task mode), exhausted retries return failure instead of continuing. */
failOnRetriesExhausted = false, observabilityWriter) {
    ...
    const explorerFailures = [];
    // P1/P2: Rate limit tracking per phase
    const rateLimitRetries = {};
    const pipelineStartedAt = Date.now();
    const totalReviewLoops = () => Object.values(retryCounts).reduce((sum, count) => sum + count, 0);
    ...
    const writeTaskPhaseNote = async (phaseName, kind, body, metadata) => {
    ...
    let i = 0;
    const retryOnlyActivations = new Set();
    while (i < phases.length) {
        ...
        if (phase.skipIfArtifact) {
            const interpolatedSkip = interpolateTaskPlaceholders(phase.skipIfArtifact, phaseMeta);
        ...
        if (phaseName === "explorer") {
            const recentExplorerFailures = explorerFailures.filter((t) => Date.now() - new Date(t).getTime() < 60 * 60 * 1000);
        ...
        if (phase.tools?.allowed) {
            phaseConfig.allowedTools = phase.tools.allowed;
        }
        if (phase.builtin) {
            ...
            if (ctx.activityPhases) {
                const completedActivityPhase = finalizePhaseRecord({ ...activityPhase, artifactPresent }, {
                    success: phaseSucceeded,
                    costUsd: 0,
                    turns: 0,
                    tokensIn: 0,
        ...
        if (phase.bash) {
            ...
            const bashResult = await runBashPhase(phase.bash, ctx.taskMeta, worktreePath, phase.artifact, phase.timeoutSecs !== undefined ? phase.timeoutSecs * 1000 : undefined, ctx.workflowConfig.sandbox);
        ...
        const result = await ctx.runPhase(phaseName, prompt, phaseConfig, progress, logFile, store, notifyClient, agentMailClient, observabilityInput, observabilityWriter);
        // 6. Release files
        if (phase.files?.reserve) {
        ...
        const commandPhaseContractReasons = [];
            ...
            if (isRateLimit) {
                ...
                if (currentRetryCount < RATE_LIMIT_BACKOFF_CONFIG.maxRetries) {
                    ...
                    if (phaseModel.includes("haiku")) {
                        ...
                        const fallbackResult = await ctx.runPhase(phaseName, prompt, fallbackPhaseConfig, progress, logFile, store, notifyClient, agentMailClient, observabilityInput, observabilityWriter);
                        ...
                        if (fallbackResult.success) {
                            ...
                            await ctx.onTaskPhaseChange?.(config.taskId ?? null, phaseName);
                            if (phase.mail?.forwardArtifactTo && phase.artifact) {
                                const interpolatedArtifact = interpolateTaskPlaceholders(phase.artifact, ctx.taskMeta ?? { id: '', title: '', description: '', type: '', priority: 2 });
        ...
        if (phase.verdict && phase.artifact) {
            ...
            if (phaseName === "qa") {
                ...
                if (verdict === "pass" && config.vcsBackend) {
                    const detectDefaultBranch = config.vcsBackend.detectDefaultBranch;
...
// ── Session log helper ──────────────────────────────────────────────────────
async function writeSessionLogSafe(ctx, progress, phaseRecords, retryCounts, qaVerdictForLog) {
    const { config } = ctx;
