...
export class ConflictResolver {
    ...
    patternLearning;
    sessionCostUsd = 0;
    constructor(projectPath, config, vcs) {
    ...
    async gitTry(args) {
        try {
            ...
            const { stdout, stderr } = await execAsync("git", args, { cwd: this.projectPath, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
    ...
    async gitMergeBase(ref1, ref2) {
        ...
        try {
            ...
            const { promisify: p } = await import("node:util");
            const execAsync = p(exec);
            const { stdout } = await execAsync("git", ["merge-base", ref1, ref2], { cwd: this.projectPath, encoding: "utf-8" });
            return stdout.trim();
        }
        catch {
            return "";
    ...
    async gitDiff(from, to) {
        ...
        try {
            ...
            const { promisify: p } = await import("node:util");
            const execAsync = p(exec);
            const { stdout } = await execAsync("git", ["diff", `${from}..${to}`, "--"], { cwd: this.projectPath, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
            return stdout;
        }
        catch {
            return "";
    ...
    async gitCheckoutBranch(branch) {
        if (this.vcs) {
    ...
    async gitMergeNoCommit(source, targetBranch) {
        if (this.vcs) {
    ...
    async gitCheckoutTheirs(filePath) {
        if (this.vcs) {
    ...
    async gitStageFileForce(filePath) {
        if (this.vcs) {
            await this.vcs.stageFile(this.projectPath, filePath);
            return;
        }
    ...
    async gitRemoveFromIndex(filePath) {
        if (this.vcs) {
    ...
    async getConflictingFilesFromDisk() {
        if (this.vcs) {
    ...
    async getAddedFiles(targetBranch, branchName) {
        if (this.vcs) {
        ...
        try {
            ...
            const { promisify: p } = await import("node:util");
            const execAsync = p(exec);
            const { stdout } = await execAsync("git", ["diff", "--name-only", "--diff-filter=A", `${targetBranch}...${branchName}`], { cwd: this.projectPath, encoding: "utf-8" });
            return stdout.split("\n").map((f) => f.trim()).filter(Boolean);
        }
        catch {
            return [];
    ...
    async getUntrackedFilesFromDisk() {
        if (this.vcs) {
        ...
        try {
            ...
            const { promisify: p } = await import("node:util");
            const execAsync = p(exec);
            const { stdout } = await execAsync("git", ["ls-files", "--others", "--exclude-standard"], { cwd: this.projectPath, encoding: "utf-8" });
            return stdout.split("\n").map((f) => f.trim()).filter(Boolean);
        }
        catch {
            return [];
     ...
     */
    async checkUntrackedConflicts(branchName, targetBranch, mode = "delete") {
        ...
        if (mode === "stash") {
            ...
            const stashDir = path.join(this.projectPath, ".foreman", "stashed", String(timestamp));
     ...
     */
    async attemptMerge(branchName, targetBranch) {
     ...
     */
    async attemptTier2Resolution(filePath, branchName, targetBranch) {
        ...
        const targetUniqueLines = targetContent
            .split("\n")
            .filter((line) => line.trim() !== "" && !baseLines.has(line) && !branchLines.has(line));
        ...
        const discardedPercent = targetLines > 0 ? (discardedLines / targetLines) * 100 : 0;
        const { maxDiscardedLines, maxDiscardedPercent } = this.config.tier2SafetyCheck;
        if (discardedLines > maxDiscardedLines ||
     ...
     */
    async attemptTier3Resolution(filePath, fileContent) {
        ...
        // ── Budget check (MQ-012) ──
        const remainingBudget = this.config.costControls.maxSessionBudgetUsd - this.sessionCostUsd;
     ...
     * @param targetBranch - The target branch (e.g. "main")
     */
    async attemptTier4Resolution(filePath, branchName, targetBranch) {
     ...
     */
    async resolveConflicts(branchName, targetBranch) {
        const resolvedTiers = new Map();
        const fallbackFiles = [];
        const costs = [];
        ...
        for (const filePath of mergeResult.conflictedFiles) {
            ...
            // Tier 2
            const tier2 = await this.attemptTier2Resolution(filePath, branchName, targetBranch);
            if (tier2.success) {
            ...
            if (!skipTier3) {
                ...
                const tier3 = await this.attemptTier3Resolution(filePath, conflictedContent);
                if (tier3.cost)
                    costs.push(tier3.cost);
            ...
            if (!skipTier4) {
                const tier4 = await this.attemptTier4Resolution(filePath, branchName, targetBranch);
                if (tier4.cost)
                    costs.push(tier4.cost);
     ...
     */
    async readConflictedFile(filePath) {
        const fullPath = path.join(this.projectPath, filePath);
        try {
     ...
     */
    async writeResolvedFile(filePath, content) {
        const fullPath = path.join(this.projectPath, filePath);
     ...
     */
    async runPostMergeTests(resolvedTiers, testCommand = "npm test", noTests = false, beforeRef = "HEAD~1") {
        // Skip if --no-tests
        if (noTests) {
            return {
                passed: true,
        ...
        const usedAI = Array.from(resolvedTiers.values()).some((tier) => tier >= 3);
        if (!usedAI) {
            return {
                passed: true,
                skipped: true,
     ...
     */
    async handleFallback(branchName, targetBranch, fallbackFiles, resolvedTiers) {
        ...
        const fileDetails = fallbackFiles
            ...
            .join("\n");
        const resolvedDetails = resolvedTiers.size > 0
     ...
     */
    static isReportFile(f) {
        if (REPORT_FILES.includes(f))
            return true;
        if (f.startsWith(".foreman/reports/"))
            return true;
        if (f.endsWith(".md") && REPORT_FILES.some((r) => f.startsWith(r.replace(".md", "."))))
            return true;
        if (f === ".claude/settings.local.json")
            return true;
        // Beads data files are auto-resolvable: take the branch version (latest bead state)
        if (f === ".beads/issues.jsonl" || f.startsWith(".beads/"))
            return true;
        return false;
    }
    /**
     ...
     */
    async autoResolveRebaseConflicts(_targetBranch) {
        ...
        for (let i = 0; i < MAX_ITERATIONS; i++) {
            ...
            for (const f of conflictFiles) {
                ...
                await this.gitCheckoutTheirs(f).catch(() => {
                    ...
                    try {
                        unlinkSync(path.join(this.projectPath, f));
                    }
                    catch { /* gone */ }
                });
