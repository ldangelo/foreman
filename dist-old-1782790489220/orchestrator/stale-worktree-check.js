 ...
 */
// ── Main function ─────────────────────────────────────────────────────────
async function logStaleWorktreeEvent(store, projectId, runId, eventType, payload, eventWriter) {
    try {
        if (eventWriter) {
 ...
 */
export async function checkAndRebaseStaleWorktree(vcs, worktreePath, targetBranch, store, projectId, runId, seedId, opts) {
    const autoRebase = opts?.autoRebase ?? true;
    const failOnConflict = opts?.failOnConflict ?? true;
    let conflictRebaseErrorMessage;
    ...
    if (!autoRebase) {
        console.warn(`[StaleWorktreeCheck] Worktree for ${seedId} is stale (behind origin/${targetBranch}) but auto-rebase is disabled`);
        return {
            rebased: false,
    ...
    console.log(`[StaleWorktreeCheck] Worktree for ${seedId} is stale — auto-rebasing onto origin/${targetBranch}`);
    try {
 ...
 */
export async function detectStaleWorktree(vcs, worktreePath, targetBranch) {
    let localHead;
    try {
        localHead = await vcs.getHeadId(worktreePath);
    }
 ...
 */
export async function hasUncommittedChanges(vcs, worktreePath) {
    try {
 ...
 */
export async function getWorktreeStatusSummary(vcs, worktreePath, targetBranch) {
    try {
...
//# sourceMappingURL=stale-worktree-check.js.map