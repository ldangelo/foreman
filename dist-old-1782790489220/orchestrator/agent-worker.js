...
import { ROLE_CONFIGS, getDisallowedTools, } from "./roles.js";
...
async function resolveRegisteredProjectIdForPath(projectPath, databaseUrl) {
    if (!databaseUrl)
        return undefined;
 ...
 */
function sendMail(client, to, subject, body) {
    if (!client)
        return;
    client.sendMessage(to, subject, JSON.stringify(body)).catch((err) => {
 ...
 * Never throws.
 */
function sendMailText(client, to, subject, body) {
    if (!client)
        return;
    client.sendMessage(to, subject, body).catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        log(`[agent-mail] send failed (non-fatal): ${msg}`);
    });
}
function compactTraceValue(value, maxLength = 160) {
    let compact = value
...
function sendTraceMail(client, event) {
 ...
 * Never throws — failures are logged but do not affect the pipeline.
 */
async function registerAgent(client, roleHint) {
    if (!client)
        return;
    try {
 ...
 * Never throws — failures are logged but do not affect the pipeline.
 */
function reserveFiles(client, paths, agentName, leaseSecs) {
    if (!client || paths.length === 0)
        return;
    client.reserveFiles(paths, agentName, leaseSecs).catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
 ...
 * Never throws — failures are logged but do not affect the pipeline.
 */
function releaseFiles(client, paths, agentName) {
    if (!client || paths.length === 0)
        return;
    client.releaseFiles(paths, agentName).catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        log(`[agent-mail] releaseFiles failed (non-fatal): ${msg}`);
    });
}
async function createRuntimeTaskClient(projectPath, registeredProjectId) {
 ...
 */
function initLogContext(config) {
    logContext = {
...
// ── Main ─────────────────────────────────────────────────────────────────────
function installTestWorkerGuard(config) {
    ...
    if (config.env.FOREMAN_WORKER_TEST_GUARD !== "1" && !isTempTestHome)
        return;
...
async function runAfterRunHook(config) {
...
async function main() {
    ...
    let progressFlushTail = Promise.resolve();
 ...
 */
async function runPhase(role, prompt, config, progress, logFile, store, notifyClient, agentMailClient, observability, observabilityWriter) {
    const baseRoleConfig = ROLE_CONFIGS[role] ?? ROLE_CONFIGS.developer;
    const roleConfig = config.allowedTools
        ? { ...baseRoleConfig, role: role, allowedTools: config.allowedTools }
        : baseRoleConfig;
    ...
    const resolvedModel = config.model || roleConfig.model;
 ...
 */
async function runTroubleshooterPhase(config, workflowConfig, store, logFile, notifyClient, agentMailClient, failureContext, pipelineProjectPath) {
 ...
 */
function deriveFallbackRefineryOptions(registeredProjectId, registeredReadStore, pipelineProjectPath, configProjectId, log) {
...
function workerReportDir(config) {
    return config.taskMeta?.projectReportsDir || getRunReportsDir(config.projectId, config.seedId, config.runId);
}
async function hasChangesAgainstBase(vcsBackend, repoPath, baseBranch, branchName) {
    if (!vcsBackend)
        return true;
...
async function runCreatePrBuiltinPhase(args) {
    ...
    const registeredRefineryOptions = deriveFallbackRefineryOptions(registeredProjectId, registeredReadStore, pipelineProjectPath, config.projectId, log);
    ...
    if (!branchHasChanges) {
        ...
        await runtimeTaskClient.close(config.seedId, "No changes against target branch; acceptance already satisfied.").catch((err) => {
...
function readPrNumberFromMetadata(worktreePath, reportDir) {
    ...
    if (!prNumber)
        throw new Error("PR metadata missing prNumber");
    return prNumber;
}
...
async function runShellForFinalize(command, cwd, timeoutMs = 120_000) {
    try {
...
function truncateFinalizeOutput(output) {
    return output.length > 3000 ? `${output.slice(0, 3000)}\n...<truncated>` : output;
}
function isVerificationTask(config) {
...
async function runFinalizeBuiltinPhase(args) {
    ...
    for (const candidate of [`origin/${baseBranch}`, baseBranch]) {
        try {
            currentTargetRef = await vcsBackend.resolveRef(config.worktreePath, candidate);
            break;
        }
        catch { /* try next */ }
    }
    ...
    if (shouldValidate) {
        let rebaseError;
        const rebase = await vcsBackend.rebase(config.worktreePath, `origin/${baseBranch}`).catch((err) => {
...
async function validatePrReviewGate(args) {
    ...
    while (true) {
        ...
        readySince = stability.readySince;
        if (waitStatus.mergeConflict)
            break;
        if (stability.stable)
            break;
        if (Date.now() - startedAt >= MERGE_GATE_TIMEOUT_MS)
            break;
        const stableFor = readySince ? Date.now() - readySince : 0;
        args.log(`[PR-REVIEW] Final gate waiting for PR #${prNumber}: checksTerminal=${String(waitStatus.checksTerminal)} ` +
    ...
    args.log(`[PR-REVIEW] Final gate for PR #${prNumber}: checksTerminal=${String(waitStatus.checksTerminal)} ` +
        `codeRabbitSeen=${String(waitStatus.codeRabbitSeen)} codeRabbitComplete=${String(waitStatus.codeRabbitComplete)} mergeConflict=${String(waitStatus.mergeConflict)} ` +
        `blocking=${reviewContext.blockingFindings.length} failedChecks=${reviewContext.failedChecks.length}`);
    if (waitStatus.mergeConflict)
        return { success: false, reason: `pr_review_merge_conflict: ${waitStatus.mergeConflictReason ?? "unknown"}` };
    if (!waitStatus.checksTerminal)
        return { success: false, reason: `pr_review_checks_pending: ${waitStatus.pendingChecks.join(", ") || "unknown"}` };
    if (!waitStatus.codeRabbitComplete)
        return { success: false, reason: waitStatus.codeRabbitSeen ? "pr_review_coderabbit_not_complete" : "pr_review_coderabbit_not_observed" };
    if (reviewContext.failedChecks.length > 0)
        return { success: false, reason: `pr_review_failed_checks: ${reviewContext.failedChecks.map((check) => check.name).join(", ")}` };
    if (reviewContext.blockingFindings.length > 0)
        return { success: false, reason: `pr_review_blocking_findings: ${reviewContext.blockingFindings.length}` };
    return { success: true };
}
...
async function runMergeBuiltinPhase(args) {
    ...
    const prNumber = (() => {
        try {
            return readPrNumberFromMetadata(config.worktreePath, workerReportDir(config));
        }
        catch {
            return undefined;
        }
...
async function runPipeline(config, store, localStore, logFile, notifyClient, agentMailClient, registeredReadStore, registeredProjectId) {
    ...
    const resolvedWorkflow = config.workflowName ?? resolveWorkflowName(config.seedType ?? "feature", config.seedLabels, projectTaskTypeWorkflowMap);
    ...
    const { taskClient: runtimeTaskClient, backendType: runtimeTaskBackend } = await createTaskClient(pipelineProjectPath, {
        registeredProjectId,
    });
    const registeredObservabilityWriter = registeredReadStore
        ? {
    ...
    let vcsBackend;
    try {
        vcsBackend = await VcsBackendFactory.fromEnv(pipelineProjectPath, process.env.FOREMAN_VCS_BACKEND);
    ...
    await executePipeline({
        ...
        async onTaskPhaseChange(taskId, phaseName) {
            if (runtimeTaskBackend !== "native" || !taskId)
                return;
        ...
        async onTaskPhaseNote(taskId, phaseName, kind, body, metadata) {
            if (runtimeTaskBackend !== "native" || !taskId || !registeredProjectId)
                return;
            try {
        ...
        runPhase,
        async runBuiltinPhase(phase, progress) {
            try {
        ...
        reserveFiles,
        releaseFiles,
        markStuck: async (storeArg, runIdArg, projectIdArg, seedIdArg, seedTitleArg, progressArg, phaseArg, reasonArg, projectPathArg, notifyClientArg) => markStuck(storeArg, localStore, runIdArg, projectIdArg, seedIdArg, seedTitleArg, progressArg, phaseArg, reasonArg, projectPathArg, notifyClientArg, registeredReadStore),
        log,
        ...
        async onPipelineComplete({ progress, success }) {
            ...
            if (agentMailClient) {
                ...
                const finalizeMsgs = foremanMsgs.filter((m) => (m.subject === "phase-complete" || m.subject === "agent-error") &&
                    (m.from === finalizeSender || m.from === "finalize"));
                const nonRetryableError = finalizeMsgs.find((m) => {
                    if (m.subject !== "agent-error")
                        return false;
                    try {
                ...
                if (nonRetryableError) {
                    const body = (() => { try {
                        return JSON.parse(nonRetryableError.body ?? "{}");
                    }
                    catch {
                        return {};
                    } })();
                    ...
                    const body = (() => { try {
                        return JSON.parse(finalizePhaseComplete.body ?? "{}");
                    }
                    catch {
                        return {};
                    } })();
                    ...
                    if (finalizeAgentError?.subject === "agent-error") {
                        const body = (() => { try {
                            return JSON.parse(finalizeAgentError.body ?? "{}");
                        }
                        catch {
                            return {};
                        } })();
            ...
            const shouldSkipTroubleshooter = !finalizeSucceeded &&
                !finalizeRetryable &&
            ...
            if (troubleshooterEnabled) {
                ...
                troubleshooterResolved = await runTroubleshooterPhase(config, workflowConfig, store, logFile, notifyClient, agentMailClient, failureContext, pipelineProjectPath);
                if (troubleshooterResolved) {
                    ...
                    try {
                        ...
                        if (enqueueResult.success) {
                            ...
                            try {
                                ...
                                log(`[FINALIZE] Immediate merge drain result: merged=${mergeResult.merged}, conflicts=${mergeResult.conflicts}, failed=${mergeResult.failed}`);
                            }
                            catch (drainErr) {
            ...
            const writeFinalizeTerminalEvent = async (eventType, data) => {
                if (registeredProjectId && registeredReadStore) {
                    try {
            ...
            if (finalizeSucceeded) {
                ...
                const continuationCheck = async (checkRunId, checkSeedId, checkProjectPath, checkRegisteredProjectId) => {
                    try {
...
async function markStuck(store, localStore, runId, projectId, seedId, seedTitle, progress, phase, reason, projectPath, notifyClient, registeredReadStore) {
    const reasonLower = reason.toLowerCase();
    const isRateLimit = reasonLower.includes("hit your limit") || reasonLower.includes("rate limit");
    const now = new Date().toISOString();
 ...
 */
async function fatalHandler(err) {
    const msg = err instanceof Error ? err.message : String(err);
    ...
    if (runId && projectPath) {
        ...
        if (seedId && runId) {
            try {
                ...
                await mailCandidate.sendMessage("foreman", "worker-error", JSON.stringify({
                    runId,
                    seedId,
                    error: msg,
                    phase: currentPhase,
