...
function parseEvents(rawOutput) {
    const events = [];
...
function isIgnoredReviewPath(fileName) {
...
function classifyFindings(events) {
    const blocking = [];
    const nonBlocking = [];
    const ignored = [];
    for (const event of events) {
        ...
        const finding = {
            ...
            suggestions: Array.isArray(event.suggestions)
                ? event.suggestions.filter((item) => typeof item === "string")
                : [],
        };
...
function renderReport(args) {
    ...
    if (result.blockingFindings.length === 0) {
        ...
        for (const finding of result.blockingFindings) {
            ...
            if (finding.comment)
                lines.push(`  - Note: ${finding.comment}`);
...
function isAuthenticationIssue(text) {
    ...
    if (!eventError) {
        ...
        for (let attempt = 0; attempt <= maxRateLimitRetries; attempt++) {
            try {
                ...
                rawOutput = typeof error.stdout === "string" ? error.stdout : "";
                ...
                if (status === "skipped" || !isRateLimitText(text) || attempt >= maxRateLimitRetries)
                    break;
...
//# sourceMappingURL=coderabbit-cli-review.js.map