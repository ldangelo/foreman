...
export function getForbiddenVcsAction(command, phase) {
    if (!command)
        return undefined;
    if (phase === "finalize")
        return undefined;
...
function deriveExpectedSkill(command) {
    if (!command)
...
export function createPhaseTrace(metadata) {
...
function findTool(trace, toolCallId) {
    return trace.toolCalls.find((tool) => tool.toolCallId === toolCallId);
}
export function createPiObservabilityExtension(trace) {
    return createPiObservabilityExtensionWithEmitter(trace);
}
export function createPiObservabilityExtensionWithEmitter(trace, emit) {
    return (pi) => {
        ...
        pi.on("tool_result", async (event) => {
            ...
            const contentPreview = summarizeUnknown(event.content
                ?.map((part) => (part && typeof part === "object" && "text" in part ? part.text : part))
                .filter(Boolean));
            if (contentPreview)
                tool.resultPreview = contentPreview;
        });
        ...
        pi.on("agent_end", async (event) => {
            ...
            if (!trace.finalMessage) {
                const assistantMessages = event.messages
                    .filter((message) => typeof message === "object" && message && "role" in message)
...
export function finalizePhaseTrace(trace, options) {
    trace.completedAt ??= new Date().toISOString();
    ...
    const wroteExpectedArtifact = trace.expectedArtifact
        ? trace.toolCalls.some((tool) => {
            if (tool.toolName.toLowerCase() !== "write" && tool.toolName.toLowerCase() !== "edit")
                return false;
        ...
        : false;
    const finalMessageMentionsExpectedSkill = Boolean(trace.expectedSkill && trace.finalMessage?.includes(trace.expectedSkill));
    const finalMessageMentionsArtifact = Boolean(trace.expectedArtifact && trace.finalMessage?.includes(basename(trace.expectedArtifact)));
...
//# sourceMappingURL=pi-observability-extension.js.map