 ...
 */
async function isBeadClosedOrGone(beadsClient, seedId) {
    try {
 ...
 */
export async function purgeZombieRunsAction(opts, beadsClient, store, projectPath) {
    ...
    console.log(chalk.bold(`Checking ${failedRuns.length} failed run(s) for zombie records…\n`));
    const result = {
    ...
    for (const run of failedRuns) {
        ...
        try {
            ...
            console.warn(chalk.yellow(`  warn  run ${run.id} (bead ${run.seed_id}): ${msg} — skipping`));
            result.errors += 1;
            continue;
        }
        if (!shouldPurge) {
            console.log(chalk.dim(`  skip  run ${run.id} — bead ${run.seed_id} is still open`));
            result.skipped += 1;
            continue;
        }
        if (dryRun) {
            console.log(chalk.cyan(`  would purge  run ${run.id} — bead ${run.seed_id} is closed/gone`));
            result.purged += 1;
        }
        else {
            store.deleteRun(run.id);
            console.log(chalk.green(`  purged  run ${run.id} — bead ${run.seed_id} is closed/gone`));
            result.purged += 1;
        }
    }
    // 4. Summary
    console.log();
    if (dryRun) {
        console.log(chalk.yellow(`Dry run complete — ${result.purged} zombie run(s) would be purged, ${result.skipped} skipped, ${result.errors} error(s).`));
    }
    else {
        console.log(chalk.green(`Done — ${result.purged} zombie run(s) purged, ${result.skipped} skipped, ${result.errors} error(s).`));
    }
    return result;
}
// ── CLI Command ─────────────────────────────────────────────────────────
export const purgeZombieRunsCommand = new Command("purge-zombie-runs")
    .description("Remove failed run records whose beads are already closed or no longer exist")
...
//# sourceMappingURL=purge-zombie-runs.js.map