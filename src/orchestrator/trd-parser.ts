 ...
 */
export function parseTableHeader(headerRow: string): ColumnMap {
  ...
  if (map.id == null || map.task == null) {
    ...
    throw new Error(
      `SLING-010: Table header missing required columns. ` +
 ...
 */
export function parseTableRow(row: string, columns: ColumnMap): TrdTask {
...
function parseStatus(raw: string): TrdTaskStatus {
  const trimmed = raw.trim();
  if (trimmed.includes("[x]") || trimmed.toLowerCase() === "done") return "completed";
  if (trimmed.includes("[~]")) return "in_progress";
  return "open";
}
...
export function parseEpic(content: string): EpicMeta {
  ...
  if (docIdMatch) documentId = docIdMatch[1].trim();

  const versionMatch = content.match(/\*\*Version:\*\*\s*(.+)/);
  if (versionMatch) version = versionMatch[1].trim();

  const epicIdMatch = content.match(/\*\*Epic ID:\*\*\s*(.+)/);
  if (epicIdMatch) epicId = epicIdMatch[1].trim();
...
export function parseSprintHeader(line: string): SprintHeader | null {
...
function parsePriorityFromText(text: string, sprintNumber: number): Priority {
  ...
  // Ordinal fallback
  if (sprintNumber <= 2) return "critical";
  if (sprintNumber <= 5) return "high";
  return "medium";
}
...
export function parseStoryHeader(line: string): { ref: string; title: string } | null {
...
export function parseAcceptanceCriteria(content: string): Map<string, string> {
  const acMap = new Map<string, string>();
  const lines = content.split("\n");
  let inSection5 = false;
  let currentFr: string | null = null;
  const currentAcs: string[] = [];
...
export function parseRiskRegister(content: string): Map<string, RiskLevel> {
  const riskMap = new Map<string, RiskLevel>();
  const lines = content.split("\n");
  let inSection7 = false;
  let columns: { likelihood: number; impact: number; tasksAffected: number } | null = null;
  ...
  for (const line of lines) {
    ...
    if (!columns || isSeparatorRow(line) || !line.includes("|")) continue;

    const cells = splitTableRow(line);
    if (cells.length <= columns.tasksAffected) continue;
...
export function parseQualityRequirements(content: string): string | undefined {
  const lines = content.split("\n");
...
export function parseSprintSummary(
  content: string,
): Map<number, SprintSummary> {
  const summaryMap = new Map<number, SprintSummary>();
  const lines = content.split("\n");
  let inSection3 = false;
  let headerColumns: { sprint: number; focus: number; hours: number; deliverables: number } | null = null;
  ...
  for (const line of lines) {
    ...
    if (!headerColumns || isSeparatorRow(line) || !line.includes("|")) continue;
 ...
 */
export function parseTrd(content: string): SlingPlan {
  const epic = parseEpic(content);
  const lines = content.split("\n");

  const sprints: TrdSprint[] = [];
  let currentSprint: TrdSprint | null = null;
  let currentStory: TrdStory | null = null;
  let currentColumns: ColumnMap | null = null;
  let currentFrRefs: string[] = [];
  ...
  if (totalTasks === 0) {
    throw new Error(
      "SLING-002: No tasks extracted from TRD. " +
