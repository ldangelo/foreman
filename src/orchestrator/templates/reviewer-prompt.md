...
```bash
foreman mail send --run-id "{{runId}}" --from "{{agentRole}}" --to foreman --subject phase-started --body '{"phase":"reviewer","seedId":"{{seedId}}"}'
```

When you finish writing REVIEW.md, run:
```bash
foreman mail send --run-id "{{runId}}" --from "{{agentRole}}" --to foreman --subject phase-complete --body '{"phase":"reviewer","seedId":"{{seedId}}","status":"complete"}'
```

If you hit an unrecoverable error, run:
```bash
foreman mail send --run-id "{{runId}}" --from "{{agentRole}}" --to foreman --subject agent-error --body '{"phase":"reviewer","seedId":"{{seedId}}","error":"<brief description>"}'
```
