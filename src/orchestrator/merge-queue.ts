...
export interface ReconcileResult {
  ...
  failedToEnqueue: Array<{ run_id: string; task_id: string; reason: string }>;
}
...
export class MergeQueue {
  ...
  enqueue(input: EnqueueInput): MergeQueueEntry {
    const { branchName, taskId, runId, operation = "auto_merge", agentName, filesModified } = input;
    ...
    const row = this.db
      .prepare(
        `INSERT INTO merge_queue (branch_name, task_id, run_id, operation, agent_name, files_modified, enqueued_at, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')
         RETURNING *`
      )
      .get(branchName, taskId, runId, operation, agentName ?? null, filesJson, now) as MergeQueueRow;
  ...
  /**
   * Reset a failed/conflict entry for a given task back to 'pending' so it
   * can be retried. Used by `foreman merge --task <id>` to allow re-processing
    ...
    const result = this.db
      .prepare(
        `UPDATE merge_queue
         ...
         WHERE task_id = ? AND status IN ('failed', 'conflict', 'merging')
         RETURNING id`
      )
      .get(now, taskId);
    return result != null;
      ...
      .prepare(
        `SELECT r.id AS run_id, r.task_id
         ...
         AND NOT EXISTS (
           ...
           WHERE mq.run_id = r.id OR mq.task_id = r.task_id
         )
  ...
  async reconcile(
    ...
    const completedRuns = db
      ...
      .all() as Array<{ id: string; task_id: string; merge_strategy?: "auto" | "pr" | "none" | null }>;

    // Get all run_ids AND task_ids already in merge_queue.
    // Dedup by task_id so that sentinel-created duplicate completed runs for
    // the same task don't each create a separate queue entry.
    const mqRows = db
      .prepare("SELECT run_id, task_id FROM merge_queue")
      .all() as Array<{ run_id: string; task_id: string }>;
    ...
    for (const run of completedRuns) {
      ...
      // Skip if any run for this task is already queued (dedup sentinel retries)
      if (existingTaskIds.has(run.task_id)) {
        skipped++;
        continue;
      }
      ...
      // Track newly enqueued task so further duplicates in this batch are skipped
      existingTaskIds.add(run.task_id);
      enqueued++;
    }
    ...
    // (refs/remotes/origin/foreman/<taskId>) which only exists after a successful
    ...
    const interruptedRuns = db
      ...
      .all() as Array<{ id: string; task_id: string; created_at: string | null }>;

    // Deduplicate by task_id: only process the oldest run per task. A task maps
    // 1-to-1 with a branch name (foreman/<taskId>), so if multiple runs exist for
    // the same task (e.g. a crashed old run and a newly-dispatched replacement),
    ...
    for (const run of interruptedRuns) {
      ...
      // Only recover one run per task to avoid marking a newer in-progress run
      ...
      // (refs/remotes/origin/foreman/<taskId>) may persist until a `git fetch
      ...
      if (run.created_at) {
        ...
        if (commitMs < runCreatedMs) {
          ...
          // from a previous run (e.g. after foreman reset --task <id>).
