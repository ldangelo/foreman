...
Do not send `agent-error` for a real CI failure, missing implementation, failing tests, stale branch, or no-op diff. Those are normal remediation outcomes: write `{{reportDir}}/DEVELOPER_REPORT.md` with the failed check, root cause, fix or no-op evidence, and verification status. Reserve `agent-error` for infrastructure failures that prevent reading the check or writing the report.
...
- If the failed check is already fixed on the target branch and this retry has no task diff to repair, document the evidence in `{{reportDir}}/DEVELOPER_REPORT.md` and leave the working tree clean.
