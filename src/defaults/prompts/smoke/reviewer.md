...
```bash
npx foreman mail send --run-id "{{runId}}" --from "{{agentRole}}" --to foreman --subject phase-started --body '{"phase":"reviewer","smoke":true}'
```
...
```bash
npx foreman mail send --run-id "{{runId}}" --from "{{agentRole}}" --to foreman --subject phase-complete --body '{"phase":"reviewer","smoke":true,"status":"complete"}'
```
