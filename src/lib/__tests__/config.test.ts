...
describe("PIPELINE_TIMEOUTS defaults", () => {
  ...
  it("taskClosureMs defaults to 30000", () => {
    // Increased from 10s to 30s (commit 5c9f686: fix: increase task-writer timeout from 10s to 30s)
    expect(PIPELINE_TIMEOUTS.taskClosureMs).toBe(30_000);
  });
