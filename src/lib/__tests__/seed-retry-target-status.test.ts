...
import { getSeedRetryTargetStatus } from "../run-status.js";
...
describe("getSeedRetryTargetStatus — reset semantics (foreman reset)", () => {
  const reset = (status: string) =>
    getSeedRetryTargetStatus(status, { command: "reset" });
...
describe("getSeedRetryTargetStatus — retry semantics, native backend", () => {
  const retryNative = (status: string) =>
    getSeedRetryTargetStatus(status, { command: "retry", backendType: "native" });
...
describe("getSeedRetryTargetStatus — retry semantics, br-style backend", () => {
  ...
  const retryBr = (status: string) =>
    getSeedRetryTargetStatus(status, { command: "retry", backendType: brBackend });
