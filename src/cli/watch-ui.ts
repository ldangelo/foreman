...
export function renderWatchDisplay(state: WatchState, showDetachHint = true, expandedRunIds?: Set<string>): string {
  ...
  // Header — build hint string incrementally so each hint can be
  // conditionally omitted without leaving stray separators.
  let detachHint = "";
  if (showDetachHint && !state.allDone) {
    const hintParts: string[] = [chalk.dim("Ctrl+C to detach")];
    // Toggle hints are only meaningful in interactive mode, i.e. when the
    // caller passes an expandedRunIds set.  Omitting them when expandedRunIds
    // is undefined prevents misleading output in non-interactive contexts
    // such as `foreman status`.
    if (expandedRunIds !== undefined) {
      hintParts.push(chalk.dim("'a' toggle all"));
      // The numeric index hint is only useful when there are multiple agents;
      // a single-agent watch shows no numeric prefix on the card, so omit it.
  ...
  for (let i = 0; i < state.runs.length; i++) {
    ...
    lines.push(renderAgentCard(run, progress, isExpanded, state.runs.length > 1 ? i : undefined));
    lines.push("");
  }
