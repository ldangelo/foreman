...
export interface TaskDetailsProps {
  ...
  /** Optional renderer for the on-disk artifact (logs/reports/files). */
  renderTaskDetail?: (summary: InboxTaskSummary, options: { messages: boolean; events: boolean; logs?: boolean; reports?: boolean; files?: boolean; limit: number; eventsLimit: number }) => string | Promise<string>;
 ...
 */
export function TaskDetails({ summary, tab = "summary", compact = false, renderTaskDetail: _renderTaskDetail, renderedDetail, limit: _limit = 50, eventsLimit: _eventsLimit = 50 }: TaskDetailsProps): ReactElement {
  if (!summary) {
    return h(Pane, { title: "Details", minHeight: 7 }, h("span", null, "No task selected."));
  }
  ...
  if (tab === "logs" && !renderedDetail) rows.push("Logs: showing compact log summary in the main pane.");
  if (tab === "reports" && !renderedDetail) rows.push("Reports: showing report directory/files in the main pane.");
  if (tab === "files" && !renderedDetail) rows.push(summary.worktreePath ? "Files: showing worktree status in the main pane." : "Files: no worktree path recorded.");

  if (renderedDetail) rows.push(...renderedDetail.split("\n").slice(0, compact ? 6 : 10));
  ...
  return h(Pane, { title: `Details · ${summary.taskId}`, minHeight: 7 },
    ...rows.filter((row): row is string => row !== null).slice(0, compact ? 8 : 14).map((row, index) =>
      h("span", { key: `${summary.runId}-detail-${index}` }, truncate(row, compact ? 84 : 132))
    ),
  );
}
