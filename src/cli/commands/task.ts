...
type TaskNoteRow = { id: string; task_id: string; body: string; author?: string | null; created_at: string; phase?: string | null; kind?: string | null };
type DependencyRow = { task_id?: string; depends_on?: string; dependency_type?: string | null; created_at?: string; type: string; from_task_id: string; to_task_id: string };
...
interface RunActivityInfo {
  ...
  workflowPhases: string[] | undefined;
...
export function renderRunStatusLine(activity: RunActivityInfo): string {
  ...
  // Workflow phases pipeline
  if (activity.workflowPhases && activity.workflowPhases.length > 0) {
    const phaseStr = activity.workflowPhases.join(" → ");
    parts.push(chalk.dim("│") + " " + chalk.dim("phases:") + " " + chalk.white(phaseStr));
  }
