...
export async function runSetupWithCache(worktreePath, projectRoot, steps, cache) {
    if (cache) {
        ...
        if (restored)
            return; // cache hit — skip setup steps
    }
...
export async function detectDefaultBranch(repoPath) {
    ...
    try {
        const gtMain = await git(["config", "get", "git-town.main-branch"], repoPath);
        if (gtMain)
            return gtMain;
    }
    catch {
    ...
    try {
        const ref = await git(["symbolic-ref", "refs/remotes/origin/HEAD", "--short"], repoPath);
...
export async function createWorktree(repoPath, seedId, baseBranch, setupSteps, setupCache) {
    ...
    try {
        await git(["worktree", "add", "-b", branchName, worktreePath, base], repoPath);
    }
    catch (err) {
