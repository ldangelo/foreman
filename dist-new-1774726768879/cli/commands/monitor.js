...
export const monitorCommand = new Command("monitor")
    ...
    if (!opts.json) {
        console.warn(chalk.yellow("⚠  'foreman monitor' is deprecated. Use 'foreman reset --detect-stuck' instead.\n" +
    ...
    try {
        ...
        if (report.active.length > 0) {
            ...
            for (const run of report.active) {
                const elapsed = run.started_at
                    ...
                    : 0;
                console.log(`  ${chalk.cyan(run.seed_id)} ${chalk.dim(`[${run.agent_type}]`)} ${elapsed}m`);
            }
            console.log();
        }
        // Completed
        ...
        const total = report.active.length +
