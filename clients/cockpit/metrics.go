...
func renderMetricsLines(metrics Metrics, w int) []ViewerLine {
	...
	if len(metrics.Counters) > 0 {
		...
		for _, key := range sortedMetricKeys(metrics.Counters) {
			lines = append(lines, ViewerLine{Key: "metrics:counter:" + key, Text: metricBar(key, metrics.Counters[key], maxMetricValue(metrics.Counters), w)})
		}
	}
	if len(metrics.Gauges) > 0 {
		...
		for _, key := range sortedMetricKeys(metrics.Gauges) {
			lines = append(lines, ViewerLine{Key: "metrics:gauge:" + key, Text: metricBar(key, metrics.Gauges[key], max(1, maxMetricValue(metrics.Gauges)), w)})
		}
	}
	if len(metrics.PhaseDuration) > 0 {
		...
		for i, duration := range metrics.PhaseDuration {
			...
			lines = append(lines, ViewerLine{Key: "metrics:duration:" + itoa(i), Text: metricBar(label, duration.DurationMS/1000, max(1, maxPhaseSeconds(metrics.PhaseDuration)), w)})
		}
	}
...
func metricBar(label string, value, maxValue, w int) string {
	if maxValue < 1 {
		maxValue = 1
	}
	...
	left := dimStyle.Render(clip(label, 18))
	right := cyanStyle.Render(bar) + " " + whiteStyle.Render(itoa(value))
