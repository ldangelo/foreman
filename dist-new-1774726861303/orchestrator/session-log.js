 ...
 */
export function formatSessionLogFilename(date) {
 ...
 */
export function generateSessionLogContent(data, date) {
    ...
    const { seedId, seedTitle, seedDescription, branchName, projectName, phases, totalCostUsd, totalTurns, filesChanged, devRetries, qaVerdict, } = data;
    ...
    lines.push("## Summary");
    lines.push("");
    lines.push(`Pipeline run for **${seedId}** — ${seedTitle}.`);
    ...
    if (failedPhases.length > 0 || devRetries > 0) {
        ...
        if (devRetries > 0) {
            ...
            lines.push("");
            lines.push(`The developer phase was retried ${devRetries} time(s) due to QA or review feedback.`);
            lines.push("");
        }
    }
 ...
 */
export async function writeSessionLog(basePath, data, date = new Date()) {
