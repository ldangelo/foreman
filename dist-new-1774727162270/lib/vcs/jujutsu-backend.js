...
export class JujutsuBackend {
    ...
    async getCurrentBranch(repoPath) {
        try {
            const bookmarks = await this.jj(["log", "--no-graph", "-r", "@", "-T", "separate(' ', bookmarks)"], repoPath);
            if (bookmarks) {
     ...
     * a bookmark `foreman/<seedId>` pointing to the new workspace's working-copy
     * change via `jj bookmark create foreman/<seedId> -r @`.
     *
     * Handles existing workspaces by running `jj workspace update-stale` and
     * rebasing onto the base branch (AC-009-3).
     *
     * AC-009-1: workspace directory is created at `.foreman-worktrees/<seedId>`
     * AC-009-2: bookmark `foreman/<seedId>` is created pointing to the new workspace's @
     * AC-009-3: update-stale is called for existing workspaces, then rebase onto base
     * AC-009-4: returns WorkspaceResult with workspacePath and branchName
     * AC-009-5: idempotent — calling twice with the same seedId does not throw
     */
    async createWorkspace(repoPath, seedId, baseBranch) {
        ...
        // If workspace directory already exists, reuse it (AC-009-5: idempotent)
        if (existsSync(workspacePath)) {
            // AC-009-3: update-stale ensures the workspace's working copy is fresh
            // before attempting a rebase
            try {
                await this.jj(["workspace", "update-stale"], workspacePath);
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
                console.error(`[jj] Warning: workspace update-stale failed for ${workspaceName}: ${msg}`);
            }
        ...
        try {
            await this.jj(["workspace", "add", "--name", workspaceName, workspacePath], repoPath);
        }
        catch (err) {
        ...
        try {
            ...
            // Bookmark may already exist — try to move it to @
            try {
                await this.jj(["bookmark", "move", branchName, "--to", workspaceRef], repoPath);
            }
            catch (moveErr) {
