...
export async function detectExistingEpic(documentId, seeds, beadsRust) {
    let sdEpicId = null;
    let brEpicId = null;
...
async function executeForSeeds(seeds, ctx, existingEpicId) {
    const { plan, parallel, options } = ctx;
    const result = { created: 0, skipped: 0, failed: 0, epicId: null, errors: [] };
    const trdIdToSdId = new Map();
    const trdIdToSdSprintId = new Map();
    const trdIdToSdStoryId = new Map();
    // Count total items for progress
    const totalTasks = plan.sprints.reduce((sum, s) => sum + s.stories.reduce((ss, st) => ss + st.tasks.length, 0), 0);
    ...
    try {
            ...
            for (const story of sprint.stories) {
                ...
                for (const task of story.tasks) {
                    ...
                    try {
                        ...
                        if (kind !== "task")
                            taskLabels.push(`kind:${kind}`);
                        if (task.estimateHours > 0)
                            taskLabels.push(`est:${task.estimateHours}h`);
                        if (task.riskLevel && !options.noRisks)
                            taskLabels.push(`risk:${task.riskLevel}`);
                        ...
                        result.failed++;
                        result.errors.push(`SLING-006: Failed to create sd task ${task.trdId}: ${err.message}`);
                    }
                    ctx.onProgress?.(created, totalItems, "sd");
                }
            }
        ...
        const containerDepErrors = await wireContainerDepsSd(seeds, plan, trdIdToSdSprintId, trdIdToSdStoryId);
        result.errors.push(...containerDepErrors);
    }
    catch (err) {
        result.errors.push(`SLING-006: Unexpected sd error: ${err.message}`);
    }
    return result;
}
async function executeForBeadsRust(beadsRust, ctx, existingEpicId) {
    const { plan, parallel, options } = ctx;
    const result = { created: 0, skipped: 0, failed: 0, epicId: null, errors: [] };
    const trdIdToBrId = new Map();
    ...
    try {
            ...
            for (const story of sprint.stories) {
                ...
                for (const task of story.tasks) {
                    ...
                    try {
                        ...
                        result.failed++;
                        result.errors.push(`SLING-006: Failed to create br task ${task.trdId}: ${err.message}`);
                    }
                    ctx.onProgress?.(created, totalItems, "br");
                }
            }
        ...
        const containerDepErrors = await wireContainerDepsBr(beadsRust, plan, trdIdToSprintId, trdIdToStoryId);
        result.errors.push(...containerDepErrors);
    }
    catch (err) {
        result.errors.push(`SLING-006: Unexpected br error: ${err.message}`);
    }
    return result;
}
// ── Dependency wiring ────────────────────────────────────────────────────
async function wireContainerDepsSd(client, plan, trdIdToSprintId, trdIdToStoryId) {
    const depErrors = [];
...
async function wireDependencies(client, plan, trdIdToTrackerId, options, result) {
    const depErrors = [];
    for (const sprint of plan.sprints) {
        for (const story of sprint.stories) {
            for (const task of story.tasks) {
                ...
                for (const depTrdId of task.dependencies) {
                    ...
                    if (!taskTrackerId)
                        continue; // Task was skipped or failed
                    if (!depTrackerId) {
                    ...
                    try {
                        ...
                        const msg = `SLING-007: Failed to wire dep ${task.trdId} -> ${depTrdId}: ${err.message}`;
                        depErrors.push(msg);
                    }
                }
            }
...
async function wireDependenciesBr(client, plan, trdIdToTrackerId, options, result) {
    const depErrors = [];
    for (const sprint of plan.sprints) {
        for (const story of sprint.stories) {
            for (const task of story.tasks) {
 ...
 */
async function wireContainerDepsBr(client, plan, trdIdToSprintId, trdIdToStoryId) {
    const depErrors = [];
    // Collect unique sprint→sprint and story→story blocking pairs
    const sprintDeps = new Set(); // "sprintId|depSprintId"
    const storyDeps = new Set(); // "storyId|depStoryId"
    for (const sprint of plan.sprints) {
        for (const story of sprint.stories) {
            for (const task of story.tasks) {
                const taskSprintId = trdIdToSprintId.get(task.trdId);
...
// ── Public API ───────────────────────────────────────────────────────────
export async function execute(plan, parallel, options, seeds, beadsRust, onProgress) {
    const result = { sd: null, br: null, depErrors: [] };
    const ctx = { plan, parallel, options, onProgress };
    // Detect existing epics
    const existing = await detectExistingEpic(plan.epic.documentId, options.force ? null : seeds, options.force ? null : beadsRust);
    ...
    // Collect dep errors
    if (result.sd)
        result.depErrors.push(...result.sd.errors.filter((e) => e.includes("SLING-007")));
    if (result.br)
        result.depErrors.push(...result.br.errors.filter((e) => e.includes("SLING-007")));
    return result;
}
