...
Foreman CLI / Dispatcher
  ...
  │    Project/global JS/MJS/TS actions in .foreman/actions or ~/.foreman/actions can override phase behavior without rebuild; action modules are bundled before load so JS/MJS can import TS helpers (`foreman actions list|show|validate|install|create`, use --global for global stubs; validation checks JS/TS syntax/imports/exports, and action names allow letters/numbers plus `.`, `_`, `-`, with at least one letter/number)
