...
base branch, and `timeout_minutes:` (alias `timeoutMinutes:`) to declare a non-negative-integer execution
timeout in minutes — `0`, or an omitted key, both mean no timeout (the
...
created/reused phase PR records. Final AutoPR titles/bodies include the
approved task summary when task metadata is available; ad-hoc runs keep the
legacy generic run summary. Full operator semantics live in
...
The MCP task tools expose `foreman_task_list`, `foreman_task_get`, and write-gated `foreman_task_update` / `foreman_task_add_comment`. Task list supports `project_id`/canonical `status` filters plus `limit`/`offset` pagination and returns `{tasks, total, limit, offset, next_offset}`.

The MCP `foreman_task_create` write tool accepts optional `task_type` and defaults it to `"task"` when omitted.
The MCP `foreman_inbox_send` write tool appends concise run progress notes to the per-run operator inbox when `allow_workflow_writes: true`; it requires `run_id` and a 1-2,000 character `body`, accepts optional `message_id`/`command_id` plus safe metadata, and is default-hidden/refused like other write tools. The MCP `foreman_task_add_comment` write tool appends structured Work Log comments to the provider-backed task for a run; it accepts `run_id`, `workflow_name`, `phase_name`, and `description`, while Foreman resolves the provider issue server-side. Use these for phase starts, material milestones, blockers, and phase completion; bundled prompt phases carry this guidance in prompt text, and the built-in `prd`/`fix` command phases receive the same non-blocking guidance through the worker `system_prompt` sidecar without changing their slash-command prompt. Use `foreman_inbox_get` to read the inbox and `foreman_run_get_logs` / `foreman_run_get_activity` for raw stdout/stderr and heartbeat evidence.
