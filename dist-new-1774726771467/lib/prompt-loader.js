...
export function loadPrompt(phase, vars, workflow, projectRoot) {
    ...
    const projectPromptPath = join(projectRoot, ".foreman", "prompts", workflow, `${phase}.md`);
 ...
 */
export function installBundledPrompts(projectRoot, force = false) {
    const installed = [];
    const skipped = [];
 ...
 */
export function findMissingPrompts(projectRoot) {
    const missing = [];
    for (const [workflow, phases] of Object.entries(REQUIRED_PHASES)) {
        for (const phase of phases) {
            const p = join(projectRoot, ".foreman", "prompts", workflow, `${phase}.md`);
            if (!existsSync(p)) {
