defmodule ForemanServer.Agents.JidoCheckpointStoreTest do
  ...
  defp clear_repo! do
    Application.put_env(:foreman_server, JidoCheckpointStore, [])
  end

  defp set_repo!(module) do
    Application.put_env(:foreman_server, JidoCheckpointStore, repo: module)
  end

  describe "API surface (matches upstream typespecs)" do
    test "put/3 returns :ok | {:error, term()}" do
      Code.ensure_loaded?(JidoCheckpointStore)
      clear_repo!()
