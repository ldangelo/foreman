...
def list_ready(project_config, _opts) when is_map(project_config) do
  database_path =
    case project_config do
      %{database_path: cached_path} when is_binary(cached_path) -> cached_path
      %{"database_path" => cached_path} when is_binary(cached_path) -> cached_path
      other -> raise ArgumentError, "expected project_config with binary :database_path, got: #{inspect(other)}"
    end

  case @runner.cmd({:ready, %{database_path: database_path}}, project_config, timeout_ms: 30_000) do
    {:ok, %{stdout: stdout}} ->
...
defp parse_issue_payload(%{} = payload) do
  with :ok <- JsonSchemaCache.validate(:ready_issue, payload),
       ...
       {:ok, assignee} <- parse_optional_string(fetch_payload_value(payload, :assignee), :assignee),
       {:ok, description} <-
       ...
       {:ok, labels} <- parse_opaque_string_list(fetch_payload_value(payload, :labels), :labels),
...
defp fetch_required_string(payload, key) do
  case fetch_payload_value(payload, key) do
    ...
    _other ->
      {:error, build_list_ready_contract_error("Beads ready issue field #{inspect(key)} must be a string.")}
  end
end
...
defp parse_optional_string(_value, field) do
  {:error, build_list_ready_contract_error("Beads ready issue field #{inspect(field)} must be a string or null.")}
end
...
defp parse_priority(_value) do
  {:error, build_list_ready_contract_error("Beads ready issue field :priority must be a non-negative integer.")}
end
...
defp parse_opaque_string_list(values, field) when is_list(values) do
  if Enum.all?(values, &is_binary/1) do
    ...
    {:error, build_list_ready_contract_error("Beads ready issue field #{inspect(field)} must be a list of strings.")}
  end
end
...
defp parse_metadata(_metadata) do
  {:error, build_list_ready_contract_error("Beads ready issue field :metadata must be an object.")}
end
