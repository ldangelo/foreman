defmodule ForemanServer.Aggregates.Notification do
      ...
      "NotificationEnqueued" ->
        %State{
          ...
          | exists?: true,
            ...
            status: :enqueued,
            last_sequence: get(payload, :sequence, state.last_sequence),
            last_enqueued_at_ms:
              get(payload, :enqueued_at_ms) || get(payload, :event_at_ms) ||
        ...
        true ->
          {:ok,
           %{
             ...
             payload:
               dto
               |> NotificationDto.to_event_payload()
               |> Map.put(:sequence, next_sequence(state))
               |> Map.put(:enqueued_at_ms, now_ms)
           }}
      end
    end
  end
  ...
  defp duplicate?(
         %State{correlation_id: correlation_id, last_enqueued_at_ms: last},
         correlation_id,
         now_ms,
         window
       )
  ...
  defp suppressed(dto, payload, reason) do
    {:ok,
     %{
       stream_id: stream_id(dto.correlation_id),
       event_type: "NotificationSuppressed",
       payload: %{
  ...
  defp result_event(state, payload, event_type, required_keys) do
    with {:ok, notification_id} <-
         ...
         {:ok, correlation_id} <-
           required_binary(get(payload, :correlation_id) || state.correlation_id, :correlation_id),
  ...
  defp put_attempt(state, payload, status) do
    ...
    attempt = %{
      ...
      retryable?: get(payload, :retryable?),
    ...
    %State{
      state
      | exists?: true,
        notification_id: get(payload, :notification_id) || state.notification_id,
        correlation_id: get(payload, :correlation_id) || state.correlation_id,
        provider: get(payload, :provider) || state.provider,
        run_id: get(payload, :run_id) || state.run_id,
        status: status,
        last_sequence: get(payload, :sequence, state.last_sequence),
  ...
  defp notification_attrs(payload) do
    Map.drop(payload, [
      :now_ms,
      "now_ms",
      :dedupe_window_ms,
