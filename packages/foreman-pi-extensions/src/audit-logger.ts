...
export function createAuditExtension(outputDir?: string): ForemanExtension {
  ...
  function writeEntry(entry: object): void {
    try {
  ...
  return {
    ...
    onToolCall(event: ToolCallEvent, _ctx: ExtensionContext): undefined {
      writeEntry({ ...makeBase('tool_call'), toolName: event.toolName });
      return undefined; // Observer only — never blocks.
    },

    onTurnEnd(event: TurnEndEvent, _ctx: ExtensionContext): void {
      writeEntry({
        ...makeBase('turn_end'),
    ...
    onToolExecutionStart(event: ToolExecutionStartEvent, _ctx: ExtensionContext): void {
      writeEntry({
    ...
    onToolExecutionEnd(event: ToolExecutionEndEvent, _ctx: ExtensionContext): void {
      writeEntry({
