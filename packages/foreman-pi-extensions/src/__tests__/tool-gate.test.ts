...
import type { ToolCallEvent, ExtensionContext, ToolCallResult } from '../types.js';
...
describe('AC6: .beads/ directory is always protected', () => {
  it('blocks Write to .beads/issues.jsonl regardless of allowlist', () => {
    ...
    const result = callSync(
      ext,
      makeEvent('Write', { file_path: '.beads/issues.jsonl' }),
      makeCtx('developer'),
    );
  ...
  it('blocks Edit to .beads/beads.jsonl', () => {
    ...
    const result = callSync(
      ext,
      makeEvent('Edit', { file_path: '.beads/beads.jsonl' }),
      makeCtx('developer'),
    );
...
describe('Edge cases', () => {
  ...
  it('does not block paths that merely contain "beads" without the directory marker', () => {
    ...
    const result = callSync(
      ext,
      makeEvent('Write', { file_path: 'src/beads-helper.ts' }),
      makeCtx('developer'),
    );
