...
export function createBudgetExtension(auditCallback?: (event: object) => void): ForemanExtension {
  let turnCount = 0;
  ...
  return {
    ...
    onTurnEnd(event: TurnEndEvent, ctx: ExtensionContext): ToolCallResult | void {
      turnCount++;
      const maxTurns = parseInt(process.env.FOREMAN_MAX_TURNS ?? String(DEFAULT_MAX_TURNS), 10);
      const maxTokens = parseInt(process.env.FOREMAN_MAX_TOKENS ?? String(DEFAULT_MAX_TOKENS), 10);
