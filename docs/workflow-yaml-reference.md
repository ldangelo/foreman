...
`task_type` must be a non-empty string. Each task type may be declared by at most one workflow. Duplicate declarations are reported by legacy `FOREMAN_BACKEND=node foreman doctor` and should be fixed before dispatch.
...
**Cache invalidation:** When the key file changes (e.g. new dependency added to `package-lock.json`), the hash changes and a new cache entry is created. Old entries remain until manually removed from `.foreman/setup-cache/` (legacy `FOREMAN_BACKEND=node foreman worktree clean` does not support cache purging).
...
| `action` | string | inferred | Reusable execution action (`prepare-worktree`, `setup-workspace`, `write-task-context`, `prompt-agent`, `command-agent`, `bash`, `qlty`, `finalize`, `cli-review`, `create-pr`, `pr-wait`, `prepare-pr-review`, `merge`, or custom) |
...
| `capabilities` | string[] | `[]` | Declared host capabilities used by custom/project action modules; valid values are `vcs`, `mail`, `task-store`, `network`, `exec` |
...
Additional `contract.completion` fields include `requireFilesChanged` and `requireValidationNotes` for handoff-style phases. `contract.policy` declares engine-level validation behavior that used to be tied to specific phase names: `requiresExplorerReport`, `explorerCircuitBreaker`, `developerCompletion`, `redPhaseCompletion`, `acceptanceCoverage`, `allowDeferredAcceptance`, `testEvidence`, `captureQaTarget`, `finalizeValidation`, `skipDeveloperRetryOnUnrelatedFinalizeFailure`, `structuredFailureChecklist`, and `terminalOnFailExhausted`. Dispatcher actions (`prepare-worktree`, `setup-workspace`, `write-task-context`) run before worker launch and may omit `prompt`, `bash`, `command`, and `builtin`. Workflow YAML resolves from an explicit YAML path, then project `.foreman/workflows/{name}.yaml|.yml`, then global `~/.foreman/workflows/{name}.yaml|.yml`, then bundled defaults; project-relative explicit paths must stay inside the project root, workflow `name` must be a non-empty safe string (letters/numbers plus `.`, `_`, `-`, with at least one letter or number), and project workflows also participate in `task_type` routing when a project path is available. Use `foreman workflows list|show|validate|install|create` to inspect, validate, install bundled YAML, or create workflow stubs for project/global override directories; validation flags duplicate `.yaml`/`.yml` variants for the same workflow name. Project actions can be overridden or added without rebuilding Foreman by adding `.foreman/actions/{action}.js`, `.foreman/actions/{action}.mjs`, or `.foreman/actions/{action}.ts`, or globally via `~/.foreman/actions/{action}.js|.mjs|.ts` (project actions win); action modules are bundled with esbuild before runtime load so JS/MJS actions may import TS helper files kept outside the actions directory (every `.js`/`.mjs`/`.ts` file directly under `.foreman/actions` or `~/.foreman/actions` is treated as an action module). Modules must export a default function or named `run` function; if both are present, a callable default takes precedence, otherwise callable `run` is used (for example `export default async function run(ctx) {}`, `export default async (ctx) => {}`, `export default async ctx => {}`, `export async function run(ctx) {}`, `export const run = async (ctx) => {}`, `export { run }`, `export { execute as run }`, `export { execute as default }`, `export default execute` where `run`/`execute` is a local function, or `export { run } from "../action-lib/run.ts"` for helper re-exports); commented or stringified export examples do not satisfy validation. A custom action phase may omit `prompt`, `bash`, `command`, and `builtin` when `action` names a project/global module; explicit unknown action names run through the custom/builtin execution path even if extra phase config fields are present. Action names must use only letters, numbers, `.`, `_`, and `-`, and include at least one letter or number. Bundled editable stubs and generated stubs for known builtin/workspace actions call `ctx.internal.runBuiltin()` by default, and legacy `FOREMAN_BACKEND=node foreman doctor` / `foreman actions validate` validate safe action names, JS/TS syntax/import resolution, required function exports, duplicate `.js`/`.mjs`/`.ts` variants, and workflow references to custom actions. Action `ctx` includes `actionType`, `capabilities`, `requireCapability(name)`, `phase`, `config`, `workflowConfig`, `log`, and `internal.runBuiltin()`; privileged worker helpers are gated by declared capabilities (`mail`, `vcs`, and `task-store` expose `ctx.mail`, `ctx.vcsBackend`, `ctx.store`, and `ctx.taskClient` respectively); worker-phase actions must return a phase result with `success`, `costUsd`, `turns`, `tokensIn`, and `tokensOut`, while workspace actions receive workspace fields such as `repoPath`, `worktreePath`, `branchName`, `seedInfo`, and setup/hook metadata plus `capabilities`/`requireCapability(name)`, and must return the updated workspace context object with required identity/model/log fields preserved. `capabilities` accepts only known non-empty values (`vcs`, `mail`, `task-store`, `network`, `exec`) with no duplicates; known privileged builtin/dispatcher actions merge default capabilities with declared additions, custom phases use their declared list, Foreman gates built-in helper access for `mail`/`vcs`/`task-store`, and custom actions can call `ctx.requireCapability("exec")` (or another declared value) to fail fast when the workflow did not declare expected host access. The built-in `qlty` action runs `qlty check`, requires the qlty CLI from https://qlty.sh/ on `PATH`, accepts an optional phase `command` override, and writes `QLTY_REPORT.md`; bundled developer workflows run it before `cli-review` and retry Developer when it fails. Before retrying a target phase or forwarding an artifact to a phase, Foreman writes a normalized `{TARGET_PHASE}_TASK.md` input file in the run report directory (for example `DEVELOPER_TASK.md`, `QA_TASK.md`, or `REVIEWER_TASK.md`) with the source phase, source artifact, failure, retry attempt, and feedback content. Phase names must use only letters, numbers, `.`, `_`, and `-`, include at least one letter or number, and must be unique; `retryWith`, `mail.onFail`, `mail.forwardArtifactTo`, `taskPhases`, and `finalPhases` must target phases declared in the same workflow (`mail.forwardArtifactTo: foreman` is also valid), and `taskPhases`/`finalPhases` cannot repeat the same phase. Top-level `task_type` must be a non-empty string. Numeric controls such as `maxTurns`, `timeoutSecs`, `cooldownSeconds`, `files.leaseSecs`, `taskTimeout`, `onFailure.maxTurns`, and `overwatch.checkEveryTurns` must be positive finite numbers; `retryOnFail` and overwatch counters must be non-negative integers. String controls (`name`, `task_type`, `setup.description`, `pr.timing`, phase `prompt`, `model`, `bash`, `command`, `artifact`, `skipIfArtifact`, `retryWith`, `contract.goal`, and mail targets) must be non-empty when present; boolean controls must be booleans, not stringified booleans. Contract string arrays (`requiredSections`, `allowedScope.canWriteOnly`), `tools.allowed`, `capabilities`, and `overwatch.blockedCommands` must contain unique non-empty strings. Object controls (`pr`, `setupCache`, `onFailure`, and phase `tools`/`contract`/`overwatch`/`mail`/`files` blocks) must be objects when present; non-object `vcs` blocks are ignored for backward compatibility. `setup` commands and optional descriptions must be non-empty. `setupCache` requires non-empty `key` and `path`; `onFailure` names must be safe phase names, optional prompt/artifact strings must be non-empty, and `models` maps accept only `default`/`P0`–`P4` keys with non-empty model strings. Additional `overwatch` controls include `forceArtifactAfterToolCalls`, `maxToolCalls`, `repeatedCommandLimit`, and `blockedCommands` (regular-expression strings matched against normalized shell commands).
...
Retry budgets are **independent per failing/source phase**, not per retry target. QA, Reviewer, CLI review, PR review, and Finalize can all loop back to Developer without consuming a shared Developer retry budget. With QA at 3 and Finalize at 6, Developer can run once initially plus additional source-phase retries. Each retry writes the target phase's normalized task input (for example `DEVELOPER_TASK.md`) from the failing phase artifact before control jumps back. If a verdict phase still reports FAIL after its own retry budget is exhausted, the pipeline stops instead of continuing to later phases with invalid evidence.
