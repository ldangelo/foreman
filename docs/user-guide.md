...
Security behavior:
- Worker environments are scoped to the project/run. Worker env vars are sourced from the registered project's `config.env`, `config.project_secrets`, and `config.run_secrets` fields; explicit project and run secret maps are merged after host environment filtering, and forbidden variables such as `FOREMAN_SERVER_AUTH_TOKEN`, `AWS_*`, `GITHUB_*`, `NPM_*`, `SSH_*`, and `DATABASE_*` are stripped. Foreman control-plane vars (`FOREMAN_SERVER_URL`, auth token, HTTP flags) always take precedence over any project-level overrides.
