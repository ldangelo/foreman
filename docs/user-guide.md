...
- `foreman purge logs --dry-run` previews Elixir-backed local log cleanup candidates without deleting files; actual log deletion and `FOREMAN_BACKEND=node foreman purge runs` remain legacy cleanup paths. `FOREMAN_BACKEND=node foreman purge runs` archives legacy failed runs whose tasks are closed; permanent deletion is available via the explicit purge option.
