...
The `FOREMAN_USE_JIDO_HARNESS` environment variable (config
`:foreman_server, :jido_harness, :enabled`, default `false`) gates the
whole adapter — including any newly added provider. Operators opt in by
exporting `FOREMAN_USE_JIDO_HARNESS=true`. A new provider inherits this
flag automatically; no per-provider flag exists.
