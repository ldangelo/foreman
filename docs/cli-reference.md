...
Workflow PR/merge behavior is **not** phase-driven. This previously claimed it "is controlled by explicit `create-pr`, `pr-wait`, and `merge` phases" — no such phase is implemented or dispatched, and a phase declaring none of `command`/`prompt`/`bash` fails to load with `{:missing_phase_action, index}`. A run opens at most one PR, from its single branch `foreman/<run-id>`, via `AutoPR.maybe_create_pr/1` at run finalization. Top-level `merge:` and `pr:` YAML tags are invalid, and there is no `stacked:` or `checkpointPr` setting — per-phase and stacked PRs are ensemble-skill behavior that `--foreman` deliberately disables. A phase-level `commit:` boolean *does* exist (this line previously listed `commit:` among the settings Foreman does not have) and controls only whether that phase commits its work; it has no effect on how many PRs a run opens. See "PR creation and merge reconciliation" in the User Guide.
