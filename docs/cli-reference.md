...
foreman merge --task bd-abc1 --target-branch dev       # Request one Elixir merge
foreman merge --list                                   # Elixir projection-backed merge candidates
foreman merge --dry-run                                # Elixir merge request preview
FOREMAN_BACKEND=node foreman merge                     # Process legacy merge queue
FOREMAN_BACKEND=node foreman merge --task bd-abc1      # Legacy merge a specific task by ID
