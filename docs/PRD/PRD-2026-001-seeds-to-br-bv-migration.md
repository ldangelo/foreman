...
| `src/orchestrator/dispatcher.ts` | `seeds.ready()`, `seeds.getGraph()`, `calculateImpactScores()`, `seeds.update()` | Replace with `BeadsRustClient` + bv ordering (always-on) |
...
The dispatcher must always call `bvClient.robotTriage()` to obtain a ranked task list when dispatching without a specific `--seed` filter. This is the default and only ordering strategy -- there is no flag to enable or disable it. Tasks are dispatched in bv's ranked order, up to `maxAgents`. When `bv` is unavailable (binary missing or timeout), the dispatcher transparently falls back to priority-field sort (P0 first, P4 last) and logs a warning.
...
`src/cli/commands/run.ts` must construct `BeadsRustClient(projectPath)` and pass it to `Dispatcher`. The `SeedsClient` construction in this file must be removed. No `--bv-triage` or similar flag is needed -- bv ordering is always active.
...
Mitigation: `detectAndFixMismatches()` only uses `seeds.show()` and `seeds.update()`, not `getGraph()`. The graph is only used in the dispatcher for ordering, which is replaced by always-on bv ordering. The `SeedGraph` type and `getGraph()` method have no consumers after the dispatcher migration.
...
| `src/orchestrator/dispatcher.ts` | Replace SeedsClient, add always-on bv ordering | Phase 1 |
