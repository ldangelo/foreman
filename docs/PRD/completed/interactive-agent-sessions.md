...
- Primary workflow: `foreman run --task X`, check `foreman status`, occasionally `foreman attach` to investigate stuck agents
...
1. User runs `foreman run --task abc123`
...
**Acceptance Criteria:**
- AC-1.1: Running `foreman run --task X` creates a tmux session where `claude` is the running process (not `tsx agent-worker.ts` directly calling SDK)
...
| Existing CLI flags | All existing `foreman run` flags must continue working: `--task`, `--attach`, `--no-attach`, `--follow`, `--model`, `--no-pipeline`, `--skip-explore`, `--skip-review`, `--resume`, `--dry-run`, `--telemetry` |
...
When: User runs `foreman run --task abc123`
