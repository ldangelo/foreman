...
```
1. Developer dispatches: foreman run --task foreman-abc1
...
**Acceptance Criteria:**
1. `foreman run --task <id>` auto-attaches to the tmux session when: (a) stdout is a TTY, (b) only one agent is being dispatched, and (c) `--no-attach` flag is not set.
2. `foreman run --task <id> --no-attach` skips auto-attach (dispatch and exit).
3. `foreman run --task <id> --attach` forces auto-attach even when dispatching multiple agents (attaches to the first).
