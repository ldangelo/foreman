...
| AC7 | `--full` mode available with formatted key-value output | Run `foreman inbox --full` → JSON bodies shown as key-value pairs (e.g., `  key: value`), long values wrapped at terminal width, non-JSON as raw text |
