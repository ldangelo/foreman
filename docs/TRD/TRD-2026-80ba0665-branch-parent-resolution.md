...
four coupled gaps that block operator ergonomics around new Foreman tasks:

1. The PR base defaults to `main` even when the worktree was branched
   from the operator's current checkout (which may be a feature branch
   like `slices/jido-migration`).
2. There is no operator-facing way to pin a task to a specific parent
   branch. The CLI's `foreman run submit` accepts no branch argument,
   and the `work.submit` envelope carries no `base_branch` field.
...
4. The Worker aggregate (`packages/foreman_server/lib/foreman_server/aggregates/worker.ex`)
   rejects a fresh `WorkerStarted` event with `{:error, :worker_terminal}`
   whenever the aggregate for `(run_id, worker_id)` is already in
   `terminal?: true` state. Because each `Overwatch.start_phase/2` call
   uses a fresh `worker_id` (generated by `generate_worker_id/0` at
   `packages/foreman_server/lib/foreman_server/overwatch.ex:196`), the
   stream id `worker:<run_id>:<worker_id>` is normally fresh, so this
   rejection should not fire on the first attempt of a phase. It does
   fire on retry / re-launch, however, and the symptom is
   `{:initialization_failed, :phase_terminal}` at p001 — which is the
   exact error that two `prd.yaml` submissions in this session produced.
   See "Worker Aggregate Re-init Bug" below for the full root-cause
   analysis.

Gaps 1–3 were exposed by PR #412 (a `fix` workflow against
...
Gap 3 was exposed by an audit of `RunExecutor.create_default_worktree/3`
...
Gap 4 was exposed by two `prd.yaml` submissions
(`run-f610a1e84e08a9f018a150604402be9c` and
`run-17491002ab0a27b062320c93480e8cbf`), both of which failed at p001
with `{:initialization_failed, :phase_terminal}`. Gap 4 blocks every
multi-phase workflow from initializing its first worker, so it must be
resolved before this TRD's other gaps can be exercised end-to-end via
`prd.yaml`. PR 0 in the master task list addresses it directly.
...
## Worker Aggregate Re-init Bug

### Symptom

`prd.yaml` and other multi-phase workflows fail at p001 with
`{:initialization_failed, :phase_terminal}`. The Phase aggregate
reports a `phase_terminal` failure, but the root cause is upstream:
`LaunchWorker` cannot append `WorkerStarted` for the phase's worker.
The error log entry is:

```text
LaunchWorker: WorkerStarted dispatch failed for <worker_id>/<run_id>:
  {:error, :worker_terminal}
```

Single-phase workflows (e.g. `fix.yaml`, `discover.yaml`) reach a
terminal state normally because they spawn only one worker. The bug is
exposed by any workflow whose worker crashes mid-launch and the
supervisor restarts the LaunchWorker.

### Root cause (confirmed against EventStore)

`WorkerSupervisor.start_worker/1` at
`packages/foreman_server/lib/foreman_server/overwatch/worker_supervisor.ex:64`
starts `LaunchWorker` with `restart: :permanent`. When the LaunchWorker
process crashes mid-activation, the supervisor restarts it **with the
same `worker_id` and `run_id`**. The restarted LaunchWorker tries to
append a fresh `WorkerStarted` event, but the Worker's stream was
sealed by a prior `WorkerCrashed` event from the previous attempt.

Direct query of the EventStore confirms the mechanism for the two
failing `prd.yaml` runs:

```sql
SELECT stream_uuid, stream_version FROM streams
WHERE stream_uuid IN (
  'worker:run-f610a1e84e08a9f018a150604402be9c:wkr-eabd9f4178d7e0af',
  'worker:run-f610a1e84e08a9f018a150604402be9c:wkr-3b0422b2efaaedc2',
  'worker:run-17491002ab0a27b062320c93480e8cbf:wkr-10f5e58f95b7ef05',
  'worker:run-17491002ab0a27b062320c93480e8cbf:wkr-72a5f7071d706d74'
);
--  worker:run-f610a1e84...:wkr-eabd9f4178d7e0af  |  19
--  worker:run-f610a1e84...:wkr-3b0422b2efaaedc2  |  23
--  worker:run-17491002ab...:wkr-10f5e58f95b7ef05  |  22
--  worker:run-17491002ab...:wkr-72a5f7071d706d74  |  23
```

Each failing stream already has 19–23 events from the prior attempt.
The event sequence for `wkr-eabd9f4178d7e0af` is, in order:

```text
  0: WorkerStarted (adapter=jido_harness, session_id=session-30bc6fe1-...)
  1: WorkerHeartbeat
  2: WorkerExited
  3: WorkerExited
  4: WorkerExited
  ...
  6: WorkerExited
  7: WorkerExited
  8: WorkerStarted            <-- second attempt
  9: WorkerHeartbeat
 10: WorkerHeartbeat
 11: WorkerExited
 12: WorkerExited
 13: WorkerStarted            <-- third attempt
 14: WorkerHeartbeat
 15: WorkerHeartbeat
 16: WorkerHeartbeat
 17: WorkerHeartbeat
 18: WorkerCrashed            <-- seals the aggregate
 19: (dispatched, REJECTED)    <-- this is what the log shows
```

The retry loop emits multiple `WorkerStarted` events before the
`WorkerCrashed` finally lands. Each retry increments the sequence by
1, so by the time the worker crashes the aggregate holds 19 events.
When the LaunchWorker restarts after the crash, it tries to append
sequence 19 (or 22, or 23 for the other streams) as another
`WorkerStarted`. `WorkerStarted` is NOT in `@terminal_events`
(worker.ex:46: `{"RunCompleted", "RunFailed", "WorkerExited",
"WorkerCrashed"}`), so `allow_after_terminal/2` (worker.ex:274-276)
rejects it with `{:error, :worker_terminal}`.

### Selected fix: pre-flight terminal check in LaunchWorker

Add a pre-flight check in `LaunchWorker.handle_continue/2` (or before
`WorkerProtocol.emit(:worker_started, ...)`) that queries
`Tracker.sequence/3` for the current `(worker_id, run_id)` key. If
the rehydrated sequence is non-negative, fetch the Worker's terminal
state via `Actor.get_state/1`. If `terminal?: true`, exit with
`{:stop, {:worker_already_terminal, {worker_id, run_id}}}` instead of
attempting the dispatch.

The supervisor (`WorkerSupervisor`) needs to handle the new exit
reason: on `{:worker_already_terminal, _}`, terminate cleanly without
restart, so the CrashLoopDetector does not loop. The Recovery
aggregate (or its equivalent) should then decide whether to open a
new worker with a fresh `worker_id` or block the run.

This is the surgical fix: it surfaces the actual cause to the operator
instead of looping silently, and gives the recovery path a clean
signal to act on.

#### Alternatives considered

##### Option A — Clear terminal state on re-launch
Append a synthetic `WorkerReset` event that clears `terminal?` before
appending `WorkerStarted`. Rejected because it pollutes the event log
and would mask the actual signal that the prior worker failed.

##### Option B — Force fresh `worker_id` per `start_phase` call (already done)
`Overwatch.generate_worker_id/0` already returns a fresh UUID per
`start_phase` call, so a re-submitted workflow gets a fresh stream.
The bug only fires when the **same LaunchWorker** retries within a
single BEAM lifetime (the `restart: :permanent` policy). Refusing
re-launch is the surgical fix.

##### Option C — Drop `restart: :permanent` on LaunchWorker
Change the child spec to `restart: :temporary` or `:transient`.
Rejected because it changes broader lifecycle semantics; the
pre-flight check is more focused.

### Touched files

| File | Change |
|---|---|
| `packages/foreman_server/lib/foreman_server/overwatch/launch_worker.ex` | Add a pre-flight check in `handle_continue/2` before `WorkerProtocol.emit(:worker_started, ...)`: if the Tracker reports a non-negative sequence and the Worker's rehydrated state is terminal, exit with `{:stop, {:worker_already_terminal, {worker_id, run_id}}}`. |
| `packages/foreman_server/lib/foreman_server/overwatch/launch_worker.ex` (telemetry) | Emit `[:foreman_server, :overwatch, :worker_already_terminal]` so operators can see the cause. |
| `packages/foreman_server/lib/foreman_server/overwatch/worker_supervisor.ex` | In `stop_worker/2`'s termination loop, recognize the new exit reason and skip the restart path. |
| `packages/foreman_server/test/foreman_server/overwatch/launch_worker_test.exs` (new) | Unit tests covering: re-launch on a non-terminal stream still works; re-launch on a terminal stream exits cleanly with `{:worker_already_terminal, _}`; fresh `worker_id` proceeds. |
| `docs/cli-reference.md`, `docs/user-guide.md` | Note: "If you see `:worker_already_terminal` in the run logs, the prior worker for that run is permanently closed. Cancel the run and start a new one with `foreman run submit`." |
...
### PR 0: Worker aggregate re-init fix (BLOCKER for multi-phase workflows)

**Shippable State:** `prd.yaml` and other multi-phase workflows
initialize their first worker without the
`{:initialization_failed, :phase_terminal}` failure. Single-phase
workflows are unaffected.

#### TRD-BPR-000 — Server: pre-flight terminal check in LaunchWorker (3h)

Edit `ForemanServer.Overwatch.LaunchWorker.handle_continue/2` (or add
a new function before `WorkerProtocol.emit(:worker_started, ...)`) to
query `Tracker.sequence/3` for the current `(worker_id, run_id)` key.
If the rehydrated sequence is non-negative, call `Actor.get_state/1`
on the Worker aggregate actor and check `state.module_state.terminal?`.
If `true`, exit with
`{:stop, {:worker_already_terminal, {worker_id, run_id}}}` and emit
`[:foreman_server, :overwatch, :worker_already_terminal]`.

Validates ACs: AC-BPR-009.

Implementation AC:

- Given a Worker aggregate with `terminal?: true` at the same
  `(worker_id, run_id)` key, when `LaunchWorker.handle_continue/2`
  runs, then it exits with
  `{:stop, {:worker_already_terminal, {worker_id, run_id}}}` instead
  of attempting the dispatch.
- Given a Worker aggregate with `terminal?: false` (or a fresh
  stream with no events), when `LaunchWorker.handle_continue/2` runs,
  then it proceeds to the dispatch as before.

#### TRD-BPR-000-TEST — Test LaunchWorker pre-flight terminal check (2h)

Unit tests covering: re-launch on a non-terminal stream still works;
re-launch on a terminal stream exits cleanly with
`{:worker_already_terminal, _}`; fresh `worker_id` proceeds; telemetry
event emitted on terminal-stream exit.

Validates ACs: AC-BPR-009.

#### TRD-BPR-000-DOCS — Note about :worker_already_terminal in user-facing docs (1h)

Add a paragraph to `docs/user-guide.md` (and reference it from
`docs/cli-reference.md`) explaining that `:worker_already_terminal`
means the prior worker for that run is permanently closed and the
operator should cancel and re-submit.

Validates ACs: AC-BPR-010.
...
| AC-BPR-009 | When a Worker aggregate is already terminal at `(worker_id, run_id)` and `LaunchWorker` starts, it exits with `{:stop, {:worker_already_terminal, {worker_id, run_id}}}` instead of looping on WorkerStarted dispatch. |
| AC-BPR-010 | `docs/user-guide.md` (referenced from `docs/cli-reference.md`) explains the `:worker_already_terminal` recovery path (cancel + re-submit). |
...
6. **Worker re-init E2E** — submit `prd.yaml` once after PR 0 lands;
   verify the first phase initializes without
   `{:initialization_failed, :phase_terminal}`. Cancel the run before
   it completes to free the worker slot.
