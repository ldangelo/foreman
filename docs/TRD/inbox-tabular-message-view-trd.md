...
6. `--full` flag preserved for detailed payload inspection with improved formatting: non-empty JSON object bodies render as key-value pairs (for example, `key: value`) with long values wrapped at terminal width; arrays, primitives, `null`, empty objects, and invalid JSON use serialized or raw fallback output
...
- [x] AC-6: `foreman inbox --full` shows non-empty JSON object bodies as formatted key-value pairs (for example, `key: value`); long values wrap at terminal width; arrays, primitives, `null`, empty objects, and invalid JSON use serialized or raw fallback output
