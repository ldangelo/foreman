...
| Check | Result |
|---|---|
| Required sections present | PASS — Product Summary/Problem context, Goals, Technical Requirements, Acceptance Criteria, Non-Goals, Dependency Map |
| REQ-NNN sequential and unique | PASS — REQ-001 … REQ-015, no gaps |
| ACs in Given/When/Then | PASS — 33/33 |
...
| Readiness score | 4.75 — PASS (≥4.0), proceed normally |
| Open ambiguity markers | 0 |

## Domain Analysis

**Project type: brownfield.** The feature extends existing workflow YAML parsing, phase execution, AutoPR, event/projection, CLI/API display, PR monitoring, and docs.

| Domain | Requirements | Existing surface |
|---|---|---|
| Manifest schema / normalization | REQ-001, REQ-002, REQ-009, REQ-010, REQ-012 | `packages/foreman_server/lib/foreman_server/workflow/interpreter.ex`, `phase_spec.ex`, `manifest_writer.ex`, commit round-trip tests |
| Phase PR orchestration | REQ-003, REQ-004, REQ-005, REQ-008, REQ-013 | `RunExecutor`, `Workflow.AutoPR`, `git`, `gh pr create` |
| Durable phase PR model | REQ-006, REQ-007, REQ-011, REQ-014 | `EventCodec`, `ProjectionStore`, run/phase projections, `PrAssociate`, `PrAssociated` |
...
|---|---|---|
| `Workflow.PhaseSpec` | Normalizes known phase fields, currently including `:commit` but not `:stack_pr` | Add `:stack_pr` as a canonical atom-keyed field; preserve explicit `false` and absence distinctly |
| `Workflow.Interpreter` | Parses booleans and validates `commit:` as boolean | Add `stack_pr:` boolean validation at load, with typed phase/key error output |
| `Workflow.ManifestWriter` | Round-trips phase fields and has commit preservation tests | Preserve explicit `stack_pr` true/false without inventing absent values |
| `RunExecutor` | Executes phase, writes artifact, commits phase work, emits terminal phase/run events, then advances | Invoke phase PR creation after phase commit/no-op decision and before `PhaseCompleted`/advance |
| `Workflow.AutoPR` | Opens one final PR from Foreman-derived head branch to recorded run base branch; returns typed success/noop/error | Extract reusable git/GitHub operations or wrap them in a new module without changing final PR semantics |
| `PrAssociated` / `pr_url` | Stores final run PR URL only | Add separate phase PR events/projection fields; do not reuse `pr_url` |
| `ProjectionStore.phases_for_run/1` | Returns ordered phase projections | Enrich phases with their phase PR records for API/CLI display |
| PR monitor | Observes PR state for run-level PR workflows | Keep phase PR monitor state separate from final run PR state |
...
| B — true stacked branch topology | Create immutable phase head branches and target phase N at phase N-1 | Clean diffs and multiple unique GitHub PRs | Explicitly out of scope; changes worktree/branch model and contradicts PRD base-branch requirement | Rejected |
| **C — phase PR records over current run branch** | Keep one run branch; open or reconcile a GitHub PR from that branch to run base at tagged phase boundaries; record each phase separately | Matches PRD target/head constraints, lowest disruption, visible phase records | Same head/base means GitHub can expose one open PR object; later tagged phases may reuse/reconcile that PR URL with cumulative diff | **Selected** |

### Key Technical Decisions

1. **`stack_pr` is a phase field only.** It is normalized once in `PhaseSpec`; downstream code reads `:stack_pr` only.
2. **Absent and `false` preserve existing behavior.** Absence remains absent in the normalized/written manifest; `false` remains explicit false.
3. **Malformed values fail at manifest load.** Strings, numbers, maps, lists, and blank nested keys are rejected with phase index and key name.
4. **`stack_pr` does not imply commit.** The phase commit path runs first; `commit: false` is valid and may produce a phase PR only if committed branch state is already ahead of base.
...
8. **GitHub duplicate open-PR limitation is handled explicitly.** When the same head/base already has an open PR, the service records a `reused`/`reconciled` phase PR record instead of treating the provider message as opaque success or creating duplicates.
9. **Closed matching PRs are typed failures.** Retrying against a closed matching head/base PR returns an actionable error.
10. **Final `pr_url` remains final-only.** Phase PR metadata lives under new event/projection fields.
11. **Final AutoPR skip consults phase PR records.** Created or reconciled phase PR records suppress final AutoPR; no-op records do not.
...
|---|---|---|
| `Workflow.PhaseSpec` | Canonical atom-keyed phase spec | Add `:stack_pr` with accepted keys `[:stack_pr, "stack_pr"]`; do not add permissive aliases unless required by docs |
...
| `Workflow.ManifestWriter` | Workflow YAML serialization | Preserve explicit `stack_pr: true/false` in phases; omit absent field |
...
| `Workflow.AutoPR` | Final PR behavior | Keep public semantics; expose small reusable helpers only if their contracts stay typed and total |
...
| `Run` aggregate / command router | Command/event contract | Add system command for recording phase PR outcomes or extend current internal dispatch with typed event payload |
| Go CLI/API renderers | Operator views | Include `phase_prs` in JSON and show phase PR links near phase status |
| `PrMonitor` | PR state observation | Either ignore phase PRs explicitly or project them separately; never treat them as final run PR merges |
...
graph TD
  ...
  K --> L[resolve run base + run branch]
...
| `phase_index` | integer | Required, zero- or one-based matching existing phase projection convention |
...
- [ ] **TRD-001** Add canonical `stack_pr` support to `Workflow.PhaseSpec` (3h) [satisfies REQ-001, REQ-002, REQ-009]
  - Validates PRD ACs: AC-001-1, AC-001-2, AC-002-2, AC-009-2
  - Implementation ACs:
    - Given `stack_pr: true`, when a phase is normalized, then `spec.stack_pr == true` under atom key `:stack_pr`.
    - Given `stack_pr: false`, when a phase is normalized, then the field is present as false and is not dropped.
...
- [ ] **TRD-001-TEST** Cover `PhaseSpec` normalization for present true, present false, and absent `stack_pr` (2h) [verifies TRD-001] [satisfies REQ-001, REQ-002, REQ-009] [depends: TRD-001]
  - Validates PRD ACs: AC-001-1, AC-001-2, AC-002-2, AC-009-2
  - Verification steps: ExUnit assertions cover string-keyed and atom-keyed phase maps.
...
- [ ] **TRD-002** Add `stack_pr` boolean validation to workflow loading (3h) [satisfies REQ-001, REQ-009, REQ-012]
  ...
  - Implementation ACs:
    - Given `stack_pr: "true"`, when `Interpreter.load/1` runs, then it returns/raises a typed manifest error naming phase 0 and `stack_pr`.
...
- [ ] **TRD-002-TEST** Add interpreter regression tests for valid and malformed `stack_pr` values (2h) [verifies TRD-002] [satisfies REQ-001, REQ-009, REQ-012] [depends: TRD-002]
  - Validates PRD ACs: AC-001-1, AC-001-2, AC-009-1, AC-009-2, AC-012-1
  - Verification steps: ExUnit tests assert error messages include phase index and key name.
...
- [ ] **TRD-003** Preserve explicit `stack_pr` values through manifest writer/read round trips (3h) [satisfies REQ-010, REQ-002]
  ...
  - Implementation ACs:
    - Given a manifest with `stack_pr: true`, when written and loaded, then the reloaded phase has boolean true.
    - Given a manifest with `stack_pr: false`, when written and loaded, then the reloaded phase has boolean false.
...
- [ ] **TRD-003-TEST** Add manifest writer round-trip tests for `stack_pr` (2h) [verifies TRD-003] [satisfies REQ-010, REQ-002] [depends: TRD-003]
  - Validates PRD ACs: AC-010-1, AC-010-2, AC-002-1
  - Verification steps: Round-trip fixture asserts true/false/absent preservation.
...
- [ ] **TRD-004** Extract or add typed git/GitHub PR helper functions for phase PR usage (5h) [satisfies REQ-003, REQ-004, REQ-005, REQ-008]
  - Validates PRD ACs: AC-003-1, AC-004-1, AC-004-2, AC-005-1, AC-005-2, AC-008-1, AC-008-2
  - Implementation ACs:
    - Given valid base/head/cwd, when commits are ahead, then helper pushes the Foreman-derived head and opens a GitHub PR with `--base <run-base>` and `--head <run-branch>`.
    - Given base or head is absent, when helper is called, then it returns a typed error and runs no `gh pr create`.
...
- [ ] **TRD-004-TEST** Characterize phase PR helper success, no-op, missing-branch, push-failure, and create-failure paths (4h) [verifies TRD-004] [satisfies REQ-003, REQ-004, REQ-005, REQ-008] [depends: TRD-004]
  - Validates PRD ACs: AC-003-1, AC-003-3, AC-004-1, AC-004-2, AC-005-2, AC-008-1, AC-008-2
  - Verification steps: Stub `git`/`gh` commands or use isolated temp repos; assert typed results, not logs only.
...
- [ ] **TRD-005-TEST** Add unit tests for `Workflow.PhasePR` typed outcomes and `commit:false` independence (4h) [verifies TRD-005] [satisfies REQ-003, REQ-008, REQ-012] [depends: TRD-005]
  - Validates PRD ACs: AC-003-1, AC-003-3, AC-008-1, AC-012-1, AC-012-2
  - Verification steps: Tests assert created/noop/error tuples and confirm no implicit commit occurs.

- [ ] **TRD-006** Invoke phase PR creation from `RunExecutor` after phase commit and before phase completion (5h) [satisfies REQ-003, REQ-008, REQ-012]
  ...
  - Implementation ACs:
    ...
    - Given `PhasePR` returns an error for proposable commits, then the phase/run terminal state records the failure instead of completing.
- [ ] **TRD-006-TEST** Add `RunExecutor` integration tests for phase PR invocation order and failure propagation (5h) [verifies TRD-006] [satisfies REQ-003, REQ-008, REQ-012] [depends: TRD-006]
  - Validates PRD ACs: AC-003-1, AC-003-2, AC-008-1, AC-012-2
  - Verification steps: Event/projection assertions show commit before PR, no PR on failed phase, and failed run on PR error.
...
- [ ] **TRD-007** Add `PhasePrRecorded` event/command codec and projection model (5h) [satisfies REQ-006, REQ-011]
  ...
  - Implementation ACs:
    - Given a created/reused phase PR, when events are read, then `run_id`, phase identity/index/name, URL, number, base, head, status, provider, and timestamp are present.
...
- [ ] **TRD-007-TEST** Cover event codec and projection behavior for `PhasePrRecorded` (4h) [verifies TRD-007] [satisfies REQ-006, REQ-011] [depends: TRD-007]
  - Validates PRD ACs: AC-006-1, AC-006-2, AC-006-3, AC-011-1
  - Verification steps: Rebuild projection from events and assert `phase_prs` plus untouched final `pr_url`.

- [ ] **TRD-008** Add phase PR idempotency and existing open/closed PR reconciliation (6h) [satisfies REQ-013, REQ-003, REQ-008]
...
- [ ] **TRD-008-TEST** Add idempotency/reconciliation tests for recorded, open-existing, and closed-existing PRs (5h) [verifies TRD-008] [satisfies REQ-013, REQ-008] [depends: TRD-008]
  - Validates PRD ACs: AC-013-1, AC-013-2, AC-013-3, AC-008-1
  - Verification steps: Stub provider responses and assert exactly zero duplicate `gh pr create` calls after a recorded/reused match.

- [ ] **TRD-009** Skip final AutoPR when any created/reused phase PR record exists (4h) [satisfies REQ-007, REQ-002]
  ...
  - Implementation ACs:
    ...
    - Given only no-op phase PR records exist and untagged committed work is ahead, when run finalizes, then existing final AutoPR eligibility still applies.
    - Given no `stack_pr` is declared, when run finalizes, then behavior matches existing AutoPR tests.
- [ ] **TRD-009-TEST** Add final AutoPR skip/no-op/default regression tests (4h) [verifies TRD-009] [satisfies REQ-007, REQ-002] [depends: TRD-009]
  - Validates PRD ACs: AC-007-1, AC-007-2, AC-002-1
  - Verification steps: Tests assert final `PrAssociated` is absent only for created/reused phase PR cases.
...
- [ ] **TRD-010-TEST** Add server/API and Go CLI JSON tests for phase PR visibility (4h) [verifies TRD-010] [satisfies REQ-011, REQ-006] [depends: TRD-010]
  - Validates PRD ACs: AC-011-1, AC-011-2, AC-006-3
  - Verification steps: ExUnit projection/API tests and Go command rendering tests assert stable fields.
...
- [ ] **TRD-011-TEST** Add human renderer/cockpit snapshot tests for phase PR labels and final PR separation (3h) [verifies TRD-011] [satisfies REQ-011, REQ-014] [depends: TRD-011]
  - Validates PRD ACs: AC-011-2, AC-014-1, AC-014-2
  - Verification steps: Snapshot or string tests show phase PR section and unchanged final PR label.
...
- [ ] **TRD-012-TEST** Add PR monitor regression tests for phase PR merge and final PR state separation (4h) [verifies TRD-012] [satisfies REQ-014, REQ-006] [depends: TRD-012]
  - Validates PRD ACs: AC-014-1, AC-014-2, AC-006-2
  - Verification steps: Tests simulate monitor update for phase PR and assert no premature run/task final merge effect.

- [ ] **TRD-013** Update living docs for `stack_pr:` syntax, semantics, failure modes, and stale no-per-phase-PR statements (4h) [satisfies REQ-015]
...
- [ ] **TRD-013-TEST** Verify doc updates and command/source consistency for `stack_pr:` (2h) [verifies TRD-013] [satisfies REQ-015] [depends: TRD-013]
  - Validates PRD ACs: AC-015-1, AC-015-2
  - Verification steps: Grep living docs for `stack_pr`; verify CLI docs match Go source/API fields.
...
| TRD-007 | TRD-005 | Event shape records service result |
...
Critical path: TRD-001 → TRD-002 → TRD-003 and TRD-004 → TRD-005 → TRD-006 → TRD-007 → TRD-008 → TRD-009/TRD-010 → TRD-011/TRD-012 → TRD-013. No circular dependencies identified.
...
| REQ-004 | Target every phase PR at run base branch | TRD-004, TRD-005 | TRD-004-TEST |
| REQ-005 | Use run worktree branch as phase PR head | TRD-004, TRD-005 | TRD-004-TEST |
...
1. **Issue:** GitHub normally allows only one open PR per head/base pair, while the PRD asks repeated tagged phases to use the same run branch and run base.  
...
3. **Issue:** Machine task parsing can miss lines without checkbox prefixes.  
   **Resolution:** Every implementation and test task begins with `- [ ] **TRD-NNN**` or `- [ ] **TRD-NNN-TEST**`; validated with `trd-cli parse`.
...
1. **Issue:** Long chain TRD-004 → TRD-005 → TRD-006 → TRD-007 → TRD-008 → TRD-009/010 could delay visible CLI/API work.  
   **Resolution:** Keep tasks ≤6h, ship PR 2 with phase behavior before PR 3/4, and allow TRD-010 API shape drafting once TRD-007 event fields are stable.
...
1. **Issue:** "Visible in supported CLI view" is broad unless a first surface is named.  
   **Resolution:** TRD-010 names `foreman run get <id> --json`; TRD-011 covers supported human view snapshots after existing renderer discovery.
2. **Issue:** "Fail loudly" can be subjective.  
   **Resolution:** ACs require typed error terms with base/head/phase/command output and phase/run failure projection assertions.
3. **Issue:** Docs verification can become subjective.  
...
|---|---:|---|
| Architecture completeness | 4 | Components, data flow, event shape, and error paths are defined; exact `gh` reconciliation command shape remains implementation detail. |
| Task coverage | 5 | All 15 PRD requirements have implementation and test coverage. |
| Dependency clarity | 4 | Dependencies are explicit and acyclic; PR 3/4 naturally depend on projection contract stability. |
| Estimate confidence | 5 | All tasks are ≤6h with paired tests and focused scopes. |
