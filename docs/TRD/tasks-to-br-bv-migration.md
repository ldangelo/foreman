# TRD: Migrate Task Management from tasks (sd) to br + bv
...
This TRD translates PRD-2026-001 into an executable implementation plan for migrating Foreman's task management from tasks (sd) to br (beads_rust) with bv (beads_viewer) as the always-on dispatch ordering engine. The migration spans 4 sprints across 4 weeks, producing 28 implementation tasks with paired verification tasks, organized into Foundation, Runtime Core, Templates/Init, and Cleanup phases.
...
Both TasksClient and BeadsRustClient will implement a shared ITaskClient interface during the transition period (Sprint 2). This enables the feature flag to swap implementations without changing consumer code in Dispatcher, Monitor, and CLI commands.
...
- [x] **TRD-NF-004**: Backwards compatibility for in-flight Postgres runs (1h) [satisfies REQ-NF-004]
  - Postgres task_id column stores IDs compatible with both sd and br formats
...
- [x] **TRD-004**: Implement foreman migrate-tasks command (4h) [satisfies REQ-021, REQ-022, REQ-023]
  - Create src/cli/commands/migrate-tasks.ts
  - Register in commander CLI
  - Read .tasks/issues.jsonl line by line
  - For each task: call br create with mapped fields (type, priority as number, title, description)
  - Track old-task-ID to new-br-ID mapping
  - After all creates: replay blocks dependency edges via br dep add
  - Tasks with status: in_progress created as status: open in br
  - Tasks with status: closed created then immediately closed via br close
  - Idempotency: skip issues that already exist (match by title)
  - Report: created count, skipped count, failed count
  - Write migration report to docs/tasks-migration-report.md
  - --dry-run flag: report what would happen without creating

- [x] **TRD-004-TEST**: Unit and integration tests for migrate-tasks (3h) [verifies TRD-004]
  - Test reads .tasks/issues.jsonl correctly
  - Test creates br issues with correct field mapping
  - Test priority "P2" maps to numeric 2 in br create
  - Test in_progress tasks created as open in br
  - Test closed tasks created and closed in br
  - Test dependency edges preserved (A blocks B in tasks --> A blocks B in br)
  - Test idempotency: re-run skips existing issues by title
  - Test dry-run produces report without creating issues
  - Test handles missing .tasks/issues.jsonl gracefully
  - Test handles empty .tasks/issues.jsonl
...
- [x] **TRD-005**: Update Dispatcher to accept BeadsRustClient (4h) [satisfies REQ-001]
  ...
  - Make both TasksClient and BeadsRustClient implement ITaskClient
...
- [x] **TRD-005-TEST**: Unit tests for Dispatcher with ITaskClient (3h) [verifies TRD-005]
  ...
  - Test Dispatcher accepts TasksClient via ITaskClient (backward compat)
...
- [x] **TRD-007**: Update run.ts to instantiate BeadsRustClient (2h) [satisfies REQ-007]
  ...
  - If "sd": construct TasksClient(projectPath) (existing behavior)
...
- [x] **TRD-007-TEST**: Unit tests for run.ts client selection (1h) [verifies TRD-007]
  ...
  - Test FOREMAN_TASK_BACKEND=sd instantiates TasksClient
  - Test default (unset) instantiates TasksClient
...
- [x] **TRD-008**: Update reset.ts to use BeadsRustClient (2h) [satisfies REQ-008]
  ...
  - Replace tasks.update() with brClient.update() when backend=br
  - Replace tasks.show() with brClient.show() when backend=br
...
- [x] **TRD-009**: Update Monitor to use BeadsRustClient (2h) [satisfies REQ-009]
  ...
  - Replace tasks.show() with taskClient.show() for completion detection
...
- [x] **TRD-010**: Update agent-worker.ts finalize() (2h) [satisfies REQ-013]
  - Read FOREMAN_TASK_BACKEND env var
  - When backend=br: call ~/.local/bin/br close <taskId> --reason "Completed via pipeline"
...
- [x] **TRD-011**: Update agent-worker.ts markStuck() (1h) [satisfies REQ-014]
  - Read FOREMAN_TASK_BACKEND env var
  - When backend=br: call ~/.local/bin/br update <taskId> --status open
...
- [x] **TRD-014**: Update worker-agent.md template (1h) [satisfies REQ-015]
  - Replace sd update TASK_ID --claim with br update TASK_ID --status in_progress
  - Replace sd close TASK_ID --reason "Completed" with br close TASK_ID --reason "Completed"
  - Replace sd update TASK_ID --notes "Blocked: ..." with br update TASK_ID --description "Blocked: ..."
...
- [x] **TRD-015**: Update foreman task command (2h) [satisfies REQ-017]
  - Replace TasksClient with BeadsRustClient in src/cli/commands/task.ts
...
- [x] **TRD-015-TEST**: Unit tests for foreman task with br (1h) [verifies TRD-015]
  - Test task command creates issues via BeadsRustClient
  - Test priority input normalized correctly

- [x] **TRD-016**: Update foreman plan command (2h) [satisfies REQ-018]
  - Replace TasksClient with BeadsRustClient in src/cli/commands/plan.ts
...
- [x] **TRD-017**: Update foreman merge command (2h) [satisfies REQ-019]
  - Replace TasksClient with BeadsRustClient in src/cli/commands/merge.ts
...
- [x] **TRD-024**: Remove FOREMAN_TASK_BACKEND feature flag (2h)
  ...
  - Remove TasksClient construction from all CLI commands
  - [depends: TRD-023]
...
- [x] **TRD-025**: Archive/delete tasks.ts and deprecated aliases (2h)
  - Delete or move src/lib/tasks.ts to src/lib/tasks.deprecated.ts
  - Remove deprecated BeadsClient, Bead, BeadDetail, BeadGraph aliases
  - Remove execSd function
  - Update any remaining imports
  - [depends: TRD-024]

- [x] **TRD-025-TEST**: Verify no tasks.ts imports remain (0.5h) [verifies TRD-025]
  - Test: grep for "TasksClient" in src/ returns zero matches (except archived files)
...
- [x] **TRD-027**: Update all test mocks to BeadsRustClient (3h)
  - Replace all TasksClient mocks in test files with BeadsRustClient mocks
...
- [x] **TRD-027-TEST**: Verify test suite passes with br-only mocks (1h) [verifies TRD-027]
  ...
  - Test: no TasksClient mock references in test files

- [x] **TRD-028**: Final documentation pass (2h)
  - Update CLAUDE.md: replace all sd references with br/bv
  - Update any README or docs referencing tasks commands
...
- [x] **TRD-028-TEST**: Verify documentation accuracy (1h) [verifies TRD-028]
  ...
  - Test: grep for "tasks" in CLAUDE.md returns only historical references
...
| TRD-004 | Implement foreman migrate-tasks command | 4h | TRD-003 | [ ] |
| TRD-004-TEST | Tests for migrate-tasks | 3h | TRD-004 | [ ] |

**Exit Criteria:**
- npm test passes
- npx tsc --noEmit passes
- foreman migrate-tasks --dry-run works against existing .tasks/ data
...
| TRD-015 | Update foreman task command | 2h | TRD-005, TRD-003 | [ ] |
| TRD-015-TEST | Tests for foreman task with br | 1h | TRD-015 | [ ] |
...
**Goal:** Remove all tasks/sd infrastructure, feature flags, and deprecated code.

| Task ID | Description | Est. | Dependencies | Status |
|---------|-------------|------|--------------|--------|
| TRD-024 | Remove feature flag | 2h | TRD-023 | [ ] |
| TRD-024-TEST | Verify no flag references | 1h | TRD-024 | [ ] |
| TRD-025 | Archive/delete tasks.ts | 2h | TRD-024 | [ ] |
| TRD-025-TEST | Verify no tasks imports | 0.5h | TRD-025 | [ ] |
...
**Exit Criteria:**
- grep -r "TasksClient|execSd|~/.bun/bin/sd" src/ returns zero results
...
|-----------------|-------------|---------------------|------------|
| REQ-001 | Replace TasksClient in dispatcher with BeadsRustClient | TRD-005 | TRD-005-TEST |
...
| REQ-017 | Update foreman task for br | TRD-015 | TRD-015-TEST |
...
| REQ-021 | Implement foreman migrate-tasks | TRD-004 | TRD-004-TEST |
| REQ-022 | Migration preserves dependency graph | TRD-004 | TRD-004-TEST |
| REQ-023 | Migration handles in_progress tasks | TRD-004 | TRD-004-TEST |
...
| AC-009 | foreman migrate-tasks is idempotent | TRD-004-TEST |
| AC-010 | migrate-tasks preserves dependency graph | TRD-004-TEST |
  ...
  builders:
    - agent: backend-developer
      ...
      domains:
        ...
        - cli-commands (run.ts, reset.ts, status.ts, init.ts, doctor.ts, task.ts, plan.ts, merge.ts, sling.ts, migrate-tasks.ts)
...
| RISK-008 | TaskGraph type consumers break | Low | Low | Only dispatcher uses graph; replaced by bv | TRD-006, TRD-026 |
...
2. grep -r "TasksClient|execSd|~/.bun/bin/sd" src/ returns zero results
...
| src/cli/commands/migrate-tasks.ts | Create | 1 | TRD-004 |
| src/orchestrator/dispatcher.ts | Replace TasksClient, add bv ordering, update PATH | 2 | TRD-005, TRD-006, TRD-012 |
| src/orchestrator/agent-worker.ts | Replace sd binary paths | 2 | TRD-010, TRD-011 |
| src/orchestrator/monitor.ts | Replace TasksClient | 2 | TRD-009 |
| src/cli/commands/run.ts | Replace TasksClient instantiation | 2 | TRD-007 |
| src/cli/commands/reset.ts | Replace TasksClient | 2 | TRD-008 |
| templates/worker-agent.md | Replace sd commands with br | 3 | TRD-014 |
| src/cli/commands/task.ts | Replace TasksClient | 3 | TRD-015 |
| src/cli/commands/plan.ts | Replace TasksClient | 3 | TRD-016 |
| src/cli/commands/merge.ts | Replace TasksClient | 3 | TRD-017 |
...
| src/lib/tasks.ts | Archive or delete | 4 | TRD-025 |
