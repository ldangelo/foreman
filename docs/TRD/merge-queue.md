...
| ID | Task | Est. | Deps | Files | Status |
|----|------|------|------|-------|--------|
| MQ-T058 | Implement `foreman merge --dry-run`: for each queued/completed branch, show branch name, task ID, `git diff --stat` output, conflict detection via `git merge-tree`. Support `--task <id>` filter. No git state modification | 4h | MQ-T008 | `src/cli/commands/merge.ts`, `src/orchestrator/merge-queue.ts` | [x] |
...
| MQ-T060 | Write tests for dry-run -- no git state modified, conflict detection accurate, `--task` filter works, graceful degradation without FR-7 data | 3h | MQ-T058, MQ-T059 | `src/cli/__tests__/merge-dryrun.test.ts` | [x] |
...
- [ ] AC-8.3: Works with `--task <id>` filter
