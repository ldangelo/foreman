...
const TEMPLATE_DIR = join(dirname(fileURLToPath(import.meta.url)), "..", "defaults", "prompts", "default");
/**
...
export function loadTemplate(filename) {
    ...
    if (filename.includes("/") || filename.includes("\\")) {
        throw new Error(`loadTemplate expects a bare filename, not a path (got "${filename}")`);
    }
    ...
    try {
        ...
        throw new Error(`Failed to load template "${filename}" from ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
    }
 ...
 * @param variables - Key/value pairs to substitute
 */
export function loadAndInterpolate(filename, variables) {
