 ...
 * Exported for unit testing.
 */
export async function initProjectStore(projectDir, projectName, store) {
    let projectId;
 ...
 */
export function installPrompts(projectDir, force = false) {
...
export const initCommand = new Command("init")
    ...
    console.log(chalk.bold(`Initializing foreman project: ${chalk.cyan(projectName)}`));
    ...
    try {
        ...
        if (installed.length > 0) {
            spinner.succeed(`Installed ${installed.length} prompt template(s) to .foreman/prompts/`);
        }
        else if (skipped.length > 0) {
            spinner.info(`Prompt templates already installed (${skipped.length} skipped). Use --force to overwrite.`);
        }
        else {
    ...
    try {
        ...
        if (skillsInstalled.length > 0) {
            skillSpinner.succeed(`Installed ${skillsInstalled.length} Pi skill(s) to ~/.pi/agent/skills/`);
        }
        else {
    ...
    try {
        ...
        if (workflowsInstalled.length > 0) {
            workflowSpinner.succeed(`Installed ${workflowsInstalled.length} workflow config(s) to .foreman/workflows/`);
        }
        else if (workflowsSkipped.length > 0) {
            workflowSpinner.info(`Workflow configs already installed (${workflowsSkipped.length} skipped). Use --force to overwrite.`);
        }
        else {
