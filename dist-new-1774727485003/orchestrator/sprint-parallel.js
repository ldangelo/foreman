 ...
 */
export function buildSprintDepGraph(sprints) {
    // Build task ID → sprint index lookup
    const taskToSprint = new Map();
 ...
 */
export function computeParallelGroups(graph, sprintCount) {
 ...
 */
export function parseTrdParallelNotes(content) {
    const pairs = [];
    ...
    for (const line of lines) {
        ...
        const parallelMatch = line.match(/Sprint\s+(\d+[a-z]?)\s+and\s+Sprint\s+(\d+[a-z]?)\s+can\s+(run\s+in\s+)?parallel/i);
        if (parallelMatch) {
            pairs.push({
 ...
 */
export function validate(groups, statedPairs, sprints) {
    const warnings = [];
    ...
    for (const { sprintA, sprintB } of statedPairs) {
        ...
        if (!computedPairs.has(key)) {
            warnings.push(`TRD states Sprint ${sprintA} and Sprint ${sprintB} are parallel, ` +
    ...
    const statedKeys = new Set(statedPairs.map(({ sprintA, sprintB }) => `${Math.min(sprintA, sprintB)}-${Math.max(sprintA, sprintB)}`));
    for (const key of computedPairs) {
        if (!statedKeys.has(key)) {
            const [a, b] = key.split("-");
            warnings.push(`Auto-computed: Sprint ${a} and Sprint ${b} can run in parallel ` +
 ...
 */
export function analyzeParallel(plan, trdContent) {
...
//# sourceMappingURL=sprint-parallel.js.map